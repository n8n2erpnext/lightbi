...
const SINGLE_CSV_FILES = ['sample data/sample-data-audit/performance/good_performance.csv'];
  ...
  const processTarget = async (target, isMulti) => {
    ...
    try {
      ...
      let waitRes = await Promise.race([
        page.waitForSelector('text=Explore dataset structure', { timeout: 30000 }).then(() => 'success'),
        page.waitForSelector('text=Analyze', { timeout: 30000 }).then(() => 'success'),
        page.waitForSelector('text=Dataset overview', { timeout: 30000 }).then(() => 'success')
      ]).catch(() => 'timeout');
