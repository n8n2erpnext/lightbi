      ...
      <div className="flex max-h-[85vh] w-full max-w-6xl flex-col rounded-xl border border-black/10 bg-white shadow-xl">
        <div className="flex items-start justify-between gap-4 border-b border-black/10 px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-[12px] text-gray-500">{t(`Showing ${Math.min(rows.length, 100).toLocaleString()} retained representative rows from ${Number(dataset.rows_count || 0).toLocaleString()} full-source rows. Analysis still runs against the governed full source when available.`)}</p>
          </div>
          <button type="button" onClick={onClose} className="rounded-md p-1.5 text-gray-500 hover:bg-gray-100" aria-label={t('Close data preview')}><X className="h-4 w-4" /></button>
        </div>
