...
function focusType(value: string): string { return value.split('_').map(part => part ? part[0].toUpperCase() + part.slice(1) : part).join(' '); }
...
export const FocusSubjectDeepAnalysisPanel: React.FC<{ action: AnalysisAction; comparison: FocusSubjectComparison }> = ({ action, comparison }) => {
  ...
  return <section data-testid="focus-deep-analysis" data-layout="management-document" className="space-y-5">
    <section className="grid gap-3 border-y border-[var(--lb-divider)] py-4 md:grid-cols-[44px_minmax(0,1fr)]" data-section-number="01">
      <div className="text-[11px] font-semibold tracking-[0.14em] text-slate-400">01</div><div><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-violet-700"><Target className="h-4 w-4" />Focus retained across Deep BA</div><div className="mt-2 flex flex-wrap items-baseline gap-2"><span className="rounded border border-violet-200 bg-violet-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-violet-700">{focusType(comparison.subject.canonicalId)}</span><h3 className="text-xl font-semibold text-slate-950">{comparison.subject.displayLabel}</h3></div><p className="mt-1 max-w-4xl text-sm leading-6 text-slate-600"><span className="font-medium text-slate-800">Perspective:</span> {action.opportunityName}. {narrative.summary}</p></div>
    </section>
