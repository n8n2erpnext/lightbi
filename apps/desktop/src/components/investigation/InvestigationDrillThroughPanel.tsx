...
export const InvestigationDrillThroughPanel: React.FC<InvestigationDrillThroughPanelProps> = ({
  ...
  const breakdowns = useMemo(() => drillResult
    ? buildDrillBreakdowns(drillResult.columns, drillResult.rows, drillResult.point.dimensionField)
    : [], [drillResult]);
  if (!isDrilling && !drillError && !drillResult) return null;
  const t = (en: string, vi: string) => preferences.language === 'vi' ? vi : en;
  ...
  return <div className="mt-5 rounded-lg border border-black/10 bg-white shadow-sm">
    ...
    {drillResult && drillResult.rows.length > 0 && <div className="p-4">
      ...
      {drillResult.rows.length > 500 && <p className="mt-2 text-xs text-gray-500">{t('The table shows the first 500 rows. Export includes every selected row in this drill-through result.', 'Bảng hiển thị 500 dòng đầu. File xuất bao gồm toàn bộ các dòng đã chọn trong kết quả phân tích này.')}</p>}
    </div>}
  </div>;
};
