...
export const InvestigationDrillThroughPanel: React.FC<InvestigationDrillThroughPanelProps> = ({
  ...
  return <div data-testid="investigation-drill-through" className="mt-5 rounded-lg border border-black/10 bg-white shadow-sm">
    ...
    {drillResult && drillResult.rows.length > 0 && <div className="p-4">
      ...
      <div className="mb-3 flex items-center justify-between text-xs text-gray-500"><span>{formatValue(selectedRows.length, 'number', preferences)} / {formatValue(drillResult.rows.length, 'number', preferences)} {t('rows selected for export')}</span>{drillResult.rows.length >= drillResult.maxRows && <span className="rounded bg-amber-50 px-2 py-0.5 font-medium text-amber-700">{t('Limited to')} {formatValue(drillResult.maxRows, 'number', preferences)} {t('rows')}</span>}</div>
      ...
      {drillResult.rows.length > 500 && <p className="mt-2 text-xs text-gray-500">{t('The table shows the first 500 rows. Export includes every selected row in this drill-through result.')}</p>}
    </div>}
  </div>;
};
