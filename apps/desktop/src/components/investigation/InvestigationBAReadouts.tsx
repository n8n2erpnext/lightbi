...
function formatMetricDelta(metric: FusionMetricDelta | undefined, preferences: DisplayPreferences, language: UiLanguage): string {
  if (!metric) return pickUiText(language, 'Not enough evidence in the current files.');
  ...
  return pickUiText(language, `${metric.label} ${direction} ${value} (${percent}) versus the previous period.`);
}
...
function buildChartTrendStatement(chartModel: ChartPreviewModel | null, preferences: DisplayPreferences, language: UiLanguage): string | null {
  ...
  return pickUiText(language, `${chartModel.yField} ${direction} from ${firstText} at ${String(first[chartModel.xField])} to ${lastText} at ${String(last[chartModel.xField])}${suffix}.`);
}
...
export const BusinessFusionAngleReadout: React.FC<{
  ...
  const focusTitle = intent === 'profitability' ? t('Profit and cash-flow focus') : intent === 'product' ? t('Product performance focus') : intent === 'operations' ? t('Operational movement focus') : intent === 'money' ? t('Money trend focus') : t('Selected decision angle focus');
  const mainAnswer = intent === 'product' && (topRow || productFallback)
    ? topRow
      ? t(`${topRow.label} is the strongest item in this chart for ${topRow.field}, contributing ${formatValue(topRow.value, 'currency', preferences, { compact: true })}.`)
  ...
  return (
    <section className="mb-5 rounded-xl border border-violet-100 bg-white shadow-sm">
      <div className="border-b border-violet-100 bg-violet-50/60 px-4 py-3">
        <p className="text-[11px] font-semibold uppercase tracking-wide text-violet-700">{t('Selected angle BA readout')}</p>
        <h3 className="mt-1 text-lg font-semibold text-[#202123]">{focusTitle}</h3>
        <p className="mt-1 text-[12px] leading-5 text-black/55">{t('Angle')}: {localize(action.opportunityName)} · {t('Chart')}: {localize(chartModel?.title || action.label)} · {t('Main measure')}: {chartModel?.yField || action.measures[0] || t('detected metric')}</p>
      </div>
      <div className="grid gap-3 p-4 lg:grid-cols-2">
        <div className="rounded-lg border border-emerald-100 bg-emerald-50 p-3 text-emerald-950"><p className="text-[13px] font-semibold">{t('Main answer')}</p><p className="mt-1 text-[13px] leading-6">{localize(mainAnswer)}</p></div>
        <div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-blue-950"><p className="text-[13px] font-semibold">{t('Why this chart exists')}</p><p className="mt-1 text-[13px] leading-6">{t(`LightBI selected ${chartModel?.chartType || 'this chart'} because the active angle combines ${action.dimensions.join(', ') || chartModel?.xField || 'a business dimension'} with ${action.measures.join(', ') || chartModel?.yField || 'a measurable outcome'}.`)}</p></div>
        <div className="rounded-lg border border-black/10 bg-white p-3"><p className="text-[13px] font-semibold text-[#202123]">{t('Overview analysis')}</p><ul className="mt-2 space-y-1 text-[12px] leading-5 text-black/65">{overviewLines.length > 0 ? overviewLines.map(line => <li key={line}>- {localize(line)}</li>) : <li>{t('No overview line could be safely generated for this angle.')}</li>}</ul></div>
        <div className="rounded-lg border border-black/10 bg-white p-3"><p className="text-[13px] font-semibold text-[#202123]">{t('Details and drivers')}</p><ul className="mt-2 space-y-1 text-[12px] leading-5 text-black/65">{detailDrivers.length > 0 ? detailDrivers.slice(0, 6).map(line => <li key={line}>- {localize(line)}</li>) : <li>{t('No ranked driver was available for this chart.')}</li>}</ul></div>
        <div className="rounded-lg border border-amber-100 bg-amber-50 p-3 text-amber-950 lg:col-span-2"><p className="text-[13px] font-semibold">{t('Risk and caveat')}</p><ul className="mt-2 space-y-1 text-[12px] leading-5 opacity-80">{riskLines.length > 0 ? riskLines.map(line => <li key={line}>- {localize(line)}</li>) : <li>{t('Use this as directional analysis until reconciliation, margin, and source-quality checks are reviewed.')}</li>}</ul></div>
      </div>
    </section>
  );
};
