...
export const UpdateStatusBar: React.FC = () => {
  ...
  const progress = updater.status === 'verifying' || updater.status === 'ready' ? 100 : Math.max(0, Math.min(100, updater.progress ?? 0));
    ...
    <div className="mx-auto flex max-w-[1440px] items-center gap-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-3 text-xs font-semibold text-slate-800">
          ...
          {!failed && <span data-testid="global-update-percent" className="tabular-nums text-slate-500">{progress}%</span>}
        </div>
      ...
      <div className="flex shrink-0 items-center gap-2">
        {ready && !updater.qaSimulation && <button type="button" onClick={()=>void updater.install()} className="rounded-md bg-emerald-700 px-3 py-1.5 text-xs font-semibold text-white">Update now</button>}
