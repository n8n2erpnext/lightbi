...
export const BusinessComparisonBriefCard: React.FC<BusinessComparisonBriefCardProps> = ({ brief, onApplyPeriodLabels, compact = false }) => {
  ...
  return (
    <section data-testid="comparison-management-document" data-layout="management-document" className="py-1">
      <section data-testid="comparison-management-section-01" data-section-number="01" data-report-section="true" data-report-role="executive_summary" data-report-keep-together="true" className="grid gap-3 border-y border-[var(--lb-divider)] py-5 md:grid-cols-[44px_minmax(0,1fr)]">
      ...
      <section data-testid="comparison-management-section-05" data-section-number="05" className="grid gap-3 py-4 md:grid-cols-[44px_minmax(0,1fr)]">
        ...
        <div><h4 className="text-[12px] font-semibold uppercase tracking-wide text-slate-600">{t('Supporting context & evidence')}</h4>
          {(criticalReason || brief.caveats.length > 0) && <section data-report-section="true" data-report-role="recommendations_risks" data-report-keep-together="true" className="border-l-2 border-amber-300 py-3 pl-3"><div className="flex items-start gap-2">{criticalReason ? <ShieldAlert className="mt-0.5 h-4 w-4 shrink-0 text-red-600" /> : <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" />}<div><p className="text-[13px] font-semibold text-[#202123]">{t(criticalReason ? 'Decision caveat' : 'Review before deciding')}</p><div className="mt-1 space-y-1 text-[12px] leading-5 text-black/60">{criticalReason && <p>{t(criticalReason.statement)}</p>}{brief.caveats.map(caveat => <p key={caveat}>- {t(caveat)}</p>)}</div></div></div></section>}
          <details data-testid="comparison-supporting-context" data-report-section="true" data-report-role="evidence_appendix" data-report-splittable="true" data-report-break-before="true" data-report-export-expand="true" className="mt-2 border-y border-[var(--lb-divider)] py-3"><summary className="cursor-pointer text-[12px] font-semibold text-slate-700">{t('Supporting context & evidence')}</summary><div className="mt-3 grid gap-5 lg:grid-cols-2">
