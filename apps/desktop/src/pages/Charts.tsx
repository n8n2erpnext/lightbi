...
export const Charts: React.FC = () => {
  ...
  const handleCreateDashboard = () => {
    const id = createDashboard(newDashboardName || 'Decision dashboard');
    setSelectedDashboardId(id);
    setNewDashboardName('');
  ...
  const handleCreateFromTemplate = (template: ChartTemplate) => {
    ...
    setNotice('Template saved as a chart card and added to dashboard.');
  };
    ...
    <div className="flex h-full min-h-0 flex-col overflow-y-auto bg-[#fbfbfa] text-[#202123]">
      <div className="mx-auto flex w-full max-w-[1320px] flex-col gap-5 px-5 py-8 md:px-8 lg:px-10">
        <header className="flex flex-col gap-4 border-b border-black/10 pb-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="mb-2 flex items-center gap-2 text-[12px] font-medium text-black/45"><BarChart3 className="h-4 w-4" strokeWidth={1.7} /> Chart Library</div>
            <h1 className="text-[28px] font-semibold tracking-normal text-[#202123]">Reusable charts for decision dashboards</h1>
            <p className="mt-2 max-w-2xl text-[14px] leading-6 text-black/50">Save chart cards from Simple BA or Advanced results, then place them into dashboards that refresh when the dataset changes.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link to="/investigation" className="rounded-md border border-black/10 bg-white px-3 py-2 text-[13px] font-medium text-black/65 shadow-sm transition-colors hover:bg-black/[0.035]">Create from BA brief</Link>
            <Link to={selectedDashboard ? `/dashboards/${selectedDashboard.id}` : '/dashboards'} className="inline-flex items-center gap-2 rounded-md bg-[#202123] px-3 py-2 text-[13px] font-medium text-white transition-colors hover:bg-black"><LayoutDashboard className="h-4 w-4" strokeWidth={1.7} /> Open dashboard</Link>
          </div>
        </header>
        ...
        <section className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <div className="rounded-lg border border-black/10 bg-white p-4 shadow-sm"><div className="text-[12px] font-medium text-black/45">Saved charts</div><div className="mt-2 text-2xl font-semibold">{charts.length}</div></div>
          <div className="rounded-lg border border-black/10 bg-white p-4 shadow-sm"><div className="text-[12px] font-medium text-black/45">Chart templates</div><div className="mt-2 text-2xl font-semibold">{chartTemplates.length}</div></div>
          <div className="rounded-lg border border-black/10 bg-white p-4 shadow-sm"><div className="text-[12px] font-medium text-black/45">Dashboard cards placed</div><div className="mt-2 text-2xl font-semibold">{selectedDashboard?.widgets.length ?? 0}</div></div>
        </section>
        ...
        <section className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="rounded-lg border border-black/10 bg-white shadow-sm">
            <div className="flex flex-col gap-3 border-b border-black/5 px-5 py-4 md:flex-row md:items-center md:justify-between">
              <div><h2 className="text-[15px] font-semibold">Saved chart cards</h2><p className="mt-1 text-[13px] text-black/45">Pick a chart card, then add it to the selected dashboard.</p></div>
              <div className="relative w-full md:w-80"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-black/35" strokeWidth={1.7} /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search saved charts" className="h-9 w-full rounded-md border border-black/10 bg-white pl-9 pr-3 text-[13px] outline-none placeholder:text-black/30 focus:border-black/25" /></div>
            </div>
                  ...
                  <div key={chart.id} className="rounded-lg border border-black/10 bg-[#fbfbfa] p-4 transition-colors hover:bg-white">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="text-[13px] text-black/45">Dataset: {dataset?.name ?? chart.datasetId}</p>
                      </div>
                      <button disabled={alreadyPlaced} onClick={() => handleAddChart(chart.id)} className="rounded-md border border-black/10 bg-white px-2.5 py-1.5 text-[12px] font-medium text-black/65 shadow-sm transition-colors hover:bg-black/[0.035] disabled:text-emerald-700 disabled:opacity-80">{alreadyPlaced ? 'Added' : 'Add'}</button>
                    </div>
                    <div className="mt-4 h-32 rounded-md border border-dashed border-black/10 bg-white">
                      {getSavedChartPayload(chart)
                        ...
                        : <div className="flex h-full items-center justify-center px-4 text-center text-[12px] text-black/35">Needs data binding from a BA or Advanced result.</div>}
                    </div>
                  </div>
                );
              })}
              {filteredCharts.length === 0 && <div className="col-span-full flex min-h-40 flex-col items-center justify-center rounded-lg border border-dashed border-black/10 bg-[#fbfbfa] p-6 text-center"><Sparkles className="mb-3 h-5 w-5 text-black/35" strokeWidth={1.7} /><h3 className="text-[14px] font-semibold">No saved chart matched</h3><p className="mt-1 max-w-sm text-[13px] text-black/45">Create a chart from a template below or save one from an analysis result.</p></div>}
            </div>
          </div>

          <aside className="rounded-lg border border-black/10 bg-white shadow-sm">
            <div className="border-b border-black/5 px-5 py-4">
              <h2 className="text-[15px] font-semibold">Dashboard target</h2>
              <p className="mt-1 text-[13px] text-black/45">Choose where chart cards will be placed.</p>
            </div>
            <div className="space-y-3 p-5">
              ...
              <div className="flex gap-2">
                <input value={newDashboardName} onChange={event => setNewDashboardName(event.target.value)} placeholder="New dashboard name" className="h-9 min-w-0 flex-1 rounded-md border border-black/10 px-3 text-[13px] outline-none" />
                <button onClick={handleCreateDashboard} className="inline-flex h-9 items-center gap-1 rounded-md bg-gray-900 px-3 text-[12px] font-medium text-white"><Plus className="h-3.5 w-3.5" /> New</button>
              </div>
              <div className="rounded-md border border-black/10 bg-[#fbfbfa] p-3">
                <div className="mb-3 flex items-center justify-between text-[12px]"><span className="font-semibold">{selectedDashboard?.name ?? 'No dashboard'}</span><span className="text-black/40">{selectedDashboardCharts.length} charts</span></div>
                <div className="grid grid-cols-2 gap-2">
                  ...
                  {selectedDashboardCharts.length === 0 && <div className="col-span-2 rounded border border-dashed border-black/10 bg-white p-4 text-center text-[12px] text-black/40">No chart cards placed yet.</div>}
                </div>
              </div>
            </div>
          </aside>
        ...
        <section className="rounded-lg border border-black/10 bg-white shadow-sm">
          <div className="grid gap-3 border-b border-black/5 px-5 py-4 md:grid-cols-[1fr_240px] md:items-end">
            <div><h2 className="text-[15px] font-semibold">Chart templates</h2><p className="mt-1 text-[13px] text-black/45">Create a reusable chart card from a pattern, then tune it later with real dataset fields.</p></div>
            ...
            {chartTemplates.map(template => {
              ...
              return (
                <div key={template.id} className="rounded-lg border border-black/10 bg-[#fbfbfa] p-4">
                  ...
                  <p className="mt-1 text-[13px] leading-5 text-black/50">{template.intent}</p>
                  <div className="mt-4 rounded-md border border-black/5 bg-white p-3 text-[12px] leading-5 text-black/45">Best for: {template.bestFor}</div>
                  <button disabled={!selectedDataset} onClick={() => handleCreateFromTemplate(template)} className="mt-4 h-9 w-full rounded-md bg-gray-900 text-[12px] font-medium text-white hover:bg-black disabled:opacity-40">Create chart card</button>
                </div>
              );
            })}
          </div>
