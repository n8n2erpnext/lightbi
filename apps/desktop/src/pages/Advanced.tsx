...
export const Advanced: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let active = true;
    const canUpdateProviderPlugins = () => (
      active
    ...
    void loadAdvancedProviderPlugins(controller.signal)
      .then(providers => {
        if (canUpdateProviderPlugins() && providers.length > 0) setProviderPlugins(providers);
      })
      .catch(error => {
        const isAbortError = error instanceof Error && error.name === 'AbortError';
        if (canUpdateProviderPlugins() && !isAbortError) {
  ...
  const { analyzeActiveResultInSimple, cancelFullExport, cancelSqlImport, copyResult, exportAllResult, exportResult, importSourceIntoTable, returnFullSourceToEasy, runAllStatements } = createAdvancedResultTransferActions({
