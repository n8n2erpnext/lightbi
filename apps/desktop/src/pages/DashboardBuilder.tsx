    ...
    <div data-testid="perspective-dashboard" className="flex h-full flex-1 flex-col bg-gray-50">
      <header className="flex min-h-16 shrink-0 items-center justify-between border-b border-gray-200 bg-white px-5 py-3">
        <div className="flex items-center space-x-3 text-gray-900">
          <button data-testid="dashboard-back" onClick={() => navigate(-1)} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50" title={t('Back to analysis')}><ArrowLeft className="h-4 w-4" />{t('Back')}</button>
          <div><h1 className="text-[15px] font-semibold">{dashboard ? localize(dashboard.name) : t('Select a dashboard')}</h1>{dashboard?.metadata?.perspective && <p className="mt-0.5 text-[11px] text-gray-500">{t('Perspective')}: {localize(String(dashboard.metadata.perspective))} · {String(dashboard.metadata.datasetId ?? '')}</p>}</div>
          <span className="rounded border border-gray-200 bg-gray-100 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-widest text-gray-500">{dashboard?.widgets.length ?? 0} {t('cards')}</span>
        </div>
        <div className="flex items-center space-x-2">
          <button className="rounded-md p-1.5 text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-800" title={t('Refresh')}><RefreshCw className="h-4 w-4" /></button>
          <button className="rounded-md p-1.5 text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-800" title={t('Edit')}><Edit2 className="h-4 w-4" /></button>
          <button className="rounded-md p-1.5 text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-800" title={t('Share')}><Share2 className="h-4 w-4" /></button>
          <div className="mx-2 h-5 w-px bg-gray-300" />
          <Link to="/charts" className="flex items-center rounded-md bg-gray-900 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-gray-800"><Plus className="mr-1 h-4 w-4" />{t('Add chart')}</Link>
        </div>
      </header>

      <div className="flex-1 overflow-y-auto p-4">
        {!dashboard ? (
          <div className="flex h-full items-center justify-center rounded-lg border border-dashed border-gray-200 bg-white text-sm text-gray-500">{t('Choose or create a dashboard from the chart library.')}</div>
          ...
          <div className="flex h-full flex-col items-center justify-center rounded-lg border border-dashed border-gray-200 bg-white text-center">
            <h2 className="text-base font-semibold text-gray-900">{t('No chart cards yet')}</h2>
            <p className="mt-1 text-sm text-gray-500">{t('Add reusable chart cards from the Chart Library.')}</p>
            <Link to="/charts" className="mt-4 rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white">{t('Open Chart Library')}</Link>
          </div>
        ) : (
          <div className="mx-auto max-w-[1500px]">
            ...
            {ba.deep && <section data-testid="dashboard-executive-brief" className="mb-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="grid min-w-72 grid-cols-2 gap-3">
                  <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-3"><div className="text-[10px] font-semibold uppercase tracking-wider text-emerald-700">{t('Decision readiness')}</div><div className="mt-1 text-2xl font-bold text-emerald-800">{ba.deep.decisionReadinessScore ?? '—'}<span className="text-xs font-medium text-emerald-600">/100</span></div></div>
                  <div className="rounded-xl border border-blue-100 bg-blue-50 p-3"><div className="text-[10px] font-semibold uppercase tracking-wider text-blue-700">{t('Data trust')}</div><div className="mt-1 text-2xl font-bold text-blue-800">{ba.deep.dataTrustScore ?? '—'}<span className="text-xs font-medium text-blue-600">/100</span></div></div>
                </div>
              </div>
            </section>}
            {(ba.perspective || ba.deep) && <section data-testid="dashboard-decision-context" className="mb-4 grid gap-3 lg:grid-cols-12">
              <div className="rounded-2xl border border-slate-200 bg-slate-950 p-5 text-white shadow-sm lg:col-span-5">
                <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-sky-300">{t('Selected decision perspective')}</div>
                ...
                <p className="mt-3 text-sm leading-6 text-slate-300">{t('Every KPI, chart, finding and action below is scoped to this perspective and its governed evidence.')}</p>
                <div className="mt-4 flex flex-wrap gap-2 text-xs"><span className="rounded-full border border-white/15 bg-white/10 px-3 py-1.5">{(ba.perspective?.sourceRowCount ?? 0).toLocaleString()} {t('source rows')}</span><span className="rounded-full border border-white/15 bg-white/10 px-3 py-1.5">{primaryInsights.length + perspectiveFindings.length} {t('BA findings')}</span><span className="rounded-full border border-white/15 bg-white/10 px-3 py-1.5">{actions.length} {t('recommended actions')}</span></div>
              </div>
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm lg:col-span-4">
                <div className="flex items-center gap-2"><Lightbulb className="h-4 w-4 text-amber-700" /><h2 className="text-sm font-semibold text-amber-950">{t('What the evidence says')}</h2></div>
                {perspectiveFindings.length > 0 ? <ol className="mt-3 space-y-2">{perspectiveFindings.slice(0, 4).map((finding, index) => <li key={index} className="flex gap-3 rounded-lg bg-white/75 p-3 text-xs leading-5 text-slate-700"><span className="font-bold text-amber-700">{index + 1}</span>{localize(finding)}</li>)}</ol> : <p className="mt-3 text-sm leading-6 text-amber-900">{localize(ba.deep?.executiveSummary) || t('Run the selected analysis to generate evidence-backed findings.')}</p>}
              </div>
              <div className="rounded-2xl border border-blue-200 bg-blue-50 p-5 shadow-sm lg:col-span-3">
                <div className="flex items-center gap-2"><Target className="h-4 w-4 text-blue-700" /><h2 className="text-sm font-semibold text-blue-950">{t('Decision priority')}</h2></div>
                <p className="mt-3 text-sm leading-6 text-blue-950">{actions[0] ? localize(actions[0]) : t('Review the strongest driver first, then open its supporting rows before taking action.')}</p>
                <div className="mt-4 rounded-xl border border-blue-200 bg-white p-3 text-xs leading-5 text-slate-600">{caveats.length > 0 ? t(`${caveats.length} evidence limitations remain visible below.`) : t('No additional evidence limitation was recorded for this dashboard.')}</div>
              </div>
            </section>}
            ...
            {(primaryInsights.length > 0 || actions.length > 0 || caveats.length > 0) && <section data-testid="dashboard-deep-ba" className="mt-4 grid gap-4 xl:grid-cols-12">
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm xl:col-span-7">
                <div className="flex items-center gap-2"><Lightbulb className="h-4 w-4 text-amber-600" /><h2 className="text-sm font-semibold text-slate-900">{t('BA findings for this perspective')}</h2></div>
