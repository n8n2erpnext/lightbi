...
const COMMERCE_CONCEPTS = [
  ...
  {
    conceptId: "descriptive_source_records",
    supportState: "conditional",
    requiredSemanticSignals: ["team", "person", "coach", "role", "segment", "previous_outcome", "contact", "campaign", "campaign_attempts", "previous_contacts"],
    requiredGrainStates: ["confirmed", "probable", "ambiguous", "unknown"],
    requiredRelationshipStates: [],
    requiredPhysicalEvidence: ["full_file_profile", "canonically_resolved_descriptive_anchor"],
    requiredReadinessCapabilities: ["physical_profile_ready", "semantic_labeling_ready", "descriptive_counting_ready"],
    blockers: [],
    limitations: ["physical_source_rows_only", "not_a_distinct_entity_count", "duplicates_remain_included", "no_causal_claims"],
    remediation: [],
    evidenceProvenance: ["canonical_physical", "canonical_semantic", "canonical_grain"],
    tuningProvenance: ["contract", "synthetic", "golden_tuning"],
  },
...
const GOVERNED_MANIFEST_BODY = {
  ...
  governedMetricIds: ["sales_revenue", "quantity_sold", "transaction_count", "inventory_on_hand", "delivery_count", "trip_count", "average_quality_score", "source_record_count", "gross_profit"],
