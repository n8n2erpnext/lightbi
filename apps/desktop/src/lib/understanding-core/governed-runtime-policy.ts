...
export const GOVERNED_RUNTIME_POLICY_V1 = {
  ...
  metricIds: ["sales_revenue", "quantity_sold", "transaction_count", "inventory_on_hand", "delivery_count", "trip_count", "average_quality_score", "source_record_count", "gross_profit"],
  operators: {
    ...
    trip_count: "governed_identity_count",
    average_quality_score: "governed_average",
    source_record_count: "governed_source_row_count",
    gross_profit: "governed_revenue_minus_cost",
  },
  conditionallyExecutableMetricIds: ["sales_revenue", "quantity_sold", "transaction_count", "inventory_on_hand", "delivery_count", "trip_count", "average_quality_score", "source_record_count"],
  rules: [
    ...
    { ruleId: "runtime.average_atomic_entity.v1", description: "Average scores require one canonically bound numeric score over an independently identified atomic entity grain." },
    { ruleId: "runtime.source_record_count.v1", description: "Source-row counting uses COUNT(*) and must retain the restriction that the result is not a distinct entity count." },
  ...
  forbiddenBehavior: [
    ...
    "source_rows_relabelled_as_entities",
