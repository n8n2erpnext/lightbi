...
describe("Phase 3A.2 acceptance truth governance", () => {
  ...
  it("keeps metrics, physical hashes, collision contracts, and production files frozen", () => {
    ...
    const betaOperationalCorrection = readJson<{
      productionFileHashes: Record<string, string>;
    }>("docs/architecture/beta-operational-semantic-support-audit.json");
    ...
    const motodetailSemanticCorrection = readJson<{
      productionFileHashes: Record<string, string>;
    }>("docs/architecture/motodetail-semantic-support-audit.json");
