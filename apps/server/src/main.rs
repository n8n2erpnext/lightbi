...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/api/advanced/connections", post(advanced::create_connection))
        .route("/api/advanced/connections/:connection_id", axum::routing::delete(advanced::delete_connection))
        .route("/api/advanced/connections/:connection_id/schema", get(advanced::get_schema))
        .route("/api/advanced/connections/:connection_id/schema/count", get(advanced::get_table_count))
        .route("/api/advanced/connections/:connection_id/query", post(advanced::execute_query))
        .route("/api/advanced/connections/:connection_id/document-query", post(advanced::execute_document_query))
        .route("/api/advanced/connections/:connection_id/explain", post(advanced::explain_query))
        .route("/api/advanced/connections/:connection_id/mutations/preview", post(advanced::preview_mutation))
        .route("/api/advanced/connections/:connection_id/mutations/commit", post(advanced::commit_mutation))
        .route("/api/advanced/connections/:connection_id/scripts/preview", post(advanced::preview_script))
        .route("/api/advanced/connections/:connection_id/scripts/commit", post(advanced::commit_script))
        .route("/api/advanced/connections/:connection_id/exports", post(advanced::start_export))
        .route("/api/advanced/exports/:job_id", get(advanced::get_export_job).delete(advanced::cancel_export_job))
        .route("/api/advanced/exports/:job_id/download", get(advanced::download_export_job))
        .route("/api/advanced/connections/:connection_id/imports/sql", post(advanced::start_sql_import))
        .route("/api/advanced/connections/:connection_id/imports/csv", post(advanced::start_csv_import))
        .route("/api/advanced/imports/:job_id", get(advanced::get_import_job).delete(advanced::cancel_import_job))
        .route("/api/advanced/runs/:run_id", axum::routing::delete(advanced::cancel_run))
        .route("/api/advanced/history", get(advanced_workspace::list_history).post(advanced_workspace::save_history).delete(advanced_workspace::clear_history))
        .route("/api/advanced/favorites", get(advanced_workspace::list_favorites).post(advanced_workspace::save_favorite))
        .route("/api/advanced/favorites/:favorite_id", axum::routing::delete(advanced_workspace::delete_favorite))
        .route("/api/advanced/profiles", get(advanced_workspace::list_profiles).post(advanced_workspace::save_profile))
        .route("/api/advanced/profiles/:profile_id", axum::routing::delete(advanced_workspace::delete_profile))
        .layer(cors)
        .with_state(state);
...
async fn fetch_microsoft_excel_bytes(url: &str) -> Result<Vec<u8>, OnlineExcelFetchError> {
    ...
    let bytes = response.bytes().await.map_err(|error| OnlineExcelFetchError {
        status: StatusCode::BAD_GATEWAY,
    ...
    let download = client
        .get(download_url)
        .header(header::ACCEPT, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,*/*")
        .send()
        .await
        .map_err(|error| OnlineExcelFetchError {
            status: StatusCode::BAD_GATEWAY,
    ...
    if !download_status.is_success() {
        return Err(OnlineExcelFetchError {
            status: if download_status == StatusCode::FORBIDDEN || download_status == StatusCode::UNAUTHORIZED {
                StatusCode::FORBIDDEN
            } else {
                StatusCode::BAD_GATEWAY
            },
    ...
    let workbook = download.bytes().await.map_err(|error| OnlineExcelFetchError {
        status: StatusCode::BAD_GATEWAY,
