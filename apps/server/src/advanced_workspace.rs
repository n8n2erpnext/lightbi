...
async fn vault_key() -> Result<[u8; 32], ApiError> {
    ...
    if let Ok(bytes) = tokio::fs::read(path).await {
        return bytes.try_into().map_err(|_| ApiError::storage("Credential vault key has an invalid length."));
    }
    let mut key = [0u8; 32];
    SystemRandom::new().fill(&mut key).map_err(|_| ApiError::storage("Could not generate credential vault key."))?;
    let mut options = tokio::fs::OpenOptions::new();
    options.write(true).create_new(true);
    #[cfg(unix)] { options.mode(0o600); }
    match options.open(path).await {
        Ok(mut file) => file.write_all(&key).await.map_err(|error| ApiError::storage(format!("Could not write vault key: {error}")))?,
        Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
            let bytes = tokio::fs::read(path).await.map_err(|error| ApiError::storage(format!("Could not read vault key: {error}")))?;
            return bytes.try_into().map_err(|_| ApiError::storage("Credential vault key has an invalid length."));
        }
        Err(error) => return Err(ApiError::storage(format!("Could not create vault key: {error}"))),
    }
    Ok(key)
}

async fn encrypt_secret(secret: &str) -> Result<(String, String), ApiError> {
    let key = aead::UnboundKey::new(&aead::AES_256_GCM, &vault_key().await?).map_err(|_| ApiError::storage("Could not initialize credential vault."))?;
    let mut nonce_bytes = [0u8; 12];
    SystemRandom::new().fill(&mut nonce_bytes).map_err(|_| ApiError::storage("Could not generate vault nonce."))?;
    let nonce = aead::Nonce::assume_unique_for_key(nonce_bytes);
    let mut value = secret.as_bytes().to_vec();
    aead::LessSafeKey::new(key).seal_in_place_append_tag(nonce, aead::Aad::empty(), &mut value).map_err(|_| ApiError::storage("Could not encrypt credential."))?;
    Ok((BASE64.encode(value), BASE64.encode(nonce_bytes)))
}

pub(crate) async fn resolve_profile_secret(pool: &SqlitePool, profile_id: &str) -> Result<String, ApiError> {
    let row = sqlx::query("SELECT credential_cipher, credential_nonce FROM advanced_connection_profiles WHERE id = ?")
        .bind(profile_id).fetch_optional(pool).await.map_err(|error| ApiError::storage(format!("Could not read connection profile: {error}")))?
        .ok_or_else(|| ApiError::bad_request("ADVANCED_PROFILE_NOT_FOUND", "Connection profile was not found."))?;
    let mut cipher = BASE64.decode(row.get::<String, _>("credential_cipher")).map_err(|_| ApiError::storage("Stored credential is invalid."))?;
    let nonce: [u8; 12] = BASE64.decode(row.get::<String, _>("credential_nonce")).map_err(|_| ApiError::storage("Stored credential nonce is invalid."))?.try_into().map_err(|_| ApiError::storage("Stored credential nonce has an invalid length."))?;
    let key = aead::UnboundKey::new(&aead::AES_256_GCM, &vault_key().await?).map_err(|_| ApiError::storage("Could not initialize credential vault."))?;
    let plain = aead::LessSafeKey::new(key).open_in_place(aead::Nonce::assume_unique_for_key(nonce), aead::Aad::empty(), &mut cipher).map_err(|_| ApiError::storage("Could not decrypt stored credential."))?;
    String::from_utf8(plain.to_vec()).map_err(|_| ApiError::storage("Stored credential is not valid UTF-8."))
}
...
pub(crate) async fn save_history(
    ...
    if sql.is_empty() || sql.len() > 100_000 {
        return Err(ApiError::bad_request("ADVANCED_HISTORY_INVALID", "History SQL must contain 1 to 100,000 characters."));
    }
...
pub(crate) async fn clear_history(State(state): State<Arc<AppState>>) -> Result<StatusCode, ApiError> {
    sqlx::query("DELETE FROM advanced_query_history").execute(&state.context.sqlite_pool).await
...
fn history_from_row(row: sqlx::sqlite::SqliteRow) -> HistoryEntry {
    HistoryEntry {
        id: row.get("id"), connection_name: row.get("connection_name"), database: row.get("database_name"),
        provider: row.get("provider"), sql: row.get("sql_text"), status: row.get("status"),
        row_count: row.get("row_count"), execution_ms: row.get("execution_ms"), created_at: row.get("created_at"),
    }
}
...
pub(crate) async fn list_favorites(State(state): State<Arc<AppState>>) -> Result<Json<Vec<FavoriteEntry>>, ApiError> {
...
pub(crate) async fn save_favorite(
    ...
    if name.is_empty() || name.len() > 120 || sql.is_empty() || sql.len() > 100_000 {
        return Err(ApiError::bad_request("ADVANCED_FAVORITE_INVALID", "Favorite requires a name and bounded SQL text."));
    }
    let now = Utc::now().to_rfc3339();
    let entry = FavoriteEntry { id: Uuid::new_v4().to_string(), name: name.to_string(), sql: sql.to_string(), provider: request.provider, database: request.database, created_at: now.clone(), updated_at: now };
...
pub(crate) async fn delete_favorite(
    ...
    sqlx::query("DELETE FROM advanced_query_favorites WHERE id = ?").bind(favorite_id)
        .execute(&state.context.sqlite_pool).await
...
fn favorite_from_row(row: sqlx::sqlite::SqliteRow) -> FavoriteEntry {
    FavoriteEntry {
        id: row.get("id"), name: row.get("name"), sql: row.get("sql_text"), provider: row.get("provider"),
        database: row.get("database_name"), created_at: row.get("created_at"), updated_at: row.get("updated_at"),
    }
}
...
pub(crate) async fn save_profile(
    State(state): State<Arc<AppState>>, Json(request): Json<SaveProfileRequest>,
    ...
    if name.is_empty() || name.len() > 120 || url.is_empty() || !providers.contains(&request.provider.as_str()) {
        return Err(ApiError::bad_request("ADVANCED_PROFILE_INVALID", "Profile requires a name, supported provider, and connection URL."));
    }
    let tls_mode = request.tls_mode.unwrap_or_else(|| "driver-default".to_string());
...
pub(crate) fn apply_tls_policy(url: &str, provider: &str, mode: &str) -> String {
    ...
    match provider {
        "postgresql" => format!("{url}{separator}sslmode={}", if mode == "verify-full" { "verify-full" } else { "require" }),
        "mysql" | "mariadb" => format!("{url}{separator}ssl-mode={}", if mode == "verify-full" { "VERIFY_IDENTITY" } else { "REQUIRED" }),
        "mongodb" => format!("{url}{separator}tls=true"),
        _ => url.to_string(),
    }
}

pub(crate) async fn delete_profile(State(state): State<Arc<AppState>>, Path(profile_id): Path<String>) -> Result<StatusCode, ApiError> {
    sqlx::query("DELETE FROM advanced_connection_profiles WHERE id = ?").bind(profile_id).execute(&state.context.sqlite_pool).await
