...
async function admin() {
  ...
  const showAdminLoadError = (section, response) => {
    document.body.innerHTML = `<main class="admin-shell"><div class="admin-head"><div><span class="eyebrow">LIGHTBI DISTRIBUTION</span><h1>${escapeHtml(section)} unavailable</h1><p>The admin session is still active, but this section could not load (HTTP ${escapeHtml(response.status)}). No data was changed.</p></div><a class="button" href="${portalBase}/admin">Back to analytics</a></div><section class="chart-card"><h2>Try again safely</h2><p>Reload this page. If the problem continues, inspect the distribution service before changing users or keys.</p><button class="button dark" onclick="location.reload()">Reload section</button></section></main>`;
  };
  if (tab === 'accounts') {
    const response = await fetch(`${routeBase}/api/admin/accounts`); if(!response.ok){showAdminLoadError('Accounts',response);return true;}
    const data=await response.json();const safe=escapeHtml;
    const rows=(data.accounts||[]).map(item=>`<tr><td><strong>${safe(item.display_name||'—')}</strong><small class="cell-note">${safe(String(item.created_at||'').slice(0,10))}</small></td><td>${safe(item.email)}</td><td>${safe(item.provider)}</td><td><span class="status-pill ${safe(item.status||'active')}">${safe(item.status||'active')}</span></td><td>${safe(item.tier)} · ${safe(item.entitlement_status)}</td><td><code class="masked-key">${safe(item.license?.masked_key||'No Pro key')}</code><small class="cell-note">${safe(item.license?.label||'')}</small></td><td>${safe(item.active_devices||0)}/${safe(item.max_devices||1)}</td><td>${safe(item.expires_at?String(item.expires_at).slice(0,10):'—')}</td><td><div class="table-actions">${item.tier==='pro'&&item.entitlement_status==='active'?`<button class="table-action danger" data-account-entitlement-revoke="${safe(item.id)}">Revoke Pro</button>`:''}<button class="table-action" data-account-sessions-revoke="${safe(item.id)}">Sign out all</button><button class="table-action ${item.status==='disabled'?'':'danger'}" data-account-status="${safe(item.id)}" data-next-status="${item.status==='disabled'?'active':'disabled'}">${item.status==='disabled'?'Enable':'Disable'}</button></div></td></tr>`).join('');
    document.body.innerHTML=`<main class="admin-shell"><div class="admin-head"><div><span class="eyebrow">LIGHTBI DISTRIBUTION</span><h1>Users, access & devices</h1><p>Manage Google/email accounts, sessions, Pro entitlement and privacy-safe device slots.</p><nav class="admin-tabs"><a href="${portalBase}/admin">Analytics</a><a href="${portalBase}/admin?tab=app">App usage</a><a class="active" href="${portalBase}/admin?tab=accounts">Accounts</a><a href="${portalBase}/admin?tab=licenses">Licenses</a><a href="${portalBase}/admin?tab=revenue">Pro revenue</a></nav></div><button class="button" data-admin-logout>Sign out</button></div><section class="chart-card"><h2>Registered users</h2><p>Disabling an account revokes every active session. Keys are permanently stored only as hashes and displayed in masked form.</p><div class="table-scroll"><table><thead><tr><th>User</th><th>Email</th><th>Identity</th><th>Account</th><th>Entitlement</th><th>Pro key</th><th>Devices</th><th>Expires</th><th>Actions</th></tr></thead><tbody>${rows||'<tr><td colspan="9">No LightBI accounts yet.</td></tr>'}</tbody></table></div></section></main>`;
    ...
    document.querySelectorAll('[data-account-sessions-revoke]').forEach(button=>button.addEventListener('click',async()=>{if(!confirm('Sign this user out on every device?'))return;const result=await fetch(`${routeBase}/api/admin/accounts/${encodeURIComponent(button.dataset.accountSessionsRevoke)}/sessions/revoke`,{method:'POST',headers:{'x-lightbi-admin-action':'1'}});if(result.ok)location.reload();}));
    document.querySelectorAll('[data-account-status]').forEach(button=>button.addEventListener('click',async()=>{const next=button.dataset.nextStatus;if(!confirm(`${next==='disabled'?'Disable':'Enable'} this account?${next==='disabled'?' All sessions will be revoked.':''}`))return;const result=await fetch(`${routeBase}/api/admin/accounts/${encodeURIComponent(button.dataset.accountStatus)}/status`,{method:'POST',headers:{'content-type':'application/json','x-lightbi-admin-action':'1'},body:JSON.stringify({status:next})});if(result.ok)location.reload();}));
  ...
  if (tab === 'licenses') {
    ...
    if (!response.ok) { showAdminLoadError('Licenses',response); return true; }
    const data = await response.json();
