...
const allowedKinds = new Set(['page_view', 'visit_end', 'download', 'install_pair', 'license_activation']);
...
export async function createDistributionAnalytics({ databaseUrl, redisUrl, pepper }) {
  ...
  async function record(input) {
    ...
    const values = [input.kind, visitorHash, installationHash, tier, clean(input.appVersion, 40), clean(input.platform, 60), clean(input.path, 200), clean(input.referrerHost, 120), clean(input.utmSource, 80), clean(input.utmMedium, 80), clean(input.utmCampaign, 120), clean(input.timezone, 80), visitHash, duration, clean(input.browser, 40), clean(input.os, 40), clean(input.device, 30), clean(input.language, 20), clean(input.networkHash, 80)];
    await pool.query(`INSERT INTO distribution_events
      (kind, visitor_hash, installation_hash, tier, app_version, platform, path, referrer_host, utm_source, utm_medium, utm_campaign, timezone, visit_hash, duration_seconds, browser, os, device, language, network_hash)
