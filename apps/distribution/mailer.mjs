export async function createMailer(config) {
  ...
  async function sendProLicense({ to, licenseKey, template = 'automatic', label, discountPercent, expiresAt }) {
    const manual = template === 'manual';
    const title = manual ? 'Your LightBI Pro partner license' : 'Your LightBI Pro purchase is ready';
    const detail = manual ? `This license was issued by LightBI Support${label ? ` for ${escapeHtml(label)}` : ''}${discountPercent ? ` with a ${Number(discountPercent)}% distribution benefit` : ''}.` : 'Your payment was confirmed and your LightBI Pro license is ready.';
    const expiry = expiresAt ? `<p style="margin:8px 0;color:#64748b">Valid until: <strong>${String(expiresAt).slice(0,10)}</strong></p>` : '';
    await transport.sendMail({
      from: `LightBI Support <${config.from}>`, to,
      subject: title,
      text: `${title}\n\n${detail}\n\nLicense key: ${licenseKey}\n${expiresAt ? `Valid until: ${String(expiresAt).slice(0,10)}\n` : ''}\nActivate it in LightBI Settings. Keep this key private.`,
      html: `<div style="margin:0;background:#f6f7fb;padding:32px 16px;font-family:Arial,sans-serif;color:#0f172a"><div style="max-width:620px;margin:auto;background:white;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden"><div style="background:#080d1f;padding:24px 30px;color:white"><strong style="font-size:22px">Light<span style="color:#ffc400">BI</span></strong><div style="margin-top:8px;color:#a8b2c7;font-size:12px">Evidence-governed business analysis</div></div><div style="padding:30px"><h1 style="font-size:24px;margin:0 0 14px">${title}</h1><p style="line-height:1.6;color:#475569">${detail}</p><div style="margin:24px 0;padding:18px;background:#f8fafc;border:1px solid #dbe3ef;border-radius:10px"><div style="font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:.08em">License key</div><code style="display:block;margin-top:8px;font-size:16px;overflow-wrap:anywhere;color:#1d4ed8">${licenseKey}</code></div>${expiry}<p style="line-height:1.6;color:#475569">Open <strong>LightBI → Settings → License</strong>, paste the key, then activate it. Keep the key private.</p></div><div style="padding:18px 30px;background:#f8fafc;color:#64748b;font-size:12px">Need help? Reply to this email or contact support@thaiduy.digital.</div></div></div>`,
    });
  }
  return { enabled: true, sendPasswordReset, sendProLicense, verify: () => transport.verify() };
}
