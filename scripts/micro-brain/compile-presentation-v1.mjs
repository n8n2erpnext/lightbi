...
const KNOWLEDGE_DIR = path.join(BRAIN_DIR, "presentation-knowledge");
const OUTPUT_PATH = path.join(BRAIN_DIR, "compiled/presentation.index.v1.json");
const SOURCE_MANIFEST_PATH = path.join(KNOWLEDGE_DIR, "manifest.v1.json");

const COMPILER_VERSION = "lightbi.micro-brain.presentation-compiler.v1";
const TOKENIZER_VERSION = "lightbi.micro-brain.tokenizer.v1";
const INDEX_VERSION = "lightbi.micro-brain.presentation-index.v1";
const MAX_FEATURES = 2304;
...
function typedTags(card) {
  return [
    ...
    ...(card.knowledgeLane ? [`lane:${normalize(card.knowledgeLane)}`] : []),
    ...(card.presentation?.advisoryKind ? [`presentation:${normalize(card.presentation.advisoryKind)}`] : []),
    ...(card.presentation?.analyticalIntents ?? []).map((intent) => `intent:${normalize(intent).replace(/\s+/g, "_")}`),
    ...(card.presentation?.chartFamilies ?? []).map((chart) => `chart:${normalize(chart).replace(/\s+/g, "_")}`),
    ...(card.presentation?.perspectives ?? []).map((view) => `view:${normalize(view).replace(/\s+/g, "_")}`),
    ...(card.presentation?.dashboardRoles ?? []).map((role) => `role:${normalize(role).replace(/\s+/g, "_")}`),
  ].sort();
}
...
// Presentation knowledge uses an isolated dense space so chart/domain guidance cannot rotate or dilute the semantic authority index.
...
const compiled = {
  ...
  cards: cards.map((card) => ({
    ...
    knowledgeLane: card.knowledgeLane ?? "semantic",
    presentation: card.presentation ?? undefined,
  })),
  units,
  featureVocabulary,
  idf: idf.map(round),
