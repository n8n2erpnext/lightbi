...
function canonicalSemVer(value: string): boolean {
  const parsed = parseSemVer(value);
  if (!parsed) return false;
  const canonical = `${parsed.version}${parsed.build.length ? `+${parsed.build.join('.')}` : ''}`;
  return value === canonical;
}
export const semverSchema = z.string().min(1).max(80).refine(canonicalSemVer, 'SemVer must use one canonical textual representation');
...
export const issuerKeysetPayloadV1Schema = strict({ schema: z.literal(1), kid: id, keyset_version: positiveInteger, issued_at: canonicalTimestampSchema, expires_at: canonicalTimestampSchema, keys: z.array(issuerKeyRecordV1Schema).min(1).max(64) }).superRefine((value, context) => {
...
export const entitlementTrustStateV1Schema = strict({ subject: entitlementSubjectSchema, entitlementVersion: positiveInteger, payloadDigest: sha, issuedAt: canonicalTimestampSchema });
...
export const organizationClaimTokenRecordV1Schema = strict({ schema: z.literal(1), token_id: id, organization_id: id, entitlement_id: id, token_hash: sha, issued_at: canonicalTimestampSchema, expires_at: canonicalTimestampSchema, consumed_at: canonicalTimestampSchema.nullable() }).superRefine((value, context) => {
  ...
  if (value.consumed_at !== null) { const consumed = Date.parse(value.consumed_at); if (consumed < issued || consumed >= expires) context.addIssue({ code: 'custom', path: ['consumed_at'], message: 'claim consumption must occur inside the half-open validity window' }); }
});
