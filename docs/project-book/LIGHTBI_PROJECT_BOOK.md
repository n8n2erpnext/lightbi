...
Current implementation status on 2026-09-04: **MB-6 is complete** on successor branch `codex/r1-roadmap-integration` at `94fa40c` (`feat(understanding): propagate analysis authority through BA`). MB-6 adds `lightbi.ba-analysis-authority-context.v1` and carries domain inference, official support, analysis mode, governed metric preflight/runtime authorization, limitations and evidence references through Focus, Deep BA and BA Step 2. Formula authority remains explicitly `not_independently_authorized`, and Micro Brain similarity/rank remains provenance rather than semantic confidence. Factual parity tests keep Focus aggregation/value/population facts and Deep BA KPI/breakdown/finding outputs unchanged with or without the new authority context. Production services and production domain/metric authority remain untouched. **MB-7** is now the primary next phase: corpus/counterfactual/performance/deterministic-index/no-network/release-authoritative acceptance and cutover review.
