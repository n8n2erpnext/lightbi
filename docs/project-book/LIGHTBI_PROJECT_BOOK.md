...
Current implementation status on 2026-09-04: **MB-7 source acceptance is complete** on successor branch `codex/r1-roadmap-integration` at `a1f6ee8` (`test(understanding): close micro brain v1 acceptance`). MB-5 remains `8a4a5e4`; MB-6 remains `94fa40c`. MB-7 validates deterministic index rebuild, negative/counterfactual safety, current independent oracle/runtime truth, local-only/no-network behavior, active-core performance, production build/release gates and Playwright authority preservation. On the ARM Neoverse-N1 reference VPS, selective MB adds ~650 ms / 6.80% p50 over the same semantic pass while changing only three TTKT ETA fields from `unknown` to evidence-bound `probable`; confirmed regressions are zero. Index footprint is ~6.61 MB raw / 1.87 MB gzip9. Production deployment and production domain/metric/formula/decision authority remain untouched. Micro Brain therefore leaves the critical path; current execution returns to integrated product UX parity, updater/security integration and packaged Road-to-1.0 acceptance.
...
## 105. 2026-09-04 successor Road-to-1.0 integration checkpoint

The owner directed the previously isolated Focus/Micro-Brain work back into one Road-to-1.0 execution stream. The product successor remains `codex/r1-roadmap-integration` and has advanced to `94fa40c` (`feat(understanding): propagate analysis authority through BA`). MB-5 authority remains `8a4a5e4`; MB-6 authority is now `94fa40c`. Neither successor integration nor MB-6 reclassifies inferred semantics or official domain support.

**MB-6 is complete; the current critical path is MB-7 → integrated product/security/release acceptance.** MB-6 carries `domainInference`, `officialSupport`, `analysisMode`, governed metric preflight/runtime authorization, explicit absence of independent formula authority, limitations and evidence references through Focus, Deep BA and BA Step 2 while preserving identical governed factual values. Machine evidence is stored at `apps/desktop/src/lib/understanding-core/micro-brain/baseline/mb6-analysis-authority-propagation.v1.json`. Final MB-6 gates: 15 Vitest files / 79 tests PASS, TypeScript PASS, production Vite build PASS (3,790 modules), and Playwright healthcare Deep BA + Step 2 acceptance 1/1 PASS. MB-7 now owns corpus/counterfactual/performance/deterministic-index/no-network/full release-authoritative acceptance and cutover review.

In parallel, a deliberately bounded Signed Transport foundation remains checkpointed without turning networking/security work into the product critical path. The Signed Transport primitive itself remains product commit `a8d55ee`, which adds a Rust canonicalization/body-digest/Ed25519 request-proof primitive compatible with the existing `lightbi.next-attestation-request.v1` rehearsal contract. It is not exposed as an arbitrary frontend signing command and is not yet wired into `native_http_request`. An isolated Rust harness passed 3/3 golden-vector tests against the authoritative JavaScript canonicalization behavior.

Private control-plane branch `codex/r1p14-signed-transport` is at `c5875eb` (`feat(trust): add signed transport verification edge`). The Distribution runtime may now act as a thin **Internal-only** UDS client to the verification-only attestation appliance for nonce/verification operations. The verifier returns `lastAcceptedSequence` with a nonce so a restarted client can resynchronize its monotonic anti-replay floor. Distribution still owns no verifier implementation, signer credential, private key, canonical signing authority or generic signing endpoint. Attestation tests pass 15/15 and the complete Distribution suite passes 220/220 after reconciling the old over-broad boundary guard to allow only this thin UDS verification edge.

This security checkpoint is foundation only. Signed-by-default application routes remain **not enabled**. Before GET/read surfaces such as announcements/releases can be protected, canonical query binding and response-integrity semantics must be frozen; mutation routes must preserve non-replay behavior. Installation trust bootstrap (`pair`, challenge/issue, nonce acquisition) remains a distinct bootstrap class.

The phrase “accepted foundation” in the current successor workflow does **not** rewrite the historical Production Trust freeze gate or canonical R1-P0…R1-P13 numbering. In particular `phase2aFrozen=false`, `rootCeremonyAuthorized=false`, and Production R1-P6/R1-P7 authority remain owner-gated exactly as recorded in Chapters 101–104. The NEXT/Internal test authority and Signed Transport rehearsal remain non-production and non-promotable.

Future Team/Workspace networking is tracked separately as **Private Authenticated Transport**, a technology-neutral capability that may later use WireGuard, QUIC overlay, MASQUE, mTLS private gateway or another suitable mechanism. It is not a LightBI 1.0 blocker and VPN/overlay membership never substitutes for application identity, entitlement, installation trust or signing authority.

Operationally, the ARM VPS SSD rule remains active. A full Tauri/Rust test attempt began compiling `libduckdb-sys`, grew the product worktree `target/` cache to about 5.6 GiB and moved `/` from 86% to 89%. The run was stopped before the 90% threshold; only the rebuildable `target/` cache was removed, returning the filesystem to 84% with about 20 GiB free. Heavy future gates must continue checking SSD before and after execution and avoid rebuilding unrelated native dependency graphs when a bounded harness can prove the scoped contract.

## 106. 2026-09-04 Micro Brain V1 acceptance closure and return to main Road-to-1.0 map

Product successor `codex/r1-roadmap-integration` is accepted through Micro Brain MB-7 at `a1f6ee8`. Durable machine evidence is `apps/desktop/src/lib/understanding-core/micro-brain/baseline/mb7-acceptance.v1.json`; active benchmark evidence is `mb7-active-core-benchmark.v1.json`. The current independent Phase 7R3.4 oracle was regenerated from the isolated oracle tool because the historical artifact carried stale corpus identity metadata; metric, identity, relationship and inventory truths remained equal, and current Phase 7R3.5/8B acceptance passes.

MB-7 exact evidence: deterministic index byte SHA `6415fdde...d8ec0`; MB-specific 14 files / 41 tests PASS; runtime/oracle 3 files / 6 tests PASS; complete `test:release-1.0` PASS with production build and governed product regression 11 files / 40 tests; Playwright MB-5 + MB-6 2/2 PASS. Active-core benchmark on ARM Neoverse-N1 covers 30 samples / 19 sources / 379 columns: MB OFF p50 ~9.558 s, selective MB ~10.208 s (+650 ms / +6.80%); 243 retrievals/run at p50 ~2.25 ms / p95 ~2.94 ms / p99 ~3.35 ms. Three ETA recoveries are the only resolution changes and no confirmed mapping regresses. Index raw/gzip9 footprint is ~6.61/1.87 MB.

The acceptance decision is **source acceptance + cutover review passed; Production not deployed**. Production runtime, official domain activation, metric/formula/decision authority and Phase-2A Trust freeze remain unchanged. `phase2aFrozen=false` remains authoritative.

Execution now returns to one Road-to-1.0 map. First: multi-file Analysis Context + Focus Subject UX parity without rewriting governed multi-source relationship/grain safety. Second: Intelligence Pack Updater V1 for signed data-only MB/dictionary/domain knowledge packs with compatibility checks and atomic rollback. Third: finish Signed Transport query/response/route/native wiring and negative probes. Then run packaged Windows/UAT and integrated release acceptance. Future Team/Workspace Private Authenticated Transport remains technology-neutral and outside the 1.0 blocker set.

## 107. 2026-09-04 Micro Brain strategic rationale and Cross-Domain Semantic Expansion direction

The owner-approved Micro Brain conversation rationale is now distilled into canonical architecture at [`../architecture/micro-brain-cross-domain-semantic-expansion.md`](../architecture/micro-brain-cross-domain-semantic-expansion.md). The durable reason for MB is broader than alias recovery: a finite dictionary cannot enumerate opaque internal codes, mixed-language terminology, pathological schemas, or industry-specific vocabulary, so LightBI needs a local semantic interpolation layer that can propose hypotheses while the Evidence Resolver, Domain Support and Metric Runtime remain separate authorities.

The semantic hierarchy is now explicitly recorded as `Registry/Dictionary -> Micro Brain -> Evidence Resolver -> Domain Support -> Metric Runtime`. `retrieval_similarity != semantic_confidence` remains invariant. Real user datasets must not directly self-train production semantic truth; learning is sanitized evidence / knowledge-gap discovery -> offline curation -> validated corpus batch -> deterministic rebuild -> held-out/counterfactual/grain/formula/performance/release acceptance. Negative knowledge and abstention remain first-class.

The next major semantic initiative after the current Road-to-1.0 integration work is **LightBI Cross-Domain Semantic Expansion**. Priority breadth is hospitality (hotel/resort), healthcare/pharma, agriculture/livestock/aquaculture, manufacturing, banking/financial-services specialization, and chemistry/physics/biology primitives. The objective is semantic breadth and reusable ontology, not a claim to officially support every listed industry. Cross-domain families such as customer/patient/guest/student/tenant/subscriber share higher-level relations while remaining non-interchangeable; capacity availability, physical stock, account balance and service entitlement must likewise remain distinct analytical structures.

The original approximately 10 MB knowledge target is retained as an engineering guardrail rather than a permanent ceiling. Future growth should be modular through domain shards/packs, lazy loading, sparse-first routing, hot/cold tiers, compact storage or other benchmarked mechanisms so runtime cost need not grow linearly with total knowledge. Stable concepts may be promoted to the canonical registry; understood but not normalized concepts may remain MB open concepts; official domain packs continue to require separate product-support and runtime acceptance.

This direction does **not** preempt the Road-to-1.0 sequence. §108 now records Multi-file UX parity / Focus Subject complete and active on NEXT031; the current executable next action is Intelligence Pack/update trust, followed by Signed Transport integration and packaged Windows/UAT + integrated release acceptance. MB V1 acceptance remains anchored at `a1f6ee8` while the product successor has advanced to `8abc669`.
## 108. 2026-09-04 Multi-file Focus Subject closure and NEXT031 runtime acceptance

The Road-to-1.0 multi-file UX-parity item is complete on product successor `codex/r1-roadmap-integration` at `8abc669` (`feat(multifile): add governed focus subject analysis`). The implementation reuses the single-file Focus interaction model but preserves multi-source governance: Focus candidate concepts come from each source's canonical understanding; cross-source identity requires the same canonical concept plus an exact selected value; same labels with different IDs are not silently equated. Focus remains a sidecar Analysis Context and does not rewrite source relationships, JOIN raw rows, or recalculate governed summary metrics.

In the result path, full-population governed summary cards remain unchanged with or without Focus. Exact Focus matches scope source evidence, focused comparison briefing, Deep BA and Step 2. A source with the concept but without the selected value, or without a governed concept binding, remains explicitly unavailable and is never populated from another source. Exported analysis keeps the governed summary while Focus evidence sheets contain only exact matched rows. The old narrative label `BA FOCUS` is renamed `Key attention` / `Điểm cần chú ý` so Focus Subject has one product meaning.

Source acceptance is green: targeted Focus contract/UI/result tests 3 files / 8 tests PASS; the full release-authoritative suite passes with governed product regression 11 files / 41 tests; desktop build transforms 3,792 modules; source-size gate passes 509 modules with `HomeWorkspaceView.tsx` at 999 lines; six-file ERP Playwright uses `Aqua 250L`, proves summary-card equality before/after Focus, proves six exact source bindings, exact row drill-through and Step 2, and passes on the final source snapshot.

The accepted source was packaged as immutable NEXT `g-2026-09-04-next-031`, parent `g-2026-09-03-next-030-focus-exp-03`. The generation manifest pins source commit `8abc66971d5cae6cb232a4d6b48bd107fd77a52a`, reused server-binary Core provenance `ed4b9233f690b9ddeb872ee858e0e9a8c46644e0`, CP `bb50b0d53542da5cd908e2237cbca368f7f87073`, schema `067_catalog_quarterly_pricing`, app `0.9.2-beta.7-next.31`, and manifest SHA-256 `08fe08ea9dcc72d575721f1db891cd46ce46d030556c53d14c472dc4149c9797`. The reused server binary is byte-identical to its predecessor at SHA-256 `95be788197a853cff8c6d1665ef4d62e62ac71fd9dfa9546253efcda0b78aa7e`.

Runtime reconciliation is complete on NEXT ports 5272/5273/5274. Core/Gateway/Control Plane/worker all carry `g-2026-09-04-next-031`; CP schema is current and worker health is `healthy`. Public NEXT generation and distribution diagnostics report the same generation/CP identities. The six-file Focus Playwright journey was rerun directly against `5273/app` and passed 1/1 in 46.9 seconds. Generation checksums pass after runtime activation. No owner role rotation was performed.

This chapter supersedes only the executable-next-action wording in §106–§107. The Road-to-1.0 primary next item is now **Intelligence Pack Updater V1**, followed by Signed Transport integration, then packaged Windows/UAT and integrated release acceptance. Future Cross-Domain Semantic Expansion remains recorded but is not inserted ahead of those 1.0 items.


## 109. 2026-09-04 Intelligence Pack V1 source closure; NEXT032 runtime gate opens

The Road-to-1.0 updater item is source-closed without changing the active runtime role. Public product successor `codex/r1-roadmap-integration` is clean/pushed at `262bd768fc48b7eb4e8948631da853da318f610c` (`feat(intelligence): add signed data-only pack updater`); private CP successor `codex/r1p14-signed-transport` is clean/pushed at `72eacf75209b6984955bec362ab7752176184478` (`feat(distribution): add internal intelligence pack edge`). These commits were created by the executing ChatGPT/Remote Desktop Commander session on the VPS; they are not unexplained external provenance.

V1 keeps executable and knowledge update lanes separate. The product accepts only strict `lightbi.intelligence-pack.v1` data-only envelopes signed by a pinned trusted key, verifies payload digest/size and Core/Brain/registry/feature compatibility, performs Micro Brain structural plus semantic-registry smoke validation, and stores immutable objects behind crash-safe `staged`, `active`, `previous` pointers. Activation advances a monotonic accepted-version floor; rollback may restore `previous` but does not lower that floor. Corrupt state fails closed to bundled intelligence with repair, and invalid active state may recover only through verified previous/bundled evidence. The existing Micro Brain retrieval/resolver path remains the sole semantic consumer; signed bytes cannot mint canonical/domain/metric/formula/decision authority.

The private Distribution edge is read-only and Internal-only: `/api/intelligence-packs/latest` plus safe-basename artifact GET/HEAD. It owns no INT private key or signer. NEXT test authority `int-next-2026-01` is separate, non-promotable and not a stable/public trust decision. Exact-head verification passes Intelligence Pack Rust `6/6`, frontend `2/2`, candidate `1.0.0-next.1` envelope/payload verification, CP TypeScript gates and compiled authoritative `222/222`, the product release-authoritative suite, and native `cargo check -p lightbi-tauri`.

This is **source acceptance, not deployed runtime acceptance**. Active immutable runtime remains `g-2026-09-04-next-031` on 5272/5273/5274 with worker healthy, `phase2aFrozen=false`, Production untouched and owner role rotation not performed. The immediate gate is to cut immutable NEXT032 from exact product `262bd768` + CP `72eacf75`, wire a generation-owned copy of signed pack `1.0.0-next.1`, then prove catalog/artifact identity, stage→activate, restart persistence, invalid/corrupt fail-closed behavior, rollback, downgrade rejection and active Micro Brain identity. Only after that runtime acceptance does the critical path return to Signed Transport integration.

## 110. 2026-09-04 Intelligence Pack V1 runtime acceptance; NEXT032 active

Intelligence Pack Updater V1 is now closed through NEXT/Internal runtime acceptance. Immutable `g-2026-09-04-next-032`, parent `g-2026-09-04-next-031`, is active on 5272/5273/5274. The generation manifest pins product `262bd768fc48b7eb4e8948631da853da318f610c`, reused Core-server binary provenance `ed4b9233f690b9ddeb872ee858e0e9a8c46644e0`, CP `72eacf75209b6984955bec362ab7752176184478`, schema `067_catalog_quarterly_pricing`, app `0.9.2-beta.7-next.32`, manifest SHA-256 `28e8f20e9acdfb037fe464a95cb0039489bca8333c620e16757de0073fecda40`, and Core binary SHA-256 `95be788197a853cff8c6d1665ef4d62e62ac71fd9dfa9546253efcda0b78aa7e`. CP snapshot provenance is exact `72eacf75`, with the Intelligence Pack directory owned by this generation and no Production authority.

The real NEXT HTTPS catalog and artifact edge pass after activation and after a full NEXT-only systemd restart. `1.0.0-next.1` is served with envelope SHA-256 `1c2446321ebecec5687f01f95dff0efe574efe4e56bdf10f13ed0802caea3376` and payload SHA-256 `6415fddef704732e0d2e08936aaed729278f4a9c759b30a4daacb1c7ab7d8ec0`; downloaded bytes are identical to the generation-owned object. Restart changed all four NEXT process IDs, then diagnostics again reported NEXT032, CP/worker `72eacf75`, schema current/pending `[]`, worker healthy and Trust still `blocked_pending_phase2a_freeze`.

Native-store acceptance uses the exact signed candidate and the pinned non-promotable INT TEST public key in isolated temp state. It proves stage→activate, fresh-process reconcile of the active pack, monotonic accepted-version-floor rejection, and corrupt-state fail-closed repair to bundled intelligence. The production frontend handoff is also exercised with the exact candidate payload: 2/2 tests prove the verified active identity reaches the normal Micro Brain accessor and bundled fallback clears it. Browser/non-native runtime deliberately remains bundled; catalog reachability is not native activation.

There is currently only one signed durable Internal pack version. Therefore deployed-live previous-version rollback is **not** claimed. Rollback, previous recovery, interrupted staging and invalid-active recovery are proven by the exact `lightbi-intelligence-pack` cryptographic fixture suite, which passes 6/6; a future second signed Internal pack can add a true two-version live rollback rehearsal without changing the contract. This preserves the rule that missing evidence is reported as missing rather than manufactured.

The accepted updater does not alter semantic authority: signature authenticates bytes, while Registry/Dictionary → Micro Brain → Evidence Resolver → Domain Support → Metric Runtime remains the authority chain. No owner role rotation was performed, `phase2aFrozen=false`, and Production remains untouched. The Road-to-1.0 executable next action is now **Signed Transport integration**: canonical query binding, response integrity, route-class policy, `native_http_request` wiring and replay/fallback negative probes, followed by packaged Windows/UAT and integrated release acceptance.

## 111. 2026-09-04 Decision Presentation + UI/UX refactor direction

- Owner approved a planning direction to refactor LightBI's presentation layer from the supplied chart-reference review through Question/Perspective, Decision Workspace, Dashboard, Deep BA, BA Step 2, evidence placement and PNG/PDF export.
- Durable implementation plan: [`../history/agent/plans/AGENT_IMPLEMENTATION_PLAN_DECISION_PRESENTATION_UI_UX_REFACTOR_2026-09-04.md`](../history/agent/plans/AGENT_IMPLEMENTATION_PLAN_DECISION_PRESENTATION_UI_UX_REFACTOR_2026-09-04.md).
- Product rule: **canvas-first, card-by-exception**. Typography, whitespace, alignment and grouping should create hierarchy before borders/cards; nested-card depth is zero by default.
- Architecture direction: `Understanding + semantic MB -> Question/Perspective Intelligence -> governed question/metric authority -> governed analysis artifacts -> MB presentation advice -> deterministic Narrative/Visualization/Dashboard planners -> Report/Presentation -> calm workspace + shared Evidence Inspector`.
- Domain and Perspective are now treated as distinct concepts. MB/domain inference may inform relevant questions, domain visual conventions and domain context, but it remains non-authoritative for metric/formula/join/runtime/domain-support decisions.
- Deep BA target is an answer-first board-style report; BA Step 2 is a focused investigation of a selected subject/finding rather than a full Deep BA rerun on filtered rows.
- Default management language is descriptive rather than evaluative: highest/lowest observed, increase/decrease and contribution are separate from favorable/adverse interpretation. Metric desirability must be known before good/bad language is used.
- Evidence remains mandatory but moves to progressive disclosure through a shared inspector. Claims should distinguish source evidence, governed calculation, MB/domain context, mixed inference and hypothesis.
- Dashboard composition must become narrative/domain-aware; hard-coded Bar breakdowns and technical deduplication alone are insufficient.
- 2026-09-06 amendment: Frappe Books is a required product-source study at DPR-0 for shell/navigation, business-screen density, tables/forms and dashboard hierarchy; Frappe UI remains the companion primitive/design-language reference. Both are external references only, not framework dependencies.
- Visualization planning now includes a canonical Chart Pattern Library: `data semantics -> analytical intent -> chart pattern -> story/layout role -> theme`, with positive/negative suitability rules, semantic color modes and approximately 20–30 high-quality patterns rather than chart-type accumulation.
- 2026-09-06 owner decision: MB formally joins DPR-2..DPR-7 as a bounded semantic/presentation advisor. A separate presentation knowledge/index lane may rank common-domain profiles, analytical intents, chart patterns, perspective priorities, dashboard roles, narrative order, anti-patterns and abstention signals; deterministic planners remain the only layer allowed to accept/reject a final presentation plan after governed evidence checks. MB advice can reduce authority or abstain, never create metric/formula/join/runtime authority.
- 2026-09-06 source closure: product `4be593ae57b4b1385a833675dd4ea2349900d378` adds the isolated MB presentation-advisory lobe (88 cards; 32 domain profiles; 31 chart patterns; 12 perspectives; 7 constitutional cards), compiled at 3,360,712 bytes. Combined semantic + presentation raw indexes are 9,966,179 bytes under the 20 MiB near-term ceiling. Targeted closure tests passed 14/14; isolated MB-7 benchmark retained the same three ETA recoveries with zero confirmed semantic regressions. Runtime wiring remains deferred to DPR-2..DPR-7.
- PNG/PDF export must move from giant-DOM screenshot slicing to a shared semantic report-page model with real pagination.
- This checkpoint is documentation/planning only. It does not change current metric truth, MB authority, active NEXT runtime, Production, or the current Signed Transport execution gate; implementation kickoff remains owner-gated.

## 112. 2026-09-05 Signed Transport V2 source closure; NEXT runtime gate opens

The Road-to-1.0 Signed Transport implementation is source-closed on the successor while the active runtime remains NEXT032. Public product branch `codex/r1-roadmap-integration` is clean/pushed at `6c1f11758093f21b1f7c03218d8fea52d3602d0f` (`feat(trust): integrate native signed transport v2`); private CP branch `codex/r1p14-signed-transport` is clean/pushed at `3afb85b38e7cd6f9bd65eafbda723f9f6e0e88d4` (`feat(trust): verify signed transport v2 requests`).

V2 preserves the earlier request-proof V1 contract and adds query-safe/native enforcement rather than redefining it. The signed proof binds method, canonical path+query, exact raw-body digest, nonce, persisted monotonic sequence, certificate identity and the nonce issuance timestamp to the installation device Ed25519 key. The actual native route set is intentionally narrow: `/api/account/*` and `/api/license/activate` are protected; pairing and installation trust challenge/issue/nonce are bootstrap; unrelated public reads remain outside V2. Protected calls require real Tauri application context, and Signed Transport failures cannot fall back to WebView transport.

The private verification plane remains verification-only. Distribution forwards V2 proof material over the Internal UDS to the isolated attestation appliance and never receives signer credentials or a generic signing endpoint. Accepted responses carry `lightbi.next-response-correlation.v1`, accepted request sequence and response SHA-256; the native client checks sequence and body digest. This is HTTPS-bound correlation/integrity evidence, not a claim of independently server-signed responses. Stable/public response signing, if later required, needs a separate purpose-specific authority decision.

Final source evidence is green: attestation `16/16`; full private CP `223/223`; frontend native transport `11/11` plus TypeScript; real Tauri Signed Transport compile/test `6/6`; source-size gate `516` modules with `main.rs` at `864` lines; full product release-authoritative suite PASS with governed regression `11/11` files and `41/41` tests. SSD guard remained below its 88% stop threshold; final filesystem use was approximately 75% with a 17 GiB rebuildable `target/` cache.

This chapter does **not** claim a new active generation. After the VPS reboot, the exact existing NEXT032 activation recipe was reused and all four NEXT user services returned healthy; public generation identity again reports `g-2026-09-04-next-032`, source `262bd768`, CP `72eacf75`. No owner role rotation occurred, `phase2aFrozen=false`, and Production was not touched. The immediate Road-to-1.0 gate is an immutable NEXT successor built from `6c1f117...` + `3afb85b...`, followed by positive native signed-request and negative query/body/replay/downgrade/response-correlation acceptance. Packaged Windows/UAT and integrated release acceptance remain after that runtime gate.

## 113. 2026-09-05 Signed Transport V2 NEXT033 edge/runtime acceptance

The Signed Transport V2 server/edge runtime gate is accepted on immutable NEXT `g-2026-09-05-next-033`, parent `g-2026-09-04-next-032`. The generation pins product source `6c1f11758093f21b1f7c03218d8fea52d3602d0f`, reused Core-server binary provenance `ed4b9233f690b9ddeb872ee858e0e9a8c46644e0`, private CP `3afb85b38e7cd6f9bd65eafbda723f9f6e0e88d4`, schema `067_catalog_quarterly_pricing`, app `0.9.2-beta.7-next.33`, manifest SHA-256 `75bece214fed73acf836caf1421f128b476bfb890d8f29dd9ba9d8d861c5ced9`, Core binary SHA-256 `95be788197a853cff8c6d1665ef4d62e62ac71fd9dfa9546253efcda0b78aa7e`, and CP server SHA-256 `768852b6d4d6e73850c2d69c6fbe1ec34b9d91ea5dead8917a2ce25f1a871742`. The previously accepted Intelligence Pack `1.0.0-next.1` remains byte-identical at envelope SHA-256 `1c2446321ebecec5687f01f95dff0efe574efe4e56bdf10f13ed0802caea3376`.

The verification-only attestation appliance was rebuilt from exact CP source `3afb85b...` against Trust Contracts `10de4da8...` as image `lightbi-next-trust-attestation:3afb85b38e7c-trust-10de4da8`, image ID `sha256:1f8ff4eedcf9ed91875eb1a4fac3f258880f1ef86831c20061ada8fd15d2fa60`. Runtime health advertises both V1 and V2 request protocols while remaining `networkAuthority=false` and `signingAuthority=false`; the container is non-root, read-only and `network=none`. Distribution receives only installation-issuer and attestation UDS paths. Process-environment and mount inspection confirms no signer token/socket/private-key authority is present in Distribution or the attestation verifier.

Acceptance crossed the real NEXT HTTPS edge. A valid ephemeral TEST ATT/device proof reached `/api/account/session?a=alpha&z=2`; the independent account layer returned unauthenticated `401`, while the response carried the correct `lightbi.next-response-correlation.v1`, accepted sequence and exact response SHA-256, proving V2 verification completed before normal route authorization. Reusing the accepted proof failed `attestation_nonce_replayed`; changing the signed query failed `attestation_request_target_mismatch`; changing signed POST body bytes failed `attestation_body_hash_mismatch`. The four NEXT services and verifier were then restarted, all process IDs changed, generation/CP/worker identity remained NEXT033 / `3afb85b`, worker remained healthy, Trust remained `blocked_pending_phase2a_freeze`, and the external HTTPS V2 positive/negative probes passed again.

This acceptance is deliberately bounded to the **NEXT/Internal server/edge transport runtime**. The probe certificate was ephemeral operator-only TEST evidence signed through the isolated rehearsal signer; signer material was never delegated to Distribution. The published TEST REL catalog currently contains only two Windows installer RELs and no `LightBI.exe` runtime REL for `release:0.9.2-beta.7-next.33:windows:x86_64:runtime`. Therefore packaged Windows `ensure_installation_trust`, OS-protected device-key persistence, actual packaged `native_http_request` execution and runtime WebView-downgrade behavior are **not yet owner/UAT accepted**. Source tests lock the no-downgrade contract, but packaged-native evidence belongs to the next Windows/UAT gate and must not be inferred from this operator harness.

No owner role rotation occurred, `phase2aFrozen=false`, and Production remains untouched. Road-to-1.0 now advances to packaged Windows/native Trust + UAT on exact `6c1f117` / `3afb85b`, including publication/verification of the exact NEXT runtime REL needed for installation trust, followed by integrated release acceptance.


## 114. 2026-09-05 NEXT033 canonical Windows runtime REL + server installation-trust closure

The packaged-Windows machine gate has advanced beyond the server/edge boundary recorded in §113 without converting machine proof into owner acceptance. GitHub Actions run `33933622073` on the governed `codex/r1p13-rc-acceptance` authority branch built exact product source `6c1f11758093f21b1f7c03218d8fea52d3602d0f` while embedding private CP identity `3afb85b38e7cd6f9bd65eafbda723f9f6e0e88d4` and NEXT generation `g-2026-09-05-next-033`. The meaningful Windows steps all passed: exact-source/authority proof, release gate, native tests, optimized Tauri/NSIS build, canonical acceptance evidence and artifact upload. The orchestration commit is `21ea8f8bc1d7af2959b11fb88aced8fa3e45bc98`; it is CI authority only and does not replace the product successor head.

The downloaded Actions artifact was verified byte-for-byte against GitHub artifact ZIP SHA-256 `162a3246e5744d4be73c6008512bf6d238682aaf7e957bb968460773406b7572`. Canonical `LightBI.exe` is 76,444,672 bytes at SHA-256 `77952884cfa93559157536e6b884a12522f4f5f02123fe8717ba91ea3a0dae9d`; the NEXT033 NSIS installer is 31,794,432 bytes at SHA-256 `e02b61b270181cffa52dff37fb3f90db8b5ed3428df001f3e0cf733d0c79c5d4`. `rc-acceptance.json` binds those bytes to version `0.9.2-beta.7-next.33`, runtime release ID `release:0.9.2-beta.7-next.33:windows:x86_64:runtime`, source `6c1f117...`, CP `3afb85b...`, `next_internal_test_only`, `production_authority=false`, `stable_publication_authority=false`, and `owner_uat_accepted=false`. Windows publisher evidence remains `NotSigned`; no OS-publisher acceptance is claimed.

The exact runtime REL was then published through the canonical NEXT TEST release signer, first into an isolated staging copy and then atomically into the live Internal trust catalog with `index.json` written last as the publication marker. External HTTPS `/distribution-api/internal-trust/` returns byte-identical catalog and REL bytes. The REL is signed by TEST REL key `next-test-rel-2026-01`, uses authority `next-test-20260902-031220`, Trust Contracts `10de4da8e551a46f93f7b62985a0a6e611581b8e`, and binds exactly the runtime SHA/size above. Independent verification using the externally fetched Root pin and issuer keyset passes. The TEST namespace remains `promotableToProduction=false`.

The first exact installation-trust challenge correctly failed `installation_issuer_release_not_allowed` because the installation-issuer process snapshots `index.json` only at startup. Source audit proved no installation-issuer code changed between its image source `4d6076b8f628` and current CP `3afb85b...`, so the same image was restarted rather than rebuilt or patched. Its PID changed, UDS health returned `next_internal_test_only`, `signingAuthority=false`, `signerPurpose=attestation_only`, and `productionAuthority=false`. A second external challenge using an ephemeral Ed25519 device key and the exact NEXT033 runtime identity succeeded through challenge, device-signature proof and certificate issue; the issued certificate remained TEST-only with `productionAuthority=false`. This closes server-side exact-runtime REL authorization and installation-certificate issuance.

Owner screenshots independently show the Windows UI running NEXT033 with parent NEXT032, product `6c1f117`, CP `3afb85b`, healthy Core/CP/worker, Isolation OK and Intelligence Pack `1.0.0-next.1` active. Those screenshots are visual identity evidence only. Current `BuildIdentityPanel` reads OS-publisher evidence but does not ingest the returned REL/artifact/installation-trust result, so its `Not evaluated` / `unavailable` fields cannot prove whether a certificate was issued and may remain unchanged after restart. Internal native startup still invokes `ensureNativeInstallationTrust` once, fire-and-forget, so a close/reopen is required to make the packaged app retry. The owner-side proof must therefore use exact issuer/attestation runtime evidence from that packaged startup and protected request, not the current panel labels. Until the packaged process proves canonical runtime acceptance, OS-vault-backed installation identity and V2 `native_http_request` with no WebView downgrade, packaged Windows/native UAT remains open.

No owner role rotation occurred, `phase2aFrozen=false`, and Production remains untouched. The immediate Road-to-1.0 gate is now **owner-side NEXT033 packaged-native startup/installation-trust + signed-request/no-downgrade proof**, followed by integrated release acceptance.


## 115. 2026-09-05 Owner packaged NEXT033 restart fails closed on non-canonical runtime bytes

The first owner-side packaged-native retry after §114 produced a decisive negative acceptance result rather than an ambiguous UI state. The owner fully closed and reopened the Windows NEXT033 app, then opened Account. The installation issuer recorded `installation_issuer_runtime_artifact_mismatch` for that packaged startup; no attestation/V2 verifier event followed. Because the issuer had already loaded the canonical NEXT033 REL and an operator exact-runtime challenge had passed earlier, this rejection means the running owner executable did not match the canonical `LightBI.exe` artifact identity pair authorized by the REL: SHA-256 `77952884cfa93559157536e6b884a12522f4f5f02123fe8717ba91ea3a0dae9d`, size 76,444,672 bytes. The issuer intentionally does not log the submitted runtime hash/size, so the exact non-canonical owner hash is not claimed.

This is expected fail-closed behavior, not a Trust bypass candidate. The Windows UI still reports version `0.9.2-beta.7-next.33`, generation NEXT033, product `6c1f117` and CP `3afb85b`, but version/generation labels are not sufficient release identity. A same-version executable produced by a prior/non-canonical build cannot obtain installation trust merely because its branding or embedded generation metadata matches. No verifier rule was weakened and no alternate REL was published for the mismatching bytes.

The packaged-native gate is therefore blocked on replacing the owner's current installation with the canonical CI-built NEXT033 installer from Actions run `33933622073` (installer SHA-256 `e02b61b270181cffa52dff37fb3f90db8b5ed3428df001f3e0cf733d0c79c5d4`). After that install, the app must be started again so `ensureNativeInstallationTrust` can hash the actual executable, obtain the TEST installation certificate on the OS-protected device key, and only then exercise a protected `/api/account/*` request through Signed Transport V2. Until those events appear in issuer/attestation/Distribution evidence, owner packaged-native acceptance remains false. Production remains untouched, owner role rotation is not performed, and `phase2aFrozen=false`.


## 116. 2026-09-05 NEXT033 NSIS runtime identity drift root cause confirmed

The owner-side failure recorded in §115 is now traced to the Windows acceptance/package pipeline rather than to an incorrect owner install. The owner independently hashed the freshly installed running `LightBI.exe` as SHA-256 `2031fc4b9185b1c649f75b5463ebd0d2a09c603dd8edcf0419aff700a57e131c`, size 76,444,672 bytes. To distinguish local residue from package behavior, a separate Windows Actions probe (`33939455110`, orchestration `972170559c635240958eae4bf31ebecdbb81df28`) downloaded the exact previously published canonical NEXT033 artifact from run `33933622073`, verified the standalone runtime SHA-256 `77952884cfa93559157536e6b884a12522f4f5f02123fe8717ba91ea3a0dae9d` and installer SHA-256 `e02b61b270181cffa52dff37fb3f90db8b5ed3428df001f3e0cf733d0c79c5d4`, silently installed that exact NSIS package into an isolated runner directory, then hashed the installed executable. The runner installed SHA was exactly `2031fc4b9185b1c649f75b5463ebd0d2a09c603dd8edcf0419aff700a57e131c`, again 76,444,672 bytes, byte-identity matching the owner machine and not the REL-authorized standalone file.

The root cause is therefore an **acceptance artifact identity error**: the old workflow treated `target/x86_64-pc-windows-msvc/release/LightBI.exe` copied beside the installer as the runtime authority, but did not prove that those bytes were the executable emitted by NSIS installation. The installation issuer correctly rejected the actual packaged runtime; the owner did not install the wrong build. The old `0.9.2-beta.7-next.33` TEST REL remains immutable historical evidence and is not rewritten to bless the different installer payload.

The remediation is isolated to NEXT/Internal acceptance. Acceptance branch `codex/r1p13-rc-acceptance` commit `7dfda736a615aaf345918dd46bd9e377c5323ec1` introduces packaging revision `0.9.2-beta.7-next.33.1`, replaces wall-clock build metadata with the accepted source commit timestamp, silently installs the generated NSIS package in CI, promotes the executable produced by that install as the canonical `LightBI.exe`, and records `runtime_identity_source=nsis_silent_install` plus `installer_payload_verified=true`. The pre-bundle executable hash remains diagnostic evidence only. GitHub Actions run `33939698146` is the sole current canonical Windows remediation run and is still pending at this checkpoint; no owner install of `33.1` is requested until that run and its installed-runtime identity evidence pass.

The private runtime-REL publisher is also hardened at source commit `263edd0c8b88a3117a51ce5f0260dd42d3e17935`. It now accepts only NSIS-installed-runtime acceptance evidence, rejects pre-bundle-only authority, computes publication conflict checks before writing public state, and refuses runtime REL re-publication for an already-published version (`next_runtime_rel_republication_forbidden`). Focused release-publication tests pass 6/6 and the complete private control-plane authoritative suite remains 223/223. This publisher hardening has **not** replaced the active NEXT033 runtime CP; active runtime remains `3afb85b...` while `263edd0...` is the operator/publication-source head.

No Production service, Production trust material, role assignment or Phase-2A freeze state changed. `phase2aFrozen=false`. The exact next gate is: wait for run `33939698146`; if it passes, verify `33.1` artifact and installed-runtime evidence, publish a **new immutable TEST runtime REL for `33.1`** through the hardened publisher, reload the NEXT installation issuer, prove server-side challenge→certificate on that exact installed-runtime SHA, and only then ask the owner to install the `33.1` package for packaged-native Signed Transport V2 UAT.


## 117. 2026-09-05 NEXT033.1 NSIS-derived runtime identity repair and server trust closure

- Corrected Windows acceptance run `33939698146` completed successfully for packaging revision `0.9.2-beta.7-next.33.1` while retaining product source `6c1f11758093f21b1f7c03218d8fea52d3602d0f`, runtime CP identity `3afb85b38e7cd6f9bd65eafbda723f9f6e0e88d4`, and immutable server generation `g-2026-09-05-next-033`. The acceptance workflow now derives runtime authority from the executable produced by a silent installation of the just-built NSIS package rather than from the pre-bundle Cargo output.
- Canonical installed runtime identity is 76,444,672 bytes / SHA-256 `8c6678d304cba3039cb8886ecbd75f6bb54dafabaca08bd85aed124e0237b811`; corrected NSIS installer identity is 31,783,547 bytes / SHA-256 `fe4357a676dfdc11130d1f79ef32b8b1f965b82932abe6d6d9378d4e85dec8f5`. `rc-acceptance.json` records `runtime_identity_source=nsis_silent_install`, `installer_payload_verified=true`, `production_authority=false`.
- Private publication authority is hardened at CP `263edd0c8b88a3117a51ce5f0260dd42d3e17935`: pre-bundle-only acceptance cannot publish a runtime REL and an existing version cannot silently change artifact SHA. The erroneous historical `next.33` TEST REL remains immutable evidence and was not rewritten.
- `release:0.9.2-beta.7-next.33.1:windows:x86_64:runtime` was published through the non-promotable TEST REL authority only after staging publication, exact artifact hashing, cryptographic Root/keyset verification and a bounded stage/live diff. Live mutation changed only the new immutable REL, `runtime-latest.json`, and `index.json`, with `index.json` written last. External HTTPS re-fetch verifies the exact REL SHA/size cryptographically.
- The snapshot-based NEXT installation issuer was restarted without rebuilding it so it could reload the new TEST index. A real external challenge using an ephemeral Ed25519 device key, exact `next.33.1` runtime identity and canonical transcript then issued TEST certificate `next-cert-ce0f0135-b5af-4dbd-8a4a-f17309aab6b3`; `productionAuthority=false`. This closes the machine/server publication and issuance repair, not owner packaged-native UAT.
- Exact next owner gate: install only the verified `0.9.2-beta.7-next.33.1` NSIS artifact, restart the app, open Account, and correlate that actual Windows process through issuer → attestation → protected V2 request. OS publisher remains `NotSigned`; Production remains untouched, `phase2aFrozen=false`, and no role rotation occurred.


## 118. 2026-09-05 NEXT033.1 packaged device-key continuity failure and NEXT034 hardening gate

- Owner installed corrected `0.9.2-beta.7-next.33.1`. Live installation issuer proved the real packaged runtime passed challenge and certificate issuance for immutable REL `release:0.9.2-beta.7-next.33.1:windows:x86_64:runtime`; certificate `next-cert-3f9a0a28-eb44-4a8f-ba64-2230ea99d9d5` was issued.
- After attestation verifier reload, the real packaged Account request received a nonce for that exact certificate but failed closed as `attestation_device_signature_invalid`. This proves packaged V2 reached the verifier, but does not prove accepted Signed Transport.
- Code audit rules out the Google/account session credential as the same key slot: account session uses `digital.thaiduy.lightbi / account-session`, while installation trust uses `digital.thaiduy.lightbi / installation-trust-ed25519-v1`. Rust→exact Trust Contracts JS V2 signature verification also passes, ruling out the current canonicalization/signature-format implementation as the observed failure.
- Product `410f318e51bc969d8ee4bad34840cecfff531f0a` hardens device-key continuity without changing V2 wire semantics: Windows cross-process named mutex around key creation/load, certificate-payload→current-key binding validation before persistence and before signed transport use, plus regression coverage. Local gates PASS: installation trust 4/4, Signed Transport 6/6, cross-language verifier proof PASS, complete `test:release-1.0` PASS with governed 41/41.
- Immutable NEXT034 is active, parent NEXT033, product/source `410f318...`, CP `3afb85b...`, reused Core binary SHA `95be7881...`, generation manifest SHA `b2c9f8f9...`; external generation/worker/Trust sanity PASS. Windows acceptance run `33943332283` is the remaining source/package compile gate before a new immutable runtime REL and owner retry.
- Production untouched, owner role rotation not performed, `phase2aFrozen=false`. Owner packaged-native UAT remains open.


## 119. 2026-09-05 NEXT036 native OAuth reaches device activation; protected account refresh exposes Signed Transport concurrency gate

Owner Windows UAT has advanced through immutable `0.9.2-beta.7-next.36` without closing R1-P13. Canonical Actions run `33956396827` PASSes machine acceptance for product `9f728bd674cd8812b035098c1a3487dfa6f4936f`; the NSIS-installed runtime authority is 76,492,288 bytes / SHA-256 `3e492343d622179b41e9dc8f6a821c5d270ee0cba94dde745b144e163404fe11`, installer SHA-256 `3518617f1dd851d9987d31cac4a68c48bc8a12a4d5650f6e0f9834dcc531d640`. The new Internal TEST runtime REL is published and a fresh server installation challenge→certificate probe PASSes. These remain machine/server proofs only; `owner_uat_accepted=false`.

The latest owner evidence shows the real Google/native account flow now reaches the Google chooser, returns to `/account?native=connected`, and creates/activates a Win32 device reporting `0.9.2-beta.7-next.36`. The desktop Account page nevertheless fails closed on its protected account refresh with `signed_transport_response_correlation_invalid`. Therefore browser OAuth/native-login completion is materially working, while packaged-native `/api/account/session` Signed Transport remains open.

Current source exposes a high-confidence concurrency mechanism matching the live trace. `AppLayout.tsx` and `Settings.tsx` each instantiate `useLightBIAccount()`, whose mount effect independently calls `loadLightBIAccount()` and protected GET `/api/account/session`. `native_http.rs` obtains a fresh nonce per request and derives `sequence = last_accepted_sequence + 1`; two concurrent reads can therefore sign the same next sequence. The current investigation observed two near-simultaneous session GETs and one `attestation_sequence_replay`. CP response correlation is attached only after successful V2 verification, while the native client requires verified correlation on the signed response, so a replay rejection can surface locally as `signed_transport_response_correlation_invalid` rather than trusting an uncorrelated error body.

This mechanism is not yet declared closed root cause until one sanitized trace binds the duplicate hook requests to the exact failing pair. The remediation must preserve authority: remove duplicate account-refresh ownership/single-flight the UI path and harden Signed Transport sequence concurrency generally, including legitimate parallel protected calls; do not accept missing correlation, relax replay protection or downgrade to WebView. Any executable fix requires a new immutable successor (NEXT037 or later), exact NSIS-installed runtime identity, new TEST REL and fresh owner UAT.

Runtime identities remain intentionally split: engine symlink is still `g-2026-09-05-next-034`; live Internal CP on 5274 runs source `2e6ad2370f54f1f22dde06cb886cc410056df0d0`; owner Windows package is NEXT036. Production remains untouched, no role rotation occurred and `phase2aFrozen=false`. Detailed continuation evidence is `../history/agent/handoffs/post-beta/AGENT_HANDOFF_NEXT036_SIGNED_TRANSPORT_RESPONSE_CORRELATION_2026-09-05.md`.

## 120. 2026-09-05 NEXT038 Signed Transport concurrency remediation reaches owner-UAT gate

The NEXT036 Account failure from §119 is now causally closed. A sanitized live trace bound the owner-visible `signed_transport_response_correlation_invalid` to concurrent protected Account refreshes: one `/api/account/session` request was accepted at sequence 32 and a near-simultaneous sibling request was rejected as `attestation_sequence_replay`. Because response correlation is attached only after successful V2 verification, the rejected request correctly failed closed at the native response-correlation boundary rather than trusting an unverified error body.

Product source `3423cad0e02a0a95cb3a80b74b12ecc818124d2e` preserves replay/correlation authority while fixing both concurrency layers. Account session reads are single-flighted per endpoint so independent `AppLayout`/`Settings` refresh ownership cannot duplicate one logical read. Native Signed Transport serializes the nonce → sequence → sign → send → correlated-response critical section process-locally and uses a named single-slot Windows semaphore for cross-process coordination without thread-affine mutex ownership. NEXT037 was superseded before publication after review identified that a Windows mutex held across `.await` could be released on a different Tokio worker thread; no NEXT037 TEST REL was published.

Local authoritative verification for `3423cad...` is green: Account API 13/13, Signed Transport 6/6, native process-lock race 1/1, Cargo check, desktop production build, complete release-authoritative suite and governed regression 41/41. Windows acceptance orchestration `1a28a5703cb68ea8ebc3c1831275d976df1cefe2` created immutable `0.9.2-beta.7-next.38` / `g-2026-09-05-next-038`, pinning live CP `2e6ad2370f54f1f22dde06cb886cc410056df0d0`. GitHub Actions run `33968609558` PASSed exact-source, Windows-specific native tests including the semaphore regression, NSIS build, canonical installed-runtime evidence and artifact upload.

Canonical NEXT038 installed runtime is 76,506,624 bytes / SHA-256 `7bc89b9a0704815e10b7c9fca627ae754bd20012b27d17139592b9ce12f11336`. The NSIS installer is 31,790,434 bytes / SHA-256 `3b5200cf0de5d182f36420f8ecbe7f7de8056195761d81f86d8f6d50790975c5`. `rc-acceptance.json` records `runtime_identity_source=nsis_silent_install`, `installer_payload_verified=true`, `publisher_status=NotSigned`, `production_authority=false`, `stable_publication_authority=false` and `owner_uat_accepted=false`.

The hardened Internal publisher staged `release:0.9.2-beta.7-next.38:windows:x86_64:runtime` first and produced a bounded live diff containing only the new immutable REL, `index.json` and `runtime-latest.json`. The live TEST catalog now points to NEXT038 under authority `next-test-20260902-031220`, Trust Contracts `10de4da8e551a46f93f7b62985a0a6e611581b8e`, `promotableToProduction=false`. Snapshot-based installation issuer and attestation verifier were restarted and both returned active with their private UDS sockets restored.

A fresh external installation-trust probe then PASSed challenge `201` → issue `201` for the exact NEXT038 runtime identity using an ephemeral Ed25519 device key kept only in memory. The issued certificate verifies against the public Root/keyset, matches the device key and runtime release, and remains `productionAuthority=false`; no probe private key was persisted. Machine/server authority is therefore closed for NEXT038.

The remaining gate is owner Windows UAT only: install the verified NEXT038 NSIS artifact, launch it, exercise Google/native Account refresh plus restart/near-simultaneous protected activity, and prove no legitimate `attestation_sequence_replay`, `device_signature_invalid` or `signed_transport_response_correlation_*` failure. Engine symlink remains NEXT034, live CP remains `2e6ad23`, Production is untouched, no role rotation occurred and `phase2aFrozen=false`.

## 121. 2026-09-06 NEXT040 closes holistic native account/auth machine-server gate

Owner testing of NEXT038 proved Signed Transport concurrency was fixed but exposed a separate native-session persistence defect. NEXT039 enabled the real Windows Credential Manager backend for `keyring` and added a persistence guard, but was deliberately not published as a TEST runtime REL after a full authentication audit found additional contract gaps. The audit covered Google OAuth, email registration/login, browser/native session authority, TOTP/recovery MFA, passkey handoff, device activation/revoke, logout, restart persistence and downgrade boundaries before any further successor was cut.

Product `6c15ef9810e0230a50895b916f8262cd110114a6` carries the complete native-account client fix. Native MFA now uses protected `/api/account/native/mfa/verify`; stale native 401 sessions clear the OS credential slot; email/native/Google token handoffs persist before the authoritative account read; registration adds password confirmation; and Windows credential persistence remains guarded as `UntilDelete`. The complete local release suite PASSed, including governed regression 41/41, Account client/stateful auth coverage, Signed Transport Rust coverage and Cargo/production build gates.

Control Plane `ac02c7f9f943d4221328e5e0fdf819a7d50cca03` makes authority boundaries explicit: any request asking for native authority must have verified Signed Transport context, browser account-security v1 remains cookie-authority, native MFA is protected, Google/MFA/passkey handoffs take precedence over stale browser sessions, and email registration cannot overwrite an established password. Focused auth/security tests PASS 56/56, full CP suite PASS 228/228, and a synthetic NEXT DB/Redis rehearsal PASSed register → verify → re-register safety → native password session → TOTP enrollment → native MFA → web MFA → logout → cleanup without persisting or printing secrets. Live NEXT API and worker were rotated together to exact CP `ac02c7f...` while engine generation remains `g-2026-09-05-next-034`; unsigned native login/MFA now fail closed with `signed_transport_required` while ordinary browser semantics remain available.

Immutable `0.9.2-beta.7-next.40` / `g-2026-09-06-next-040` is pinned by acceptance orchestration `b3bc5888c9b9117981a28d51e2666a432e71784b`. GitHub Actions run `34004815638` PASSed every Windows step: exact-source release gate, Windows account-vault persistence test, Signed Transport/native HTTP/installation trust tests, NSIS build, silent-install canonical runtime extraction, publisher evidence and artifact upload. Canonical installed runtime is 76,523,520 bytes / SHA-256 `36b323ad05beb6b13a6b6752fda588c6dcb023a9670be84b0bb559b26115d7c1`; installer is 31,792,197 bytes / SHA-256 `3ee3d10bdbc75ee14f7de12987dca136f0d7e8260aaca6a12a496f0c1de178a9`. Acceptance records `runtime_identity_source=nsis_silent_install`, `installer_payload_verified=true`, `publisher_status=NotSigned`, `production_authority=false`, `stable_publication_authority=false`, and `owner_uat_accepted=false`.

Transfer-only run `34007566525` verified those exact bytes and copied the canonical runtime, acceptance JSON and installer to immutable private R2 path `lightbi-next/acceptance-transfer/34004815638`; VPS re-fetch and re-hash PASSed. Hardened runtime publisher logic is byte-unchanged from baseline `263edd0...`. Staging publication produced a bounded three-file delta only: the new `next.40` runtime REL, `runtime-latest.json`, and `index.json`; the identical bounded live publication advanced TEST runtime authority to NEXT040 under `next-test-20260902-031220`, Trust Contracts `10de4da8e551a46f93f7b62985a0a6e611581b8e`, with `promotableToProduction=false`. External HTTPS `runtime-latest.json` is byte-identical to the live catalog snapshot.

Installation issuer and attestation verifier were restarted against the new snapshot; issuer reports TEST-only `productionAuthority=false` and attestation reports `statePersisted=true`. A fresh external challenge → device-signature → issue probe PASSed `201/201` for the exact NEXT040 runtime with an ephemeral in-memory Ed25519 key. The issued certificate verifies against the public Root/keyset, matches the device key and release, persists no private probe key, and remains non-production.

Machine/server authority for the complete account flow is therefore closed. The only remaining gate is owner packaged Windows UAT on the verified NEXT040 installer: test Google native login, email registration + verification, email/password login, configured MFA/passkey where applicable, account refresh, logout/re-login, and full app restart persistence. Acceptance additionally requires no legitimate `attestation_sequence_replay`, `device_signature_invalid`, `signed_transport_response_correlation_*`, `installation_issuer_release_not_allowed`, or missing native Bearer/session behavior. Production remains untouched, no owner role rotation occurred, and `phase2aFrozen=false`.

## 122. 2026-09-06 NEXT041 explicit device-slot replacement closes owner auth/device-management UAT

NEXT040 owner Windows testing closed the native Google/account path that had previously failed across response correlation, installation trust and Windows credential persistence. After NEXT040 TEST authority was available, the packaged desktop recovered without a reinstall, completed Google native login, persisted the native session and exposed authenticated account UI including `Invite to LightBI`. This established the accepted auth baseline before the device-slot successor was evaluated.

Product `4510b1ad15a82d084a8adcdbcc559a53e37bf70f` adds explicit device-slot replacement on top of NEXT040 without removing that authenticated UI. When an account has free capacity, the current installation attaches normally. When capacity is full, desktop enters a governed `Device limit reached` state instead of silently evicting another device or surfacing a raw server error. The owner may open `Replace a device`, `Manage devices`, retry after manual capacity changes, or back out. No hardware fingerprinting is introduced.

Replacement uses a short-lived one-time browser handoff carried in the URL fragment rather than the request URL. The browser shows the account's active devices and requires the owner to select the exact device to replace; no device is preselected and no oldest-device heuristic is authoritative. Commit re-checks slot capacity under the server-side transaction: if a slot became free meanwhile, the pending installation attaches without revoking anyone; if the account is still full, only the selected device is revoked, its account session is invalidated, and the pending native login is completed for the current installation. The desktop keeps the same pending native-login identity, so successful replacement resumes the original login without a second Google/password round trip.

Machine gates for the replacement change closed before packaging: product release-authoritative regression PASSed, CP full suite PASSed 229/229, and live synthetic NEXT DB/Redis rehearsals covered Basic 1-slot explicit replacement, Basic manual-revoke/resume, no-extra-revoke after capacity becomes free, multi-slot targeted replacement, isolation of non-selected devices and cleanup. Production authority remained false throughout.

Immutable `0.9.2-beta.7-next.41` / `g-2026-09-06-next-041` is pinned by acceptance orchestration `e5d86455f45998a6e80056b3d51586a0ea966886`, product source `4510b1ad15a82d084a8adcdbcc559a53e37bf70f` and Control Plane `f1f053e7e008a43b572edb414e3a986da04fed0c`. GitHub Actions run `34009698180` PASSed the complete Windows acceptance job. Canonical NSIS-installed runtime is 76,523,520 bytes / SHA-256 `aea8254493a5cde6c138afce6ed2afb421bb7c4e6a650e7d05a75b40a80647a2`; installer is 31,798,320 bytes / SHA-256 `d149c1fdb6eee6980b2cd0982079c948a1e6eb78c43717dcf288570450c9de6d`. Acceptance records `runtime_identity_source=nsis_silent_install`, `installer_payload_verified=true`, `publisher_status=NotSigned`, `production_authority=false` and `owner_uat_accepted=false` at machine-evidence creation time.

The NEXT TEST runtime REL was then advanced to NEXT041 under the existing non-production authority. Installation issuer and attestation verifier were reloaded, and a fresh external exact-runtime challenge → issue probe PASSed `201/201`; the certificate matched the ephemeral device key and exact release and verified against the public Root/keyset with `productionAuthority=false`.

Owner packaged UAT then exercised the actual full-slot path. Desktop showed the governed `Device limit reached` panel, `Replace a device` opened the browser selector, the owner explicitly selected the existing Win32 device, and completion returned the desktop to an authenticated account without a second login. The account view showed one active device and the historical devices as revoked, demonstrating selected-device revocation plus current-installation attachment. This owner evidence closes the native Google/account + explicit device-slot management workstream for NEXT041.

This section does **not** declare the whole Road-to-1.0 or all owner UAT complete. The owner has explicitly stated that additional technical debt remains before returning to the main roadmap. Email/password/MFA packaged owner testing is not promoted to PASS by this device-replacement closure unless separately demonstrated. Production remains untouched, no role rotation occurred, and `phase2aFrozen=false`.


## 123. 2026-09-07 Micro Brain Redis learning scheduler reaches MB-LQ2 machine closure on NEXT

The owner-approved Micro Brain R2/Redis learning pipeline has advanced through MB-LQ2 without changing Production authority. Control Plane source `dbf7bfeb77cac8bbabff4b1525b6e7efe3665697` adds a durable scheduler above the MB-LQ1 Postgres/Redis worker foundation. Learning jobs now carry an explicit `server|client` execution target so VPS Redis workers cannot consume Desktop contribution requests. Automatic selection is bounded by the approved 5:3:1 Small/Medium/Large service pattern, candidate aging/value/network-fit scoring, per-installation/global quotas and ready-lake pressure states `open -> reduced -> small_only -> paused`. Admin Queue/Scheduler mutations are strong-auth protected and audited; Redis remains coordination/dispatch acceleration rather than job or intelligence authority.

The complete Control Plane suite after LQ2 is `264/264 PASS`. Migration `070_micro_brain_learning_scheduler_hardening` was applied only after read-only status proved it was the sole pending migration; NEXT now reports `24` migrations and `pending=0`. The migration adds execution-target constraints/indexes, one-active-collection-job-per-installation protection and DB ceilings of 2 GiB for hard job size, active working set and per-installation daily contribution allowance.

A real NEXT Postgres+Redis rehearsal then proved Redis scheduler leader election, creation of two `client` collection jobs for synthetic active apps, exclusion of those jobs from server dispatch, rejection of a server lease attempt against a client job, and deterministic ready-lake `100%` pressure blocking with `backpressure_paused`. The Postgres rehearsal ran inside a rollback boundary; post-proof counts were `installations=0`, `jobs=0`, `contributions=0`, and the temporary Redis leader key was removed.

This is **source + schema + synthetic machine closure**, not live Admin rollout or owner UAT. Live NEXT Control Plane still runs `eea0de340615c145feeb8461d568364c5d041d66`; it has not been rotated to `dbf7bfe`, so the new Queue/Scheduler UI/API is not yet live-served. The user systemd manager PID `943` also remains uncontrollable because `/run/user/1000/bus` is absent; the new `short/default/long` units therefore remain unactivated rather than forcing a manager restart that could disturb protected trust services. Issuer PID `3817149` and signer PID `12020` remained unchanged. Next implementation phase is MB-LQ3 direct App -> R2 contribution transport; R2 learning intake is not active yet and no large payload is proxied through the VPS. See [`../history/agent/plans/AGENT_IMPLEMENTATION_PLAN_MICRO_BRAIN_R2_REDIS_LEARNING_PIPELINE_2026-09-07.md`](../history/agent/plans/AGENT_IMPLEMENTATION_PLAN_MICRO_BRAIN_R2_REDIS_LEARNING_PIPELINE_2026-09-07.md).

## 124. 2026-09-07 MB-LQ3 direct R2 contribution foundation reaches live fail-closed gate

The owner-approved Micro Brain learning pipeline has advanced from the MB-LQ2 scheduler into the MB-LQ3 direct contribution transport without routing large training bytes through the Control Plane. Product `65092cd86c7c63c861109a6c01796f1c35a62978` adds the native Desktop contribution loop and R2-only streaming uploader. Control Plane `387db91e1123cb9e6da6d565951c393fd43a65bf` adds the signed contribution API, durable upload/multipart metadata, privacy validator, least-privileged Unix-socket R2 broker boundary and migration `071_micro_brain_learning_r2_transport`; descendant `c302f9935f2d7809167838defffc19f9be24af15` hardens scheduler/Admin issuance so no collection job is created unless that broker is healthy.

The byte path is explicitly `Desktop -> Cloudflare R2`, while the CP receives only bounded signed control metadata. Desktop packages remain inside the LightBI app-cache outbox, use gzip/NDJSON with strict allowlisted evidence, and stream either one direct PUT or fixed 16 MiB multipart ranges. Server-side state is durable in PostgreSQL; Redis remains dispatch/coordination only. Installation ownership comes from the attestation-verified installation certificate rather than a client-supplied installation identifier. Validation verifies object size, SHA-256, record counts and privacy/schema constraints before `quarantine -> ready`; malformed/raw-business fields fail closed. Consent withdrawal aborts and deletes the staged local package and requires remote object cleanup rather than leaving an orphaned upload.

Machine gates are green for the implemented boundary: exact CP `c302f99...` passes `275/275`; Product release-authoritative regression passes with governed `41/41`; native Tauri tests pass `28`, fail `0`, with one live-only ignored case; the attestation verifier suite passes `18/18`. Migration `071` was applied only after it was proven to be the sole pending migration, and NEXT now reports `25` applied migrations with `pending=0`. Live NEXT CP, the existing worker and the new `short/default/long` workers all run exact `c302f99...`.

LQ3 also required a stronger Signed Transport identity return. The attestation verifier was rebuilt as `lightbi-next-trust-attestation:387db91e1123-trust-10de4da8`, preserving the exact TEST Trust Contracts commit `10de4da8e551a46f93f7b62985a0a6e611581b8e` and non-production/non-signing provenance. A live V2 proof on `/api/micro-brain/learning/poll` for the exact NEXT046 runtime returned a verified installation binding and exact runtime release with `productionAuthority=false`. Installation issuer PID `3817149` and signer PID `12020` were not restarted.

The live R2 transfer gate remains **OPEN by design**. The broker socket is absent and the scheduler's readiness gate has emitted no client collection jobs (`0` active and `0` created in the observed 15-minute window). All R2 credentials currently present on the VPS were checked by equality/fingerprint only and are the same broad credential/bucket already used by Production/distribution/backup; they were deliberately not reused. The VPS also has no Cloudflare API credential capable of creating a dedicated learning token. Full MB-LQ3 closure therefore requires a dedicated NEXT learning-only R2 credential/bucket, broker activation, and a real direct-upload/multipart/resume/validation/dedup/rejection/cleanup rehearsal.

Acceptance orchestration `2ba518263d7ef5072de1b364dfc977ee1f4515ef` has cut successor candidate `0.9.2-beta.7-next.47` / `g-2026-09-07-next-047`, pinning Product `65092cd...`, CP `c302f99...`, schema `071` and parent NEXT046. The workflow is push-triggered but intentionally not polled here; no NEXT047 build or owner-UAT PASS is claimed. Production remains untouched. See [`../history/agent/plans/AGENT_IMPLEMENTATION_PLAN_MICRO_BRAIN_R2_REDIS_LEARNING_PIPELINE_2026-09-07.md`](../history/agent/plans/AGENT_IMPLEMENTATION_PLAN_MICRO_BRAIN_R2_REDIS_LEARNING_PIPELINE_2026-09-07.md).


## 125. 2026-09-07 Worker Safety source/schema closure; live activation remains gated

The Micro Brain learning workstream has added a bounded Worker Safety layer before any real MB-LQ3 R2 traffic is allowed. Exact Control Plane source is now `975884e4a009049e94341aab570c9acef5c30c71`; this supersedes the source candidate used by the currently running CP/workers but is **not yet live-activated**. The exact full SHA behind the live `c302f99` abbreviation is `c302f9933c8ba50b189df2e596eb84f6e936881a`; the different suffix recorded in older §124/checkpoint prose is stale documentation and must not be used as Git truth.

Worker Safety fixes the execution model at one process and one running server job per `short|default|long` lane, a default global active-job budget of two with hard maximum three, PostgreSQL advisory process-slot authority, durable lane states/circuit policy, 10-second blocking Redis idle reads, 60-second DB rehydration/reclaim cadence, and bounded exponential DB/Redis outage backoff. Source closure is green at focused Worker Safety `23/23`, complete Control Plane `286/286`, and the real NEXT PostgreSQL rehearsal proving duplicate-process rejection, global-cap enforcement, stale-owner recovery rules, drain completion, poison-circuit opening and full synthetic cleanup.

Migration `072_micro_brain_worker_safety` was the sole pending migration before apply. NEXT now has `26` applied migrations, `pending=0`, and cleaned normal lane policy/state. An immutable exact-source deploy already exists at `/home/ubuntu/services/lightbi-control-plane-next034-975884e`, with prepared `0600` runtime env and user-systemd drop-ins for API, legacy worker and all three async lanes. The live API/workers still execute exact `c302f9933c8ba50b189df2e596eb84f6e936881a` because `/run/user/1000/bus` is absent. User systemd manager PID `943` also parents protected Trust launchers, so restarting/killing that manager merely to regain control is explicitly forbidden; if no demonstrably safe systemd-control path exists, the live activation gate stays OPEN. Installation issuer PID `3817149` and signer PID `12020` remain unchanged.

Real R2 learning remains downstream of this gate. The dedicated learning broker socket/credential is still absent; existing broad Production R2 authority must not be reused. After exact `975884e` is live and passes the full idle/resource/outage/stuck-job/Admin-control acceptance, provision a least-privileged NEXT learning-only R2 authority and then execute the bounded direct-upload rehearsal. NEXT047 remains only an older acceptance candidate pinned to pre-Worker-Safety CP/schema; prefer a fresh immutable successor after Worker Safety and R2 closure rather than consuming another owner install prematurely. Production remains untouched. Detailed continuation evidence is [`../history/agent/handoffs/post-beta/AGENT_HANDOFF_MB_LQ3_WORKER_SAFETY_AND_R2_GATE_2026-09-07.md`](../history/agent/handoffs/post-beta/AGENT_HANDOFF_MB_LQ3_WORKER_SAFETY_AND_R2_GATE_2026-09-07.md).


## 126. 2026-09-07 Worker Safety live gate closes on Redis-backoff successor `2d2b87e`

The Worker Safety live gate recorded as OPEN in §125 is now **closed on a newer exact Control Plane source**, after live acceptance found and fixed one real dependency-failure defect. The earlier `/run/user/1000` systemd blocker was a UID-path mistake: the live `ubuntu` account is UID `1001`, and `/run/user/1001/bus` plus `/run/user/1001/systemd/private` are present. Safe user-systemd control works by binding `XDG_RUNTIME_DIR=/run/user/1001` and the matching bus socket. User manager PID `943` was not restarted or killed, so protected Trust launchers were never used as a recovery mechanism.

Exact Worker Safety source is now Control Plane `2d2b87e51e63cbd8e2c7c037a415e98f5884603f` (`fix(worker-safety): bound redis reconnect ownership`), descendant of the §125 candidate `975884e4a009049e94341aab570c9acef5c30c71`. During the first isolated live Redis-outage rehearsal on `975884e`, `node-redis` kept reconnecting internally inside `connect()` and no outer `redis_bootstrap_error` was emitted for more than a minute. That behavior would prevent the Worker Safety exponential-backoff loop from owning retry cadence. The successor disables library-owned reconnect inside each connect attempt, bounds connect timeout to two seconds, and adds an explicit same-process reconnect path for Redis loss after startup. Full Control Plane verification remains `286/286 PASS`.

Post-fix machine evidence proves bounded failure behavior rather than merely process survival. An unavailable isolated Redis endpoint produced bootstrap attempts with approximately `0.931s -> 2.112s -> 4.175s` intervals; unavailable PostgreSQL produced approximately `1.168s -> 1.770s -> 3.209s` intervals under configured jitter. Both workers terminated cleanly on SIGTERM without a restart storm or hard kill. A separate isolated Redis container rehearsal dropped Redis after a successful connection and proved the same worker PID traversed `cycle_error -> redis_reconnect_error -> redis_reconnected` without spawning a replacement worker.

A fresh immutable deploy exists at `/home/ubuntu/services/lightbi-control-plane-next034-2d2b87e`. The live API, legacy worker and all three async lanes now run from that exact directory: API PID `1979932`, legacy PID `1979945`, short PID `1999295`, default PID `1980491`, long PID `1980855` at final closure read. Migration state remains `26/26`, `pending=0`, latest `072_micro_brain_worker_safety`. Loaded async systemd bounds remain `Restart=on-failure`, `RestartSec=15s`, start-limit `3/300s`, `TasksMax=64`, `MemoryHigh=384 MiB`, `MemoryMax=512 MiB`, `CPUQuota=75%`, `Nice=5`.

The complete live acceptance list is PASS on the successor boundary. A duplicate short-lane process exits `0` with `process_cap_rejected` and process count remains unchanged. An exact-candidate stuck handler spawns no child/helper and is aborted after `30.059s` with `async_job_max_wall_time_exceeded`, returning one bounded retry. Expired lease recovery remains heartbeat-aware: a fresh owner is protected while a stale owner is recoverable. The PostgreSQL global active cap allows two cross-lane leases and denies the third. A synthetic poison job follows `retry(1) -> retry(2) -> failed(3)`, opens the lane circuit for the configured cooldown, and `Reset circuit` returns failures/circuit to zero/null.

The Admin logical authority layer also passed without shell/systemd delegation: `Pause -> Resume -> Stop -> Start -> Drain -> Paused -> Resume -> Reset circuit` produced seven durable audit events. Drain completed to `paused` in about six seconds with no active job, while the short worker PID stayed unchanged. A final 35-second queue-empty observation measured short/default/long CPU at `0.0000% / 0.0286% / 0.0286%`; RSS delta was `+312 / 0 / 0 KiB`; no `cycle_error`, bootstrap, reconnect or circuit-open event appeared. Final cleanup proves `0` async jobs, `0` synthetic jobs, `0` synthetic heartbeats and `0` synthetic Redis dispatch markers; all lanes are `running`, failures `0`, circuit null, last error null.

Trust remained isolated throughout. Installation issuer launcher PID `3817149`, signer launcher PID `12020`, attestation launcher PID `1505209`, and user manager PID `943` were not intentionally rotated for Worker Safety. Final issuer health is `ok=true`, `trustSnapshotHealth=healthy`, publication count `12`, latest TEST release `0.9.2-beta.7-next.46`, `productionAuthority=false`; attestation health is `ok=true`, `networkAuthority=false`, `signingAuthority=false`. Production remains untouched.

Worker Safety Gate B is therefore **PASS/CLOSED**. Gate C is now the only immediate MB-LQ3 blocker: `/home/ubuntu/.local/run/lightbi-mb-r2-broker/broker.sock` remains absent and no dedicated least-privileged NEXT learning-only R2 credential/bucket has been provisioned. Existing broad Production/distribution/backup R2 authority remains forbidden for this purpose. Only after dedicated broker authority exists may the 13-point real R2 rehearsal begin. NEXT047 still predates Worker Safety/schema 072 and this Redis fix; prefer a fresh successor after R2 closure rather than promoting that older candidate.


## 127. 2026-09-07 MB-LQ3 Gate C preflight hardens the R2 broker; external credential remains the only activation blocker

Gate C preparation continued after Worker Safety closure without activating R2 authority. The Control Plane source advanced to `07ab6c3dd4bfbb3d4107d8c47d225b413a7f3f58` only to harden the dedicated Micro Brain R2 broker service. Review found the broker unit still used `Restart=always` with a three-second delay, which could create restart churn under bad/missing storage configuration. The hardened unit now uses `Restart=on-failure`, `RestartSec=15s`, `StartLimitBurst=3` per 300 seconds, `TasksMax=64`, `MemoryHigh=384 MiB`, `MemoryMax=512 MiB`, `CPUQuota=75%`, and `Nice=5`. Focused R2 tests PASS `11/11`; the complete Control Plane suite PASSes `287/287`.

An immutable broker candidate was built at `/home/ubuntu/services/lightbi-control-plane-next034-07ab6c3`. Its user-systemd unit is loaded with that exact WorkingDirectory but deliberately remains `inactive` and `disabled`; `/home/ubuntu/.config/lightbi-mb-r2-broker.env` is still absent, so no learning storage authority has been activated. A daemon reload did not rotate the live API/legacy/async units, which remain exact Worker Safety runtime `2d2b87e51e63cbd8e2c7c037a415e98f5884603f`.

The storage-secret boundary was re-proved against the live process environments. API, legacy worker, short, default and long processes contain `LIGHTBI_MB_R2_BROKER_SOCKET` but contain none of the R2 account/access-key/secret/bucket fields and none of the generic AWS/R2 access-key secret variables inspected. The intended authority therefore remains: Control Plane and workers may call the local Unix-socket broker, while only the broker process may ever receive S3-compatible R2 credentials.

External provider discovery found no connected Cloudflare/R2 integration. Wrangler `4.129.0` is present on the VPS but reports unauthenticated state. Existing configured R2 credentials remain the previously rejected broad Production/distribution/backup authority and are not eligible as a learning parent. Current Cloudflare R2 documentation supports R2 API tokens restricted to specific buckets and also temporary credentials scoped beneath an existing parent; this project must not use the latter to launder the broad Production parent into learning authority. Gate C therefore still requires a new dedicated NEXT learning bucket plus a new Object Read & Write R2 token restricted to that bucket. References: `https://developers.cloudflare.com/r2/api/tokens/` and `https://developers.cloudflare.com/r2/api/s3/temporary-credentials/`.

Until those dedicated credentials are placed only in a mode-`0600` broker environment and prefix confinement is proven, the broker must stay stopped and the 13-point real R2 rehearsal must not begin. Production remains untouched.


## 128. 2026-09-07 MB-LQ3 R2 learning transport and signed capability detour close; Roadmap 1.0 may resume

The MB-LQ3 learning detour is now **engineering-closed on NEXT** after Worker Safety, signed capability hardening, real Cloudflare R2 rehearsal and complete cleanup. This closure does not promote a Windows build, change Production authority or convert machine evidence into owner release acceptance. Product source is `40ca2d8b1dbd213bca510e51b9e09c5c19279f5d`; current Control Plane source is `b0d65b343d9bc73e90e90097d87eda3f92218d36`. The five live CP/API/worker units intentionally remain immutable runtime `fd18429e271da398062719a0e404389d85f46f97`, because descendants through `b0d65b3` only change the R2 broker/SigV4 path; the live broker alone runs immutable `b0d65b3`.

The owner explicitly superseded the dedicated-credential requirement from §127 for this NEXT lane: the broker may reuse the existing Distribution R2 S3 authority, but only behind a separate physical `mb-learn-db` base prefix beneath the existing Distribution namespace. Logical database/object paths remain `micro-brain-learning/next_internal_test_only/...`, so PostgreSQL is not coupled to the legacy Distribution bucket/base-prefix encoding. The mode-`0600` broker environment is the only process boundary holding R2 account/access-key/secret material; API, legacy worker and all three async workers retain only the Unix-socket broker reference. Broker authority remains hard-limited to the NEXT Micro Brain prefix and Production services were not modified.

The server capability boundary was also tightened before storage activation. Every `/api/micro-brain/learning/*` control request requires verified native Signed Transport plus attestation-derived installation binding. Product `40ca2d8...` uses strict native signed transport with no WebView fallback and persists a consent-withdrawal cleanup intent until the signed server cleanup succeeds. Withdrawal now removes `ready|training|learned` authority first, cancels related client/validation jobs, then removes R2 residue; validator transitions are guarded so stale workers cannot resurrect withdrawn data. Permanent schema/privacy rejection completes the validation job as rejected instead of feeding poison retry/circuit accounting.

Intelligence Pack update authority is now similarly layered: the catalog is a Signed Transport protected route; the catalog grants a short-lived artifact capability; package activation still requires the independent INT Ed25519 package signature and trusted key. Thus removing a desktop/UI guard from a fork does not recreate these protected server authorities. This statement is scoped to capability-bearing surfaces hardened in this workstream; it is not a blanket claim that every future LightBI server feature is already gated. Local Core remains intentionally usable without server authority.

The ordinary user Account security UI was repaired on the same successor lineage. TOTP and Passkey enrollment/revoke code already existed, but browser `/account` called v1 security routes without the canonical `/distribution-api` Control Plane prefix and therefore fell into the temporary-unavailable state. Commit `f1dd5e87a35ede84fefc72974700a676f3fd5128` fixes that routing; focused account-security verification passed `29/29`, and the full CP suite at that point passed `292/292`. The build-test `NEXT047` `installation_issuer_release_not_allowed` mismatch remains deliberately deferred until an official release build; no Trust launcher was restarted merely to service that test-build mismatch.

Real R2 rehearsal exposed and closed several issues that source-only tests had not found. Multipart presigned URLs initially returned `403` because SigV4 canonical query sorting used locale collation rather than AWS byte/ASCII ordering; missing-object HEAD returned HTTP `404` instead of the client's `{exists:false}` contract; and Cloudflare R2 multipart HEAD may omit `Content-Length`, requiring a one-byte Range GET fallback to recover total object size without downloading the object. Final broker source `b0d65b3...` includes all three fixes. Focused R2 verification passes `17/17`; complete Control Plane regression passes `297/297`.

The final live rehearsal passed all `13/13` checkpoints against real PostgreSQL, Redis, the Unix-socket broker and Cloudflare R2: broker/prefix health; app-like direct single upload; intake -> quarantine -> ready; streaming size/SHA/schema/privacy verification; unique ready authority; duplicate dedupe without authority growth; multipart crash/resume with durable part receipt; multipart materialization/validation to ready; malformed/private-field rejection with zero retry; bounded broker outage without CP payload proxy fallback; consent withdrawal removing authority and remote object; proof that contribution bytes travel by presigned direct R2 upload rather than through CP; and complete synthetic remote cleanup. Final cleanup is `contributions=0`, learning jobs `0`, Redis residue `0`, R2 residue `0`, scheduler disabled, all lanes running, failures `0`, circuits closed.

A final NEXT-only scheduler smoke used the exact live scheduler in a database transaction. Real installations were hidden only inside that transaction, one synthetic installation was inserted, policy was enabled, `runOnce()` scheduled exactly one `client` `intelligence.learning.collect.v1` job with reason `auto`, policy was disabled, and the transaction was rolled back. Post-smoke state is scheduler `false`, synthetic installations `0`, synthetic jobs `0`. This proves automatic issuance without exposing a real owner installation to a test learning job.

Worker Safety remains closed on the descendant runtime: all five CP processes execute immutable `fd18429...`; schema is `26/26`, pending `0`; final queue state is empty; short/default/long lanes are running with zero failures/circuits. Broker is active+enabled on immutable `b0d65b3...`. Installation issuer launcher PID `3817149`, signer PID `12020` and attestation launcher PID `1505209` remained unchanged through the final broker rotations. SSD was reduced from `92%` to `67%` by deleting only the Product Cargo `target/` build cache (~30 GiB); live deploys, databases and Trust state were not removed.

MB-LQ3, Worker Safety and the signed capability detour are therefore **CLOSED for NEXT engineering**. The immediate project position returns to the canonical Roadmap 1.0. A fresh successor should be cut from current source when the next owner-install/release gate requires packaging; the older NEXT047 candidate must not be treated as the post-LQ3 release candidate. Production remains untouched and promotion/role rotation remains owner-gated.

## 129. 2026-09-08 DPR-0 contract freeze chunk 1 closes React/i18n/presentation-debt boundaries

- Roadmap 1.0 execution resumed on a clean Product worktree based on optimized-li primary-logo commit `fde259a5441b36825f211914cbd0c82fc595f27f`; the main integration worktree and its five Phase-5m2 audit JSON files remained untouched.
- Product `85734e4a262b93b6b0964b37cd0b03148320d115` on `codex/dpr0-contract-freeze` removes the duplicate React-18 runtime from `@lightbi/ui`, establishes React 19 host-peer ownership, freezes existing i18n debt monotonically, and records chart/question presentation defects as explicit DPR-owned debt fixtures.
- The i18n baseline is evidence, not a waiver: 7 mixed-language Vietnamese messages, 263 uncataloged English presentation strings and 1 uncataloged Vietnamese presentation string may only decrease; new presentation-language debt fails CI.
- Release-authoritative CI now includes the DPR React-boundary and presentation/i18n debt guards. Final suite PASSed with production build, source-size gate, Intelligence Pack contract and governed product regression `11 files / 41 tests`; focused DPR guards PASS `5/5`.
- Dashboard chart-type collapse remains intentionally OPEN for DPR-6; duplicate Universal/Canonical question lanes remain OPEN for DPR-2. DPR-0 did not mutate governed metrics, evidence semantics, planner authority or current runtime behavior.
- SSD remains healthy at ~68% used / ~40 GiB free; Cargo cache generated by the suite is ~198 MiB. Production/Trust/NEXT runtime were not rotated.
- DPR-0 remains ACTIVE. Next execution is shared design-token/primitive foundation derived from the Books/Frappe study, followed by baseline visual acceptance capture before broad canvas-first screen migration.

## 130. 2026-09-08 DPR-0 elevates information architecture, multi-file workflow convergence and Web Live Demo parity

The owner clarified that the Books-derived visual direction is intentionally clean, but LightBI must support substantially denser BI/BA information without either hiding useful evidence or returning to card-everywhere presentation. The active Decision Presentation/UI/UX plan therefore now carries a product-wide Information Architecture + Density Contract. Its controlling rule is **“More information must create more structure, not more cards.”** One surface owns one primary objective; analytical depth progresses from orientation/answer through evidence and raw/technical detail by reversible states, bounded inspectors or dedicated surfaces rather than by appending every layer into one infinite page. Card remains a bounded-object primitive, not a density mechanism.

The same review confirmed an unresolved multi-file terminal workflow gap. Multi-file governance, Focus Subject, governed summary, Deep BA logic and Step 2 logic are implemented, but `PerspectiveCollectionResultCard` still owns `showDeepDive`, `chartSelection` and `showSubsetDeepDive` locally and can append overview, raw evidence, full-scope Deep BA and selected-data Step 2 into the same scrolling surface. Single-file already models `deepAnalysisView` as mutually exclusive `perspective | selected_data | null` states and opens `InvestigationDeepAnalysis` as a separate reversible surface. Multi-file must converge on that state/surface grammar before broad visual migration: `decision_workspace -> evidence_drill -> deep_perspective | deep_selected`. Full-scope Deep BA and selected-data Step 2 must never render simultaneously at full visual weight; multi-source relationship/grain/period/currency/cardinality/Focus authority remains unchanged.

A visual-baseline capture started during DPR-0 was deliberately stopped before acceptance because reproducing the current giant multi-file scroll would freeze a known information-architecture defect. The corrected order is now: authority/debt freeze -> shared React/design tokens -> multi-file terminal workflow convergence -> corrected visual baseline -> broad canvas-first migration. Product DPR-0 design foundation is already source-closed at `ca454ecab5aa029fb504388374837695e1e2cd11`; no governed numeric/evidence logic changed in that commit.

The owner also elevated Web Live Demo `/app` from “demo” status to a near-1:1 fast product-validation environment. Desktop/Tauri and `/app` must reuse the same React routes, business components, analysis state machines, planners, design tokens, i18n and evidence/chart behavior wherever the platform supports them. Presentation work should validate on `/app` first for fast iteration and then on Desktop/shared source; packaged/native validation remains required for Tauri window chrome, filesystem/open/save permissions, installation-bound Signed Transport/trust and other genuinely native capabilities. Platform differences must use explicit capability adapters or truthful unavailable states, not a second simplified UI or fake native success.

Every DPR phase that changes shared presentation now carries an environment acceptance matrix: Web Live Demo `/app`, Desktop shared surface, and packaged/native when native behavior is involved. Tracked/sanitized sample corpus is the default reproducible test data. Production/Trust/NEXT runtime are unaffected by this planning correction.

## 131. 2026-09-08 DPR-0 closes on corrected multi-file terminal flow and NEXT visual baseline

Product `ce5c961d099bf4d2343a74508211a828daa3df4b` closes the multi-file terminal workflow gap identified in §130 without changing governed metric/evidence authority. `PerspectiveCollectionResultCard` now owns one discriminated presentation state: `decision_workspace | evidence_drill | deep_perspective | deep_selected`. Full-scope Deep BA and selected-data Step 2 no longer coexist at full visual weight; each replaces the prior analytical surface and has a reversible Back path. Evidence Drill preserves exact source separation and selected-source state across Step 2 return.

Focused + integration verification PASSed `17/17`, including the existing multi-source production suite that preserves relationship identity, period partitioning, currency/cardinality truth and fail-closed probes. The complete release-authoritative suite PASSed with production build and governed regression `11 files / 42 tests`. The DPR-0 i18n guard caught four new UI strings during implementation; they were cataloged rather than added to debt or allowlists.

The accepted runtime proof ran only on NEXT's 52xx namespace. A UI-only immutable root `/home/ubuntu/services/lightbi-next-web/dpr0-ce5c961` was built from exact `ce5c961` and attached only to gateway 5273 via the final EnvironmentFile override; engine `current` and Core 5272 were not rotated. Gateway PID changed intentionally, while Core PID `3376963`, Control Plane PID `2346997`, signer `12020`, installation issuer `3817149` and attestation `1505209` remained unchanged. Direct `5273/app` and public `https://lightbi-next.thaiduy.digital/app/` both expose `lightbi-ui-build.json` identifying exact source `ce5c961` and Core generation `g-2026-09-05-next-034`.

A real NEXT `/app` multi-file smoke PASSed `1/1` in 26.6s using the tracked six-file ERP corpus and the live 5272/5274 backends. The corrected visual-baseline spec then PASSed `2/2` in 32.9s and captured nine surfaces: Home, multi-file Understanding, multi-file Decision Workspace, full Deep BA, Evidence Drill, Step 2, Dashboard, single-file Decision Workspace and single-file Deep BA. All nine full-page captures are exactly `1440x900`, so the acceptance states no longer produce document-level giant-scroll at the reference viewport. PNGs remain ignored under `ui-audit/`; Product `14c2d99801cde82716f58127bd006e150118e8c5` source-closes the reproducible spec and SHA-256/bytes/dimensions manifest.

DPR-0 is therefore **CLOSED**. The next declared implementation phase is DPR-1 Claim + Question Provenance foundation. DPR-8 remains the later owner of broad Books-derived canvas-first surface migration after the preceding provenance/question/narrative/visualization/dashboard planner phases. Production is untouched.

## 132. 2026-09-08 DPR-1 claim/question provenance foundation closes on NEXT; DPR-2 becomes active

Product `d027bb34e403968a60f6f3b053e3cfaf9ad42025` closes the DPR-1 presentation-provenance foundation without changing governed metric/formula/join/execution authority. The public claim-basis vocabulary now matches the active plan exactly: `OBSERVED`, `CALCULATED`, `SEMANTICALLY_RESOLVED`, `DOMAIN_CONTEXT`, `INFERRED`, and `HYPOTHESIS`. Each provenance record separately retains evidence references, knowledge references, derivation, limitations, causal status and decision-use restrictions. Micro Brain/domain knowledge is attached only when a presentation derivation actually consumes that context; retrieval/advisory relevance is never promoted to semantic confidence.

A shared non-escalation guard (`capPresentationAuthority`) now encodes the cross-DPR rule that Question, Narrative, Visualization, Dashboard and Report presentation planners may preserve or reduce upstream authority but never strengthen it. The Deep BA projector keeps observed findings descriptive/evidence-bound and hypothesis or needs-verification findings hypothesis-only. The canonical-question projector exposes governed/canonical evidence while explicitly refusing to convert question eligibility/policy into result, execution, decision-use or causal authority.

The shared `EvidenceInspector` implements the DPR-1 progressive-disclosure surface: collapsed by default beside Deep BA findings, with claim basis, causal status, derivation, evidence groups, bounded knowledge context, limitations and decision-use restrictions available on demand. Question UI is deliberately not reworked in DPR-1 because DPR-2 owns merging/deduplicating/ranking the current Universal + Canonical presentation lanes.

Verification is green: DPR-1/MB/Deep-BA/i18n focused tests `25/25`, TypeScript PASS, release-authoritative suite PASS including governed regression `11 files / 42 tests` and production build. Exact Product `d027bb3...` was built into immutable `/home/ubuntu/services/lightbi-next-web/dpr1-d027bb3` and attached only to NEXT gateway 5273. Runtime `/app` provenance acceptance PASSed `1/1` in `8.8s`. Gateway PID rotated intentionally; Core 5272 PID `3376963`, Control Plane 5274 PID `2346997`, signer `12020`, issuer `3817149` and attestation `1505209` remained unchanged. Direct and public NEXT UI identity both report exact `d027bb3...`.

DPR-1 is therefore **CLOSED**. DPR-2 Open-world Question / Perspective Intelligence is active. It is the first phase in this refactor allowed to change question/perspective selection logic by feeding canonical `domainInference` plus bounded MB presentation advice into relevance/answerability/ranking, but the governed metric/question gate remains final and MB cannot make an ungoverned question executable. Production remains untouched.

## 133. 2026-09-08 DPR-2 open-world Question / Perspective Intelligence closes on NEXT; DPR-3 becomes active

Product `a876fb998b980f1eef0c5834822f622a80435b44` closes DPR-2 by moving question/perspective selection from two presentation lanes into one deterministic intelligence projection above the existing governed question/action authority. Domain and Perspective are explicit separate inputs. Canonical `domainInference` and bounded Micro Brain presentation advice may affect relevance, order, domain context, evidence requirements and abstention, but neither may activate official domain support, strengthen action authority, authorize a metric/formula/join, or turn retrieval relevance into semantic confidence.

The planner normalizes governed and Universal candidates, deduplicates semantic siblings, and models answerability as `executable_now`, `descriptive_only`, `needs_more_evidence`, or `unsupported`. Governed action authority is reused only when already granted upstream. Existing safe descriptive actions remain visibly descriptive. Open-world MB/domain suggestions are allowed to create review questions that the current data should consider, but those candidates are forced to `needs_more_evidence`, `actionAuthority=none`, and have no execution button until an upstream governed/safe action exists. Internal retrieval scores and semantic dedupe keys do not escape the public planner contract.

The Understanding surface now has one primary/ranked question path rather than separate Universal and Canonical “Other questions” lanes. Evidence and decision-use restrictions for the primary governed question remain inspectable through progressive disclosure. Domain-context questions live in the collapsed evidence/review surface with explicit advisory-only wording and missing-evidence disclosure. The DPR-0 duplicate-question-lane debt fixture is therefore retired; the unrelated DPR-6 Dashboard chart-type-collapse debt remains open. Neutral wording rules avoid treating high/low numeric position as good/bad by default.

Acceptance is green on exact source. Focused DPR-2/authority/i18n verification PASSed `56/56`; TypeScript PASSed; the release-authoritative suite PASSed with production build, source-size gate and governed regression `11 files / 42 tests`. An immutable UI root `/home/ubuntu/services/lightbi-next-web/dpr2-a876fb9` was attached only to NEXT gateway 5273. Direct and public UI identity report exact `a876fb9...`. Runtime Playwright acceptance PASSed `1/1` in `5.3s`, proving one question lane, visible domain context, open-world MB review questions and zero execution buttons in that advisory-only section. Gateway PID changed intentionally to `3876137`; Core 5272 PID `3376963`, Control Plane 5274 PID `2346997`, signer `12020`, installation issuer `3817149` and attestation `1505209` did not rotate. Production remains untouched.

DPR-2 is **CLOSED**. DPR-3 Analysis Narrative Planner + Deep BA hierarchy is now active. It owns answer-first narrative planning, supporting-analysis relevance, narrative-role prioritization and MB bounded domain-context advice while preserving the DPR-1 provenance/non-escalation contract and the DPR-0 reversible analysis-surface grammar.

## 134. 2026-09-08 DPR-3 Analysis Narrative Planner closes on NEXT; DPR-4 becomes active

Product `804fb2feec8528dc061ce299486ff653d02a92db` closes DPR-3 with one deterministic `lightbi.analysis-narrative-plan.v1` above existing governed Deep BA and multi-source comparison outputs. The plan makes `primary_answer` first, then orders `key_driver`, `risk_exception`, `hypothesis`, `supporting_observation`, `unknown`, `next_action` and `supporting_evidence` sections by deterministic base priority and actual retained evidence. It preserves finding identity and evidence rows and does not change metric execution or numeric truth.

Micro Brain remains bounded presentation advice only. DPR-3 may consume domain/perspective priorities, abstention conditions, evidence requirements, constraints and prohibitions as ordinal narrative priors, but its public policy explicitly forbids MB from strengthening authority and forbids treating retrieval score as confidence. Existing DPR-1 provenance, causal status, decision-use restrictions and evidence inspection remain authoritative.

The shared Deep BA presentation now uses `AnalysisNarrativeBoard` for answer-first hierarchy. Repeated legacy finding/action blocks are suppressed when the narrative plan is available; comparison reason statements are deduplicated; growth/decline/profit driver panels appear only when evidence exists. New DPR-3 presentation code follows the Books-derived `canvas-first, card-by-exception` direction by using typography, whitespace, dividers and restrained grouping instead of adding another nested card layer. Broad shell/surface migration remains owned by DPR-8.

Exact-source acceptance is green. Focused DPR-3/Deep-BA/comparison/MB/i18n verification PASSed `56/56` across `10` test files; `tsc -b && vite build` PASSed with `3807` modules; `git diff --check` PASSed. Immutable `/home/ubuntu/services/lightbi-next-web/dpr3-804fb2f` is live only on NEXT gateway 5273 and direct/public build identity reports exact `804fb2f...`. Browser acceptance traversed the built-in governed sales demo to `/app/investigation`, opened `Analyze deeper`, and rendered `Analysis Narrative`, answer-first guidance, Main Answer, Main observed contributors, Risks & exceptions, supporting evidence/checks and next-check guidance with no page error. A pre-existing account-session CORS warning remains visible on the public NEXT host but is outside the DPR-3 diff and did not prevent the shared product surface from mounting or executing the governed demo.

Gateway 5273 rotated intentionally to PID `4013056`. Core 5272 PID `3376963`, Control Plane 5274 PID `2346997`, signer `12020`, installation issuer `3817149`, attestation `1505209` and user manager `943` remained unchanged; Production was not touched. DPR-3 is therefore **CLOSED**. DPR-4 BA Step 2 investigation model is active and must keep selected-data investigation mutually exclusive with full-scope Deep BA at full visual weight while reusing the new narrative/provenance contracts.

## 135. 2026-09-08 DPR-4 selected-subject investigation closes on NEXT; DPR-5 becomes active

Product `9d1d4a6c1df4a69bf8aa3c0366e7f4eac8fce311` closes DPR-4 by replacing the old “Deep BA on selected rows / Step 2” presentation with one `lightbi.selected-subject-investigation.v1` contract shared by single-file filtered drill-through and multi-file evidence drill. Full-scope Deep BA and selected-subject investigation remain separate reversible states rather than coexisting as full-weight reports.

The selected-subject contract is deliberately non-escalating. It is selected-scope only, preserves source separation, forbids cross-source JOIN authority, keeps governed summary values unchanged, treats benchmark coverage as context rather than authority, and explicitly forbids Micro Brain from strengthening authority. Single-file uses existing selected/matched/source-result/truncation evidence; multi-file no longer assumes matched rows equal selected rows when that is not proven. Bounded/representative evidence is disclosed as such, and Focus remains exact-match fail-closed when a source has no exact binding.

The presentation follows the product-wide information-density rule: one attributed primary answer, benchmark/context as secondary material, parallel source evidence with explicit attribution, concise next actions/questions/unknowns, and detailed source narratives collapsed by default. This adds analytical structure without recreating card soup or exposing every backend layer at equal visual weight.

Verification is green on exact source. DPR-4 integration+i18n PASSed `16/16` across `6` files; Focus/MB/DPR-3 authority regression PASSed `52/52` across `12` files; TypeScript/Vite production build PASSed with `3809` modules; `git diff --check` PASSed; the release-authoritative suite PASSed including governed product regression `11 files / 42 tests`. The committed SHA was rebuilt successfully after push.

Immutable `/home/ubuntu/services/lightbi-next-web/dpr4-9d1d4a6` is live only on NEXT gateway 5273 and identifies exact `9d1d4a6...`. Browser acceptance PASSed both a six-file ERP + Focus `Aqua 250L` parallel-source path and a canonical single-source Revenue drill path; the latter PASSed `1/1` in `19.7s` and reached `Investigate selected evidence -> selected-subject investigation`. Gateway rotated intentionally to PID `15820`; Core 5272 PID `3376963`, Control Plane 5274 PID `2346997`, signer launcher `12020`, issuer launcher `3817149`, attestation launcher `1505209` and user manager `943` remained unchanged. Production was not touched.

DPR-4 is therefore **CLOSED**. DPR-5 Visualization ontology + Chart Pattern Library + Domain Visual Profiles is active. DPR-5 must define intent/pattern suitability, negative rules, semantic color/cardinality/label/axis/tooltip rules and safe fallbacks before DPR-6 may choose renderer types; MB remains bounded advisory knowledge rather than chart or metric authority.

## 136. 2026-09-08 DPR-5 Visualization Ontology closes on NEXT; DPR-6 becomes active

Product `d3770e761ed2a5f08eb50aa6927f4b1dce58bded` closes DPR-5 with `lightbi.visualization-ontology.v1`: exactly 30 semantic chart patterns above renderer primitives. Each pattern declares analytical intents, alternative evidence-role requirements, unit policy, cardinality limits, categorical/sequential/diverging/status color semantics, label/axis/tooltip rules, negative rules and safe pattern fallbacks. The ontology deliberately has no `ChartType` or renderer binding, so DPR-5 does not choose or persist a renderer.

`lightbi.visualization-suitability.v1` provides the deterministic eligibility boundary needed by the next phase. It rejects intent mismatch, missing evidence roles, excessive cardinality, incompatible or unnormalized units and status coloring without explicit desirability. High/low numeric position does not imply good/bad. Risky patterns are evidence-gated: donut requires a valid denominator and low cardinality; radar requires a normalized common scale; map requires geographic evidence; Sankey requires genuine source/target/flow roles; process control requires control-limit evidence; scatter preserves association-not-causation wording.

`lightbi.domain-visual-profile.v1` projects bundled Micro Brain domain presentation advice into canonical pattern IDs while retaining `authority=advisory_only`. Every currently published MB chart family and analytical intent is covered; an unknown future family remains an explicit unmapped gap rather than becoming a fabricated pattern. MB cannot authorize metrics, formulas, joins or renderer selection and retrieval rank remains advisory relevance rather than confidence.

The reusable `lightbi.visualization-acceptance-cases.v1` corpus now binds the plan's representative cases to semantic expectations: revenue ranking, ordered revenue trend, actual-versus-target, low/high-cardinality composition, cost-versus-revenue relationship, fresh waste-rate neutrality and context-sensitive inventory coverage. These cases are renderer-free by design so DPR-6 and DPR-10 can reuse them.

Verification is green. Focused semantic tests PASSed `19/19`; focused + i18n PASSed `22/22`; MB presentation + provenance integration PASSed `30/30`; source-size gate PASSed with `537` production modules; exact committed TypeScript/Vite rebuild PASSed with `3809` modules. The release-authoritative suite PASSed including governed product regression `11 files / 42 tests`. The i18n guard initially caught 29 new pattern labels, which were cataloged in Vietnamese rather than allowlisted; the monotonic debt guard remains intact.

Immutable `/home/ubuntu/services/lightbi-next-web/dpr5-d3770e7` is live only on NEXT gateway 5273 and identifies exact `d3770e7...`. Gateway rotated `15820 -> 170768`; Core 5272 stayed `3376963`, Control Plane 5274 stayed `2346997`, user manager `943` and Trust launchers `12020/3817149/1505209` remained unchanged. Web Live Demo regression acceptance traversed `/app -> canonical single source -> Revenue -> governed analysis -> /investigation` with zero page errors. The known account-session CORS console warning remains pre-existing and outside DPR-5. Desktop shared-source behavior is covered by the same TypeScript/build/test source; packaged native acceptance is not applicable because no native capability changed.

DPR-5 is therefore **CLOSED**. DPR-6 Visualization Planner + renderer/type preservation is active. The existing `dashboard-renderer-type-collapse` fixture remains explicitly OPEN under DPR-6; DPR-5 did not modify `DecisionVisualizationPlan` renderer choice, `ChartPreviewModel` mapping, `DashboardBuilder` collapse logic or persisted `ChartType`.

## 137. 2026-09-08 DPR-6 Visualization Planner and renderer/type preservation close on NEXT; DPR-7 becomes active

Product `18e267a0f0b248d40536f6c6f003cd8d2a099fa9` closes DPR-6 by introducing `lightbi.visualization-plan.v1` above renderer primitives, a versioned `lightbi.visualization-renderer-registry.v1`, and `lightbi.decision-visualization-plan.v2`. Analytical intent is resolved to a canonical DPR-5 chart pattern first, then deterministic suitability decides whether that pattern is allowed, and only then may the renderer registry bind a renderer family. Unsupported capability falls back to a safer pattern/table or abstains; renderer convenience cannot override semantic suitability.

Micro Brain/domain presentation advice remains bounded and non-authoritative. It may reorder otherwise eligible candidates but cannot convert a deterministic suitability failure into PASS, choose or persist `ChartType`, strengthen evidence, authorize metrics/formulas/joins or turn retrieval relevance into confidence. Single-file Investigation uses its actual `RuntimeIntent.type` plus canonical primary-domain context; multi-file period logic distinguishes ordered trend from one-period snapshot rather than inferring Line merely from row count.

The chart-type collapse debt is now retired with executable end-to-end proof. `Scatter` is a real persisted `ChartType`; Investigation persistence retains the two measure axes and the v2 visualization-plan metadata, `DashboardBuilder` resolves `Scatter` instead of falling through to Bar, and `DashboardChartWidget` emits the corresponding ECharts scatter series. Analysis workbook export preserves Decision Plan identity, canonical pattern, renderer family and pattern rules in the overview while governed metric rows/evidence remain unchanged. Session identity also revalidates the current Decision Plan before reusing export authority.

Verification is green. Broad DPR-6 integration PASSed `74/74` across `18` files; focused post-retirement/extraction checks PASSed `22/22`; TypeScript PASSed. The production source-size gate caught `Investigation.tsx` at `1018` lines during implementation; the planner adapter was extracted into a pure helper instead of weakening the gate, leaving `Investigation.tsx` at `981` lines and the final gate PASSing `540` production modules. A clean release-authoritative rerun PASSed i18n/debt guards, a `3815`-module production build and governed product regression `11 files / 43 tests`, ending in `release_1_0_suite=passed`. Exact committed-SHA rebuild after push also PASSed with `3815` modules.

Immutable `/home/ubuntu/services/lightbi-next-web/dpr6-18e267a` is live only on NEXT gateway 5273 and identifies exact `18e267a...`. Gateway rotated intentionally `170768 -> 303041`; Core 5272 remains PID `3376963`, Control Plane 5274 remains PID `2346997`, user manager remains `943`, and signer/issuer/attestation launchers remain `12020/3817149/1505209`. Chromium ARM browser acceptance PASSed `2/2` in `44.4s`: canonical single-source Revenue through governed chart/drill to selected-subject investigation, and six-file ERP + Focus `Aqua 250L` through parallel selected-subject investigation. Desktop shared-source behavior is covered by the same React source/build/tests; packaged native acceptance is not applicable because DPR-6 changes no native capability. Production was not touched.

DPR-6 is therefore **CLOSED**. DPR-7 Dashboard Composition Planner is active. It owns semantic Dashboard roles/story order, duplication and information-budget checks, rejectable MB Dashboard priors, and removal of hard-coded Bar breakdown generation as the default while preserving governed metric/evidence and DPR-6 visualization-plan authority. Broad layout/canvas migration remains DPR-8 work.

## 138. 2026-09-08 DPR-7 Dashboard Composition Planner closes on NEXT; DPR-8 becomes active

Product `324eceb91329fbff5a26fb70d6840d00f3483483` closes DPR-7 with `lightbi.dashboard-composition-plan.v1`, a deterministic semantic composition layer above the already-governed DPR-3 narrative and DPR-6 visualization plans. Dashboard candidates now carry canonical semantic role, derived story stage, distinct management question, evidence references, decision importance, placement intent, inclusion reason and optional visualization-plan identity. Membership is decided before any chart is materialized, so rejected duplicates or over-budget candidates cannot pollute Chart Library or the Dashboard store.

The planner enforces evidence-backed membership, management-question deduplication, analytical-shape deduplication, comparative metric/grain compatibility and caller-supplied information budgets. The current automated executive writers use a bounded relevance band of at most four metrics plus four visuals, but that is a writer acceptance policy rather than a global DPR-8 layout quota. Semantic roles and insertion order are retained specifically so the next phase can turn the same analytical story into a responsive canvas without re-deriving meaning from `ChartType`.

Micro Brain remains advisory. `dashboardRoles` from domain/perspective/dashboard narrative knowledge are projected as open-string ordering priors only after deterministic membership has been fixed; MB cannot add/remove admitted evidence, strengthen authority, choose/persist renderer types or convert retrieval relevance into confidence. The composition contract retains decision perspective and domain context. Current automated writers have no upstream authoritative audience value, so `audience` is deliberately `null/unavailable` instead of inventing CEO/manager semantics.

Both automated Dashboard writers now consume the composition plan. Single-file `createInvestigationChartActions()` and multi-file `PerspectiveCollectionResultCard` create only admitted chart artifacts and add them in plan story order. Narrative metadata is retained only when its narrative candidate survives the same composition gates because the current Dashboard renderer exposes that metadata as visible executive context. Supporting breakdowns no longer default to a hard-coded Bar; they obtain a DPR-6 `DecisionVisualizationPlan v2` for ranking and persist the renderer type selected by the governed renderer registry. Governed metrics, source evidence, Focus separation and multi-source relationship truth are unchanged.

Verification is green. Focused DPR-7 integration PASSed `19/19`; broad presentation/MB/Focus/renderer regression PASSed `27` files / `116` tests; TypeScript, `git diff --check` and source-size (`543` production modules) PASSed. The clean release-authoritative suite PASSed with a `3818`-module production build and governed regression `11` files / `44` tests, ending in `release_1_0_suite=passed`; the exact committed SHA rebuilt successfully after push.

Immutable `/home/ubuntu/services/lightbi-next-web/dpr7-324eceb` is live only on NEXT gateway 5273 and identifies exact `324eceb...`. Gateway rotated `303041 -> 471868`; Core `3376963`, Control Plane `2346997`, user manager `943` and Trust container/parent PIDs remained unchanged. Chromium ARM browser acceptance PASSed `2/2` in `34.7s`: tracked six-file multi-source -> Decision Workspace -> governed Dashboard and tracked single-file inventory -> Deep BA -> governed Dashboard. Both surfaces remained within the current 1–8 materialized-widget relevance band and produced no page errors. Desktop shared-source behavior is covered by the same source/build/tests; packaged native is not applicable because no native capability changed. Production remains untouched.

DPR-7 is therefore **CLOSED**. DPR-8 Canvas-first surface refactor + shared UI system is active. It owns the broad layout change: apply the Information Architecture + Density Contract, shared LightBI tokens/primitives, Books/Frappe-derived hierarchy principles, card-by-exception rules and Desktop/Web `/app` parity while consuming—not re-inventing—the semantic contracts already established by DPR-1 through DPR-7.

## 139. 2026-09-08 DPR-8 shared canvas foundation is live; Understanding migration is next

Product `e030e4ddfc2997b8abe0ca75d5fe2541b8d9d5f7` completes the first implementation chunk of DPR-8 without closing the phase. `@lightbi/ui` now provides canvas-first `Canvas`, `Section`, `SectionHeader`, `Inset` and `Divider` primitives plus explicit `summary`, `working` and `evidence` density classes. A universal Card primitive is intentionally absent so spacing and hierarchy do not regress into card-everywhere composition.

The owner-approved navigation shell now consumes LightBI design tokens directly: desktop expanded sidebar geometry is the DPR-0 target `224px`, compact geometry remains `56px`, and header/canvas/ink use shared tokens. Tailwind shared-package scanning is narrowed to package root plus shared component sources, eliminating recursive `node_modules` scanning while still compiling shared TSX primitives. No route/state/native-capability behavior changed.

Verification is green. The new DPR-8 contract PASSed `3/3`; TypeScript and `git diff --check` PASSed; source-size PASSed `545` production modules. The release-authoritative suite PASSed with a `3818`-module production build and governed regression `11 files / 44 tests`, ending in `release_1_0_suite=passed`; exact committed-SHA rebuild also PASSed.

Immutable `/home/ubuntu/services/lightbi-next-web/dpr8-foundation-e030e4d` is live only on NEXT gateway 5273. Gateway rotated `471868 -> 558013`; Core `3376963`, Control Plane `2346997`, user manager `943` and Trust container/parent PIDs remained unchanged. Browser shell acceptance PASSed `1/1`: 224px desktop expanded, 56px collapsed, 56px compact at 720px viewport, zero page errors. Production remains untouched.

DPR-8 remains **ACTIVE**. The foundation does not claim that Home/Understanding/Decision Workspace/Deep BA/selected-subject have already been visually migrated. The next implementation chunk is Understanding: preserve existing semantic/governed output and evidence reachability while replacing equal-weight card composition with the plan's canvas-first information hierarchy and ranked/progressive disclosure structure.

## 140. 2026-09-08 DPR-8 Understanding meets above-fold acceptance on NEXT; Home / New Brief is next

Product `900d7aa93d59b2c58501ba6efe49390182a157be` completes the bounded Understanding target inside DPR-8; DPR-8 itself remains active. The final surface keeps the shared canvas hierarchy and governed analytical behavior from the earlier migration while tightening information density so the decision action appears in the normal desktop viewport. Meaning is compact source/grain/domain context, perspectives are 40px ranked rows, Focus is an inline optional control, and readiness/domain/key-signal detail is progressively disclosed under Evidence rather than competing with the primary answer.

The predecessor `524747c...` is intentionally recorded as an intermediate, not an accepted closure. Its functional browser checks passed, but a direct reading of section 17 required the primary Analyze CTA to be visible without scroll. Live measurement at 1440x900 put the CTA bottom at about `1690.75px`; the docs closure draft was therefore reverted. The final `900d7aa...` follow-up measures the same CTA at `852–896px` with `scrollY=0`, satisfying the acceptance contract without weakening it.

Verification is green: DPR-8 contract `4/4`; Understanding `8/8`; focused Understanding+i18n `11/11`; TypeScript/diff-check PASS; source-size `545` modules; release-authoritative build `3822` modules and governed regression `11 files / 44 tests`, marker `release_1_0_suite=passed`; exact-SHA rebuild `3822` PASS. No metric/formula/join/planner/evidence/MB authority changed.

Immutable `/home/ubuntu/services/lightbi-next-web/dpr8-understanding-900d7aa` is live only on NEXT gateway 5273. Gateway rotated `638133 -> 729433`; Core `3376963`, Control Plane `2346997`, manager `943`, signer `12020`, issuer `3817149` and attestation `1505209` stayed unchanged. NetBird and external `/app` return HTTP 200. Final Chromium acceptance shows four ordered Understanding regions, four 40px vertical perspectives, three vertical secondary questions, collapsed semantic+technical evidence, visible primary analysis, CTA `bottom=896px` inside the 900px viewport, shell `224px/56px`, and `pageerror=[]`. Production remains untouched.

DPR-8 remains **ACTIVE**. Home / New Brief is the first remaining unmigrated bounded target in section 17; Decision Workspace, Deep BA and BA Step 2 follow as later DPR-8 presentation targets.

## 141. 2026-09-08 DPR-8 Home / New Brief is flat and live; Decision Workspace is next

Product `eeaf4464da4d28306edf4f4daa095655a94c0a21` completes the bounded Home / New Brief presentation target inside DPR-8; DPR-8 remains active. The question prompt stays the primary command surface and quick suggestions remain compact pills/actions. The three source-entry cards are replaced by one vertical source-action list, while Session History becomes a divider/list section with inline empty/retry states instead of a card containing nested cards. Intake handlers, saved-session open/delete/retry behavior and governed analysis semantics are unchanged.

Verification is green: Home history functional `2/2`; i18n `3/3`; DPR-8 contract `5/5`; TypeScript/diff-check PASS; source-size `545` modules; release-authoritative build `3822` modules and governed regression `11 files / 44 tests`, marker `release_1_0_suite=passed`; exact-SHA rebuild `3822` PASS. `HomeWorkspaceView.tsx` drops from `999` to `987` lines rather than crossing the 1000-line hard gate.

Immutable `/home/ubuntu/services/lightbi-next-web/dpr8-home-eeaf446` is live only on NEXT gateway 5273. Gateway rotated `729433 -> 804962`; Core `3376963`, Control Plane `2346997`, manager `943`, signer `12020`, issuer `3817149` and attestation `1505209` stayed unchanged; NetBird/external `/app` return HTTP 200 and Production remains untouched. Live Chromium acceptance at 1440x900 shows one 80px command surface, compact suggestions, three vertical source rows (~66px each), a real six-session flat history list, no horizontal overflow; 720px compact also has no horizontal overflow and `pageerror=[]`.

DPR-8 remains **ACTIVE**. Per plan section 17, Decision Workspace is the next bounded target; Deep BA and BA Step 2 follow. The refactor must remain presentation-only and consume DPR-1..DPR-7 authority rather than re-deriving it.

## 142. 2026-09-08 DPR-8 Decision Workspace is answer-first and live; Deep BA is next

Product `73506f0ef8f1a656d592df6031399106a7de3ffe` completes the bounded Decision Workspace presentation target inside DPR-8; DPR-8 remains active. Single-file and multi-file analysis now follow the same answer-first hierarchy instead of leading with charts: Main answer, authoritative key number when available, primary visual, explanation, supporting evidence/visuals, next action, then collapsed Evidence/technical detail. This is a presentation migration only; existing governed questions, metrics, visualization plans, Focus/source separation, Dashboard authority and evidence contracts remain upstream.

Single-file BA content is split into answer, explanation and next-action regions while retaining the legacy canonical result test identities inside the new hierarchy. A key number is intentionally absent when the canonical full-scope total has not been proven; the inventory acceptance fixture exercised this fail-closed case rather than fabricating a KPI. Multi-file uses the already-derived Key attention observation and largest movement, keeps the primary visual bounded, moves explanation before supporting metrics, and preserves the existing `decision_workspace -> evidence_drill -> deep_perspective | deep_selected` reversible state grammar. Focus supporting context is flattened into divider/list structure while retaining the same narrative and exact-match authority.

Verification is green: DPR-8 contract `6/6`; focused checks `16/16`; `Investigation.test.tsx` `19/19`; broad regression `14 files / 64 tests`; TypeScript/diff-check and source-size (`545` modules) PASS. Release-authoritative PASSed build `3822` + governed regression `11 files / 45 tests`, marker `release_1_0_suite=passed`; exact-SHA rebuild PASSed. Immutable NEXT root `/home/ubuntu/services/lightbi-next-web/dpr8-decision-73506f0` is live on gateway 5273. Gateway rotated `804962 -> 961459`; Core `3376963`, CP `2346997`, manager `943` and Trust container/parent PIDs were unchanged; Production was untouched.

Live Chromium acceptance PASSed both tracked lanes with no page errors or horizontal overflow. Six-file multi-source preserved the complete answer-first chain and evidence-drill Back round-trip. Sanitized single-file inventory preserved the available answer-first regions while correctly omitting an unproven key number; Evidence remained collapsed by default. DPR-8 remains **ACTIVE**. Per section 17, Deep BA is next; BA Step 2 follows.

## 143. 2026-09-08 DPR-8 Deep BA is a management document and live; BA Step 2 is next

Product `d7ea5ce8d024145df612c7bdea44701bf10e4129` completes the bounded Deep BA presentation target inside DPR-8; DPR-8 remains active. Single-file and multi-file full-scope Deep BA now reduce chrome as analytical depth increases: numbered document sections, typography, whitespace, dividers and progressive disclosure replace equal-weight nested cards. Existing narrative plans, provenance, governed metrics, Focus/source separation, comparison truth and export authority are unchanged.

Single-file `AnalysisNarrativeBoard` and `SingleSourceBAOverviewCard` preserve the DPR-3 answer-first section membership while rendering it vertically as a management document. Focus full-scope analysis follows the same reduced-chrome rhythm, while legacy population/Business Fusion evidence is retained behind supporting disclosures. Multi-file `deep_perspective` keeps the established reversible state grammar and renders the existing comparison brief as numbered sections `01–05`; supporting context/reasons remain collapsed by default. Selected-subject / BA Step 2 is intentionally outside this chunk.
...
## 145. 2026-09-09 DPR-9 semantic report pagination is source/live; exact-live single artifact closure remains pending

Product `aa511844bb7b2f9fbfb5b575bc726fef293e9952` implements the active DPR-9 Report Page Model without changing analytical authority. `lightbi.analysis-report-plan.v1` provides one deterministic semantic page model shared by single-file and multi-file Deep BA/selected-subject export. Existing DPR-8 screen sections are mapped to executive summary, answer/performance, drivers/components, explanation/status, recommendations/risks and evidence appendix roles. Keep-together and semantic break rules preserve reading order; pagination governance explicitly cannot strengthen metric, formula, join, causal or evidence authority.

The old giant-DOM-image PDF technique has been removed from both Deep BA exporters. The shared exporter captures top-level semantic sections, temporarily expands report-marked details and restores their exact prior open/closed state, composes bounded A4 page canvases, writes one PNG when one page is sufficient or a ZIP of numbered PNG pages for longer reports, and writes each semantic page as a real jsPDF page. Candidate browser artifact proof produced four bounded PNG/PDF pages for the canonical six-file multi-source Deep BA and five pages for the tracked MB-6 Healthcare single-file Deep BA.

Verification is release-green: pure report-plan `4/4`, focused report/Deep-BA/Step-2/multi `20/20`, TypeScript PASS, DPR-8 + DPR-9 executable contracts `11/11`, source-size PASS over `547` production modules, production build `3824`, governed regression `11 files / 45 tests`, final `release_1_0_suite=passed`, and exact committed-SHA rebuild `3824` PASS. The i18n gate initially rejected two new PNG Save-dialog descriptions; both were cataloged rather than allowlisted and the full suite was rerun from the beginning.

Immutable `/home/ubuntu/services/lightbi-next-web/dpr9-aa51184` is live only on NEXT gateway 5273 and identifies exact Product `aa511844...`. Gateway rotated `1211805 -> 1461645`; Core 5272 remains `3376963`, Control Plane 5274 remains `2346997`, user manager remains `943`, and current Trust container init PIDs `1505255/3817195/13384/13437` were unchanged by the UI-only rotate. Direct and external NEXT `/app` return HTTP 200; Production and 51xx were untouched.
...
## 147. 2026-09-09 DPR-10 web/shared-source acceptance is release-green; native Windows visual UAT remains

Product `8ed2370809a5a4ddc413277a3d1eb7956eaa5020` adds only DPR-10 acceptance/release gates; no production feature or runtime behavior changes. The release-authoritative suite now runs a cross-domain presentation matrix plus a byte-deterministic Micro Brain presentation rebuild contract. Six current catalog domains preserve question relevance, governed rows, metric IDs/source refs and semantic visualization patterns; deterministic suitability can reject an MB chart prior without numeric drift.

Micro Brain acceptance is green and bounded. The presentation rebuild reproduces byte SHA-256 `a74cad36a714ad3a296cef4f902ad409b447e4fee978ddc6a9b062903e65f041`; presentation index size is `3,360,712` bytes and foundation+presentation total `9,966,179` bytes, below the `20 MiB` ceiling. The presentation corpus remains `88` cards, including `32` domain profiles and `31` chart-pattern cards. MB stays `advisory_only`, with prohibition/abstention recall and no metric/formula/join/causal authority.

Final release-authoritative verification PASSed DPR-10 cross-domain `50/50`, deterministic rebuild `1/1`, i18n, source-size `547`, production build `3824`, governed regression `11 files / 45 tests`, and `release_1_0_suite=passed`. Visual acceptance explicitly retains neutral/context-dependent semantics for Fresh waste and inventory coverage and enforces a bounded Dashboard information budget rather than decorative chart diversity.
External NEXT live acceptance uses the unchanged runtime Product `aa511844...` because `8ed2370...` is test-only. Canonical six-file + Focus `Aqua 250L` PASSed `1/1` in `52s`: governed key/supporting metrics are identical before/after Focus, exact-source evidence remains scoped, selected-subject Step 2 opens correctly and `pageerror=[]`. Tracked Healthcare PASSed `1/1` in `8.3s`: Deep BA and selected-subject Step 2 retain evidence-bound inferred-domain authority, `Not production-active`, no metric/formula escalation, report-plan v1, no horizontal overflow and no page errors.

Additional inferred-domain live probes PASSed `3/3`: Manufacturing and Agriculture resolve evidence-bound; Hospitality safely remains `Domain unresolved / Unknown or ambiguous`. This is correct because the semantic lobe currently lacks a Hospitality bridge even though the isolated presentation lobe has Hospitality chart/domain advice. Presentation knowledge must not be promoted into semantic or official-support authority merely to make the UI resolve a domain.

DPR-10 remains **ACTIVE**. Web Live Demo/shared-source acceptance is green, but the plan still requires packaged Windows/native visual UAT before stable promotion. No Production/51xx/Core/Control Plane/Trust service was changed by this acceptance work.

## 148. 2026-09-09 DPR-10 native-UAT convergence is exact-SHA packaged; owner visual acceptance remains

Product `4848c0cc5d22ef6b14236e823ab4ef497cb75e0f` implements the bounded owner UAT convergence batch recorded in the canonical DPR-10 plan at docs commit `d67d70f671d9d1fd22fc2cfab1bcc04ac51ef94b`. The batch does not reopen analytical authority: it restores the owner-selected pre-optimized LightBI mark, removes residual card/pill stacks from Settings/Connection/Updates/privacy surfaces, integrates the sidebar account row, separates Bell from brand chrome, normalizes shared reading gutters, presents secondary questions in a responsive two-column grid, and makes single-file plus multi-file Deep BA / selected-subject Step 2 available as an independently scrollable right dock on wide desktop while preserving the primary governed workspace. Narrow layouts retain the existing bounded overlay/fallback behavior.

Brand rollback is exact rather than cosmetic. `apps/desktop/public/branding/lightbi-icon.svg` and `favicon.svg` both use SHA-256 `47220c4ae55ec2fc9abb6d621cb1c2c3f638c956620144ceb5a922de5b8889a5`, matching the owner-requested pre-`fde259a` source at Product commit `40ca2d8...`. Native Tauri derivatives were regenerated from that mark, and the NSIS header/sidebar assets were restored to SHA-256 `512c50e...` / `18a54a7...`. The existing LF pinning remains in force so Windows checkout cannot mutate byte-sensitive SVG authority.

Verification is release-green. Native icon/capability validation PASSed; the new DPR-10 native-UAT convergence contract PASSed `5/5`; DPR-8 PASSed `8/8`; DPR-9 PASSed `3/3`; DPR-10 cross-domain PASSed `52/52`; the deterministic MB rebuild PASSed `1/1`; focused component+i18n checks PASSed `15/15`. The complete release-authoritative suite PASSed source-size over `548` production modules, a `3828`-module production build and governed regression `11 files / 45 tests`, ending in `release_1_0_suite=passed`.

The exact web candidate is live only on NEXT as immutable `/home/ubuntu/services/lightbi-next-web/dpr10-4848c0c`. Gateway 5273 rotated `1461645 -> 3057143`; Core 5272 stayed PID `3376963`, Control Plane 5274 stayed PID `2346997`, and all four Trust container identities remained unchanged. Direct and external `/app` both return HTTP 200. Production and 51xx were not touched.

Windows Native Acceptance run `34319524815` (run #12) completed SUCCESS on exact head `4848c0cc5d22ef6b14236e823ab4ef497cb75e0f`. Artifact `LightBI-Windows-R1P12-Native-Acceptance` has ZIP SHA-256 `aa3a645f6fd701a81204cd22ca685a8a76e88244a8994775e68a5645e8e94990`. Its installer `LightBI-NEXT-0.9.2-next.r1p12.12-x64-setup.exe` is `32,670,062` bytes with independently rechecked SHA-256 `78af8a79495e2c160321cd532182ff23cc7b2f7ce3cd3438aa91ecae0327b128`. Provenance reports generation `g-actions-r1p12-34319524815-1`, `publisher_status=NotSigned` and `production_authority=false`; this is intentionally an internal acceptance artifact and is not stable release authority.

DPR-10 remains **ACTIVE**. The remaining gate is owner native visual UAT on this exact installer: restored branding including the installer wizard, Bell/header alignment, Settings convergence, sidebar/footer integration, balanced gutters, two-column questions, single/multi right-docked Deep BA and Step 2, report pagination, and Account trust hot-recovery. If `installation_issuer_release_not_allowed` reproduces on this exact artifact, it becomes a real Trust defect and must not be hidden. Stable promotion and any Production/51xx action remain owner-gated.

## 149. 2026-09-09 Windows run #12 fails owner UAT; DPR-10 re-baselines to whole-product coherence + native shell restoration

Exact Windows Native Acceptance run `34319524815` remains valid reproducible package evidence for Product `4848c0cc5d22ef6b14236e823ab4ef497cb75e0f`, but owner native UAT **FAILED** and therefore supersedes the earlier “artifact ready / UAT pending” interpretation. The run #12 workflow itself succeeded; the failure is product acceptance, not packaging provenance. DPR-10 remains ACTIVE and run #12 must never be relabeled as accepted.

The most important newly proven functional defect is installation trust. The exact package still reports `installation_issuer_release_not_allowed`. Source/history verification shows that the native client derives an installation release identity from its channel + packaged version and the installation issuer correctly rejects identities outside its signed REL authority. Existing Trust evidence already distinguishes unknown release identity (`release_not_allowed`) from allowlisted-but-unsigned (`release_not_signed`). Therefore the successor fix is an orchestration contract: exact Product/package identity -> immutable TEST REL binding exact installer digest -> existing signing/verification authority -> issuer refresh/reload -> challenge/issue/native probe. The issuer allowlist must not be weakened and the error must not be hidden. Production Trust authority is unchanged.

Native shell acceptance also regressed. `crates/lightbi-tauri/src/desktop_menu.rs` and `main.rs` still own the real Tauri `File / Edit / View / Help` menu and event routing, but the Windows config at run #12 uses `decorations:false` and the frontend supplies `NativeWindowTitleBar`. The owner requires the real native menu back. The new canonical contract restores Windows native decorations/menu and retires duplicate custom Windows titlebar controls; Web `/app` must not imitate native chrome.

The owner also rejected automatic right-docking of Deep BA / BA Step 2. Analysis state and presentation state must be independent: Deep BA and selected-subject Step 2 open in the primary/full experience by default; only an explicit user side-panel toggle moves the same analysis into a right panel on a sufficiently wide desktop. Closing the panel returns the same analysis to primary mode without changing analytical/evidence authority. This rule applies equally to single-file and multi-file paths.

Session History is now governed as visibility + recovery + execution + pagination rather than one ambiguous “restore” action. Multi-file sessions must remain visible when persisted metadata is valid, while governed execution continues to fail closed until exact source/relationship identity can be reconstructed. The UI displays six sessions per page and at most five pages (newest 30 displayed) without silently deleting older durable session/source records merely to meet presentation density.

The visual remainder is now treated as a codebase-wide coherence program rather than screenshot-local polish. Static inventory plus targeted source reads found residual card/round/shadow language across Home, Settings, Charts, DashboardBuilder, Datasets, multi-source review, dataset understanding, drill-through, database/intake and display-preference surfaces. The migration classifies shell, document/list, command/editor, semantic inset, chart/widget and true interactive containment separately; cards are not banned universally, but unexplained nested cards/giant rounded sheets are. Frappe Books remains the external density/divider study source and Advanced remains an internal reference for squarer dense editor composition.

Chart work is also re-baselined around the existing system rather than adding another library. LightBI already uses ECharts and has a richer canonical visualization ontology / MB chart-advice corpus than its materialized renderer registry. The successor must build an explicit renderer-capability matrix, expand suitable ECharts families for tracked/common analytical shapes, add non-authoritative chart templates mapped to canonical pattern IDs, and replace the repetitive first-indigo-series appearance with semantic palette rules. MB remains advisory and chart diversity is not a goal by itself. Dashboard composition keeps DPR-7 membership/story authority and uses the same visualization plan/capability matrix so legitimate trend/ranking/share/relationship/variance roles can materialize differently without a diversity quota.

The canonical execution and acceptance program is now [`§23B of the Decision Presentation + UI/UX Refactor plan`](../history/agent/plans/AGENT_IMPLEMENTATION_PLAN_DECISION_PRESENTATION_UI_UX_REFACTOR_2026-09-04.md#23b-dpr-10-whole-product-coherence--native-shell-restoration-program--2026-09-09). It runs five staged gates: native shell + Trust; history + explicit side-panel interaction; shared UI primitives + whole-codebase migration; chart renderer/template/palette + Dashboard materialization; then Web/exact-native release acceptance. No new owner installer is handed off until the exact signed installation-trust probe and full machine gates pass. Production/51xx remains untouched and stable promotion remains separately owner-gated.

## 150. 2026-09-10 DPR-10 Gate D chart intelligence is source-green; Gate E final Web/native acceptance is active

The post-run-#12 whole-product program in canonical plan §23B has progressed through Gate A–D source implementation. Final Product source candidate is `b9827129f296bd1d99e4efdd1b63667479a3feda`. Native Windows returns to real OS/Tauri chrome and installed-runtime provenance; History and Deep BA/Step 2 use the accepted visibility/presentation model; the broad UI migration and later multi-file refinement remove the rejected auto-dock/narrow-layout behavior while restoring per-source selected-subject filters without cross-source joins.

Visualization capability is now global rather than bounded to official domain support. The canonical 30-pattern library is shared by three presentation lanes: `official_domain_prior` for the six official domains, `inferred_domain_advice` for evidence-bound open-world domains, and shape-only fallback when domain is unresolved. Domain/MB advice can rank presentation candidates but cannot create metric/formula/join/evidence/official-support authority. ECharts renderer coverage, semantic palette behavior, rich tooltips and Dashboard pattern persistence were expanded; one primary plus at most two supporting charts remains the bounded presentation target. Geospatial Map remains truthful renderer-pending rather than being faked by another family.

Final source-candidate verification ended `release_1_0_suite=passed`, with production build `3828` modules and governed regression `11 files / 46 tests`. Gate-D live NEXT evidence on Product `6be3422...` proves rich Revenue hover values and Healthcare as semantic inference with `Official support: Not production-active`, no confidence percentage and no overflow. The latest Product `b982712...` is not yet live: Gateway 5273 still serves immutable `dpr10-gated-6be3422`.

Gate E Trust groundwork is also active on NEXT. Control Plane 5274 was deliberately rotated to immutable `435e38a5bba2d7cecf44399d2ac2dffc8eb0e30b` (`feat(trust): gate native runtime readiness`) after its distribution suite passed `305/305`. The trust-ready flow binds the exact installed Windows runtime to an internal signed TEST REL, verifies issuer hot reload, certificate/attestation behavior and issuer/signer process continuity, and must stay `productionAuthority=false`. The final Product workflow/RC/eSigner references now pin this exact CP authority.

DPR-10 is **not closed**. Gate E must rebuild `/app` from exact `b982712...`, run final browser acceptance, build one exact Windows artifact, verify installed-runtime provenance, publish/verify the exact signed TEST REL, prove Account/native trust recovery and native File/Edit/View/Help, then hand one installer to the owner. Owner UAT is the closing authority; Production/51xx and stable promotion remain separately owner-gated. Operational continuation is in [`AGENT_HANDOFF_DPR10_GATE_E_FINAL_ACCEPTANCE_2026-09-10.md`](../history/agent/handoffs/AGENT_HANDOFF_DPR10_GATE_E_FINAL_ACCEPTANCE_2026-09-10.md).
## 151. 2026-09-10 DPR-10 owner defect convergence closes final Web Gate E; native E3 is active

Exact Product `aa45aee12e4afc926c386106564deb6922d8b7c5` is the current DPR-10 candidate and matches remote. It supersedes `b982712...` and the intermediate `12dd81bb...` candidate after owner Web/native review exposed six bounded issues: multi-file renderer materialization lagged the 30-pattern planner; the three BA follow-up questions reused one answer brief; Online/Database intake rendered behind an effectively invisible animated panel; Focus semantic type could conflict with the governed question dimension; report export lacked corporate cover/footer/about structure and could orphan content; and some runtime intent mappings underused the visualization library. The fixes preserve source separation and evidence authority: MB/domain knowledge remains advisory only, no cross-source measure join is authorized, and Focus compatibility is evaluated generically by semantic axis/type rather than by hard-coded sample values.

The final modal defect was proven by exact-live browser acceptance: the panel had `initial opacity:0` but its Framer Motion target omitted `opacity:1`. Commit `aa45aee1` restores the visible target and adds a regression. The complete release-authoritative run then PASSed with build `3828`, governed regression `11 files / 49 tests`, DPR/i18n/public-boundary/Trust/routing gates green, source-size `552`, and marker `release_1_0_suite=passed`.

Immutable NEXT `/home/ubuntu/services/lightbi-next-web/dpr10-ownerfix-aa45aee1` is live on Gateway 5273 PID `1714808`. Core 5272 remains PID `3376963`; trust-ready CP 5274 remains PID `1116289` at `435e38a5bba2d7cecf44399d2ac2dffc8eb0e30b`; Trust container identities remain `6f8d49919877/ba78fa6bb2ce/c58c90a4604b/cb6395655a5d`. Direct/public `/app` are HTTP 200 and Production/51xx is untouched.

Final exact-live Web acceptance is green: Chart Library `3/3`; Product Focus -> Category semantic gate `1/1`; multi-file Profitability follow-up contract `1/1` with three distinct Gross Profit answers; canonical six-file Deep BA/source-local Step 2 `1/1`; Online + Database intake modal `1/1`; and report export `1/1`. The exported artifact is six `2830x4002` A4 PNG pages plus a six-page PDF with cover/Executive Summary, document reference, version/time/source footer, numbered pages, Evidence Appendix without a one-line orphan, and a final LightBI information/disclaimer page.

DPR-10 remains **ACTIVE**. Web Gate E is accepted for `aa45aee1`, but native E3/E4 remains: exact Windows package -> installed-runtime provenance -> signed internal TEST REL -> issuer/certificate/attestation/process-continuity proof -> Account/native-menu smoke -> one owner installer. Runs #12 and #13 are superseded candidate provenance only. Stable/Production promotion remains separately owner-gated.


## 152. 2026-09-10 Visual Narrative Composition is live on NEXT; owner Web UAT is next

Product `6ba98d9d39a95adc5f0ddd9a5aaa48c63cea2220` replaces the fixed chart-set mentality with `lightbi.visual-narrative-composition.v1`. Single-file presentation is anchored by the selected question; one strong chart is sufficient, while extra visuals are admitted only when they add an evidence-backed story role. Safe same-source/same-grain aggregate layers may combine into one grouped or bar+line visual; duplicate/repetitive or unrelated executable charts are rejected. Visible composition normalizes to balanced `1/3/5` units with a stable full-width primary anchor and dynamic height, eliminating orphan half-width cells and fixed low-information chart height.

The independent audit's MB wiring defects are addressed at the presentation boundary. Real selected perspective now reaches primary/support planners and persisted decision identity; domain/perspective aliases improve exact MB retrieval; `domain_profile`, `perspective_profile` and `chart_pattern` advice retain retrieval rank/provenance instead of being flattened. MB can affect ordering/membership only inside the deterministic evidence/suitability-eligible set. It still cannot authorize metrics, formulas, joins, causal inference, renderer capability or official domain support. A regression proves the same governed evidence can yield different eligible presentation membership for different user perspectives without changing primary/evidence authority.

Official Domain Visual Playbooks V2 provide six official domains with story-role, intent, combination and rejection priors. Multi-file exactly-two-period movement now resolves as comparison rather than automatic trend. Generated Single/Multi Dashboards materialize the same narrative membership and 20-column dynamic layout grammar rather than independently reviving rejected charts.

Exact source verification is green: Visual Narrative aggregate `17 files / 92 tests`; exact-commit release-authoritative build `3832` modules; governed `11 files / 50 tests`; marker `release_1_0_suite=passed`. Exact Product `6ba98d9d...` is now live from immutable root `/home/ubuntu/services/lightbi-next-web/dpr10-visual-narrative-6ba98d9d` on Gateway 5273 PID `2436036`; Core `3376963`, CP `1116289` / `435e38a...` and Trust identities are unchanged. Final Chromium acceptance ends `LIVE_UAT=PASS`: single Revenue uses one 220px answer visual and no irrelevant support; its Dashboard is four KPI widgets plus a full-width hero; canonical six-file two-period Executive Overview is comparison at 230px and its Dashboard packs a complete 7/7/6 KPI row plus full-width hero. Revenue versus Operations on the same Sales source resolves different questions and preserves governed versus safe-descriptive authority without forcing decorative chart diversity. Owner Web UAT on this exact candidate is the next gate; Windows run #14 is superseded. DPR-10 remains ACTIVE and Production/51xx remains untouched. Canonical implementation detail is §23B.15 of the Decision Presentation/UI-UX plan.
## 153. 2026-09-11 Visual Narrative semantic remediation is source-green; immutable NEXT Web UAT is next

Product `1ad549ac4a1b7a2b256ad7fe7df261d1a1647762` closes the audit-driven §23B.16 VN-R1..R10 remediation at source and private-browser level without changing the established authority model. The selected question and governed semantic answer remain primary; deterministic result-shape/suitability and complementarity decide eligibility before MB/domain advisory reranking. MB remains `advisory_only`; retrieval does not become confidence and cannot authorize metrics, formulas, joins, causal claims, renderer capability or official domain support.

Fresh exact-candidate evidence is green: canonical Single `126/126` across the tracked six-file ERP corpus with zero fail/timeout/overflow/pageerror; synthetic `14/14` oracle-compliant with one deliberate Inventory Value abstention and zero oracle violation; Multi R10 `34/34` across seven file combinations with seven Data Trust surfaces, zero fail/overflow/pageerror and distinct follow-up labels/answers. The one genuine Multi defect found after harness repair was Accounting-only Profitability: source evidence already contained governed gross-profit capability, but the collection builder incorrectly required Sales+Accounting pairing. The fix uses independent Accounting `gross_profit` period partitions and preserves `no_cross_period_row_join`; cross-source Sales↔Accounting relationship governance is unchanged when Sales exists.

Final verification PASSed the modified regression aggregate `11 files / 110 tests`, i18n coverage/registry, hard-code/private-port scan, `git diff --check`, and root `pnpm test:release-1.0`: source-size `557` production modules, production build `3832`, governed regression `11 files / 50 tests`, final marker `release_1_0_suite=passed`. Product is committed/pushed exact at `1ad549ac4a1b7a2b256ad7fe7df261d1a1647762`. Public NEXT 5273 still serves `6ba98d9d...`; therefore owner Web UAT is **not yet accepted** for this remediation. Next authority step is immutable NEXT deployment of `1ad549ac4a1b7a2b256ad7fe7df261d1a1647762`, exact-live representative Single/synthetic/Multi checks, then owner Web UAT. Native E3/E4 stays blocked until that acceptance. Production/51xx remains untouched.

Exact immutable NEXT deployment is now complete. `/home/ubuntu/services/lightbi-next-web/dpr10-visual-remediation-1ad549ac` serves Product `1ad549ac4a1b7a2b256ad7fe7df261d1a1647762` on Gateway 5273 PID `800669`; the UI-only rotate left Core `3376963`, CP `1116289` / `435e38a...`, and Trust containers `6f8d49919877/ba78fa6bb2ce/c58c90a4604b/cb6395655a5d` unchanged. Public/direct `/app` are HTTP 200 and public build provenance reports the exact Product SHA while retaining the unchanged NEXT-034 Core generation. Exact-live browser evidence PASSed Single Revenue + Dashboard, full-six comparison + Dashboard, Revenue-vs-Operations perspective separation, Accounting pair `4/4`, full-six R10 `7/7`, S08 truthful Inventory Value abstention and S09 Actual/Target `target_combo` controls, with no accepted overflow/pageerror or duplicate Multi follow-up answers. The broad public synthetic replay is not counted as a full live oracle gate because its legacy execution-button waiter can race hidden/visible controls; the authoritative exact-worktree synthetic benchmark remains `14/14`. Owner Web UAT is still pending and remains the closing Web authority. Native E3/E4 and Production/51xx remain blocked/untouched.

## 154. 2026-09-11 Owner Web UAT rejects `1ad549ac` chart behavior; Chart Presentation Plane rebuild is active

Owner review of exact live NEXT Product `1ad549ac4a1b7a2b256ad7fe7df261d1a1647762` rejects chart/presentation acceptance despite the green §23B.16 machine evidence. The new evidence is owner-visible and structural: repeated questions still converge heavily on line/bar, 1/3/5 behaves effectively as one chart for many real questions, compound/combo stories rarely materialize, ranked horizontal bars use an inconsistent pale palette, chart drill/BA-Step-2 affordance disappeared, and Display Options has no curated chart-palette presets. Therefore §23B.16 automated closure does not authorize native progression.

Git/source tracing establishes why prior audits could correctly report that MB understands chart/domain semantics while Product output still ignored that knowledge. `6be3422f` created `OFFICIAL_DOMAIN_CHART_SETS_V1` and a domain-trio selector but Product runtime never consumed that module. `6ba98d9d` introduced a second domain-playbook/narrative path and initially allowed official domain complements into composition. `1ad549ac` then hardened duplicate/complementarity rules and removed that official-complement admission path, effectively requiring metric overlap or legal combination for many supports. `storyOrder` has no runtime consumer, MB enters after a bounded upstream support pool is formed, visualization planning selects the first eligible pattern inside an already-fixed analytical intent, and combo discovery occurs only after compatible independent analyses happen to materialize. Unit/module tests proved idealized candidate behavior rather than end-to-end actuation.

Canonical plan §23B.17 now supersedes the failed chart-decision architecture while preserving §23B.16 as historical evidence. Scope is strictly Chart Presentation Plane: Understanding/Data semantics, metric/dimension identity, canonical evidence, governed totals, formula/SQL/aggregation/join authority, source separation and MB semantic recovery are frozen. The corrected authority model gives deterministic governance hard veto over illegal/unsafe presentations, while Official Domain Policy + MB receive material voting power among legal evidence-backed alternatives. MB may request presentation companion analyses from a read-only capability inventory, influence primary/support roles, combination proposals, story order and target `1|3|5`, but may never invent or strengthen data truth.

The rebuild unifies the split domain chart-set/playbook knowledge into one runtime-consumed Official Domain Presentation Policy; introduces pre/post-execution MB/domain ballots with rejection trace; plans candidate requests and combos before the support pool closes; changes 1/3/5 from accidental normalization into an evidence-justified story target with explicit degradation; restores coherent palettes and click-to-filter/BA-Step-2 UX; and adds source-driven E2E gates for domain-policy actuation, MB actuation, 1/3/5, combos, semantic freeze, palette and exact-live browser behavior. Native E3/E4 remains blocked until an exact immutable NEXT candidate passes these gates and receives explicit owner Web acceptance. Production/51xx remains untouched.
