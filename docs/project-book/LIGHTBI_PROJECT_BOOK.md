...
Owner screenshots independently show the Windows UI running NEXT033 with parent NEXT032, product `6c1f117`, CP `3afb85b`, healthy Core/CP/worker, Isolation OK and Intelligence Pack `1.0.0-next.1` active. Those screenshots are visual identity evidence only: at capture time Build Identity still showed artifact/REL `Not evaluated` and installation certificate `unavailable`, because the open application instance predated the new REL publication. Internal native startup invokes `ensureNativeInstallationTrust` once, fire-and-forget, so a close/reopen is required to retry. Until the owner-side process proves that the installed `LightBI.exe` is the canonical SHA, receives/persists the OS-vault-backed installation identity and exercises packaged V2 `native_http_request` with no WebView downgrade, packaged Windows/native UAT remains open.
