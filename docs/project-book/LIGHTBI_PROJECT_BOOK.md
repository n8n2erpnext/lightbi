...
Current implementation status on 2026-09-04: **MB-5** remains completed at product commit `8a4a5e4` (`feat(understanding): separate inferred domain support`), and the accepted successor branch is now `codex/r1-roadmap-integration` at `a8d55ee`. That successor preserves the MB-5 behavior and adds an independent Signed Transport proof primitive without changing semantic authority. MB-5 adds a versioned domain-inference artifact, keeps official support owned by the governed manifest/runtime gate, and renders inferred-domain/support state separately in Understanding. The production build and Playwright healthcare evidence from the MB-5 checkpoint remain valid; no percentage/similarity is exposed as semantic confidence. Production services and production domain/metric authority remain untouched. **MB-6** remains the primary next phase: propagate `domainInference`, official support, analysis mode, authorization and limitations through Focus / Deep BA / BA Step 2 without status loss.
...
The owner directed the previously isolated Focus/Micro-Brain work back into one Road-to-1.0 execution stream. The product worktree has therefore moved from the experiment-named branch to successor branch `codex/r1-roadmap-integration`, currently at `a8d55ee` (`feat(trust): add signed request proof primitive`). The prior MB-5 authority remains commit `8a4a5e4`; the branch rename/integration does not reclassify inferred semantics or official domain support.

The current **critical path remains MB-6 → MB-7**. MB-6 must carry `domainInference`, `officialSupport`, `analysisMode`, metric/formula authorization, limitations and evidence references through Focus, Deep BA and BA Step 2 while preserving identical governed factual values. MB-7 remains the corpus/counterfactual/performance/release-authoritative acceptance and cutover gate.

In parallel, a deliberately bounded Signed Transport foundation has been checkpointed without turning networking/security work into the product critical path. Product commit `a8d55ee` adds a Rust canonicalization/body-digest/Ed25519 request-proof primitive compatible with the existing `lightbi.next-attestation-request.v1` rehearsal contract. It is not exposed as an arbitrary frontend signing command and is not yet wired into `native_http_request`. An isolated Rust harness passed 3/3 golden-vector tests against the authoritative JavaScript canonicalization behavior.
