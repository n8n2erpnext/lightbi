...
## 59. Visual Narrative semantic remediation owners and same-role profitability execution

Product `1ad549ac4a1b7a2b256ad7fe7df261d1a1647762` keeps one visualization-intelligence pipeline rather than adding a Multi-specific engine. `question-engine-primary.ts`, `question-engine-secondary.ts`, `question-engine-shared.ts`, `signal-engine.ts`, `ontology.ts` and `next-adapter.ts` own semantic recognition/question binding; `question-perspective-intelligence.ts` owns perspective-aware candidate intelligence; `visualization-planner.ts`, `domain-visual-playbooks.ts`, `investigation-visualization-plan.ts` and `visual-narrative-composition.ts` own deterministic suitability, domain/MB advisory ranking, complementarity and final 1/3/5 presentation normalization. `ChartPreviewRenderer.tsx` applies bounded Top-N presentation shaping and localizes its disclosure without changing governed rows.

Multi collection execution remains in `home-canonical-multisource-build.ts`. Sales and Logistics same-role comparisons continue through `CanonicalPeriodPartitionWorkspace`; this remediation extends the same governed primitive to Accounting `gross_profit` when a selected collection has Accounting evidence but no Sales source. Each Accounting file executes independently and only metric results are combined by reporting period. `canonical-source-candidate-projection.ts` exposes `gross_profit` in period-comparison capability only when direct Accounting gross-profit evidence spans multiple periods. When Sales is present, Profitability retains the pre-existing Sales↔Accounting relationship-gated `buildCanonicalMultiSourceDataset`/`executeCanonicalMultiSourceMetric` path. No raw cross-period or unrelated-source join is introduced.

`safe-sql-preview.ts` + `local-duckdb-executor.ts` own adaptive temporal shaping/provenance; `decision-visualization-plan.ts` and `visualization-ontology.ts` keep renderer/pattern semantics separate from authority. The release proof for this ownership is the §23B.16 closure: Single `126/126`, synthetic `14/14` oracle-compliant, Multi `34/34`, modified aggregate `110/110`, and authoritative release suite `release_1_0_suite=passed`. Public NEXT deployment and owner acceptance are operational gates, not code-ownership changes.
