...
## 23. 2026-09-03 R1-P12 Windows Native Acceptance #6 is green but not Authenticode signed

`Native Acceptance Artifact` run `33696477945` / #6 completed successfully at exact public source `13202fd7136b134f05768a797f3c5dd12d717889`. The uploaded `LightBI-Windows-R1P12-Native-Acceptance` artifact is GitHub artifact `9873149821` with archive digest `sha256:5562bafc43ceefa4f654f5721dccaf119b9353d7355e7a86d8ffc0413aff2a70`. Its acceptance manifest binds installer `LightBI-NEXT-0.9.2-next.r1p12.6-x64-setup.exe` to SHA-256 `398760459a69520f9b6fa3ec80d7007dc6c8989d64e17764ac66fe2810887cd7` and generation `g-actions-r1p12-33696477945-1`.

The accompanying native publisher sidecar reports `signature_status=NotSigned`, `signer_subject=null`, `signer_thumbprint=null`; the acceptance manifest records `production_authority=false`. The lane therefore proves package/build/evidence integrity while failing closed on real Windows publisher trust. It must not publish an Official/stable identity state from these bytes. Owner UAT accepted the bounded install/sidebar/external-link shell; Bell dropdown is deferred non-blocking UX debt. R1-P12 remains open on Authenticode plus the owner-gated Production trust/verifier chain, and R1-P13 remains closed. This CI/UAT evidence is independent of the live NEXT service-generation drift: Core/gateway presents NEXT-030, while CP diagnostics/worker identity still reports NEXT-029; that NEXT-only runtime reconciliation remains open.

## 24. 2026-09-03 post-acceptance NEXT runtime reconciliation does not change signing authority

After Windows Native Acceptance #6, the NEXT application runtime was reconciled so the immutable NEXT-030 manifest, live Distribution CP diagnostics and worker heartbeat all identify private CP `bb50b0d53542da5cd908e2237cbca368f7f87073`. The private exact-head suite is **209/209**, schema remains 067 with 21 migrations applied / 0 pending, and the public R1-P12 source `13202fd...` re-passes the complete release-1.0 suite including release contract 8/8 and governed regression 39/39.

This runtime repair changes no artifact-publisher authority. GitHub Actions #6 remains the current native acceptance evidence and its exact installer remains `NotSigned`. The Windows workflows collect and validate Authenticode evidence but contain no configured real certificate/signing step. R1-P12 cannot claim platform-signing closure until final signed bytes produce native `Valid` evidence bound to the exact artifact digest and expected publisher subject; Production trust/release authority remains separately owner-gated.

## 25. 2026-09-03 R1-P12 NEXT SSL.com eSigner lane is armed, exact-source gated and non-publishing

Public source `901dda8fee8d386ed0779c3f4b8ebd5a6d7d8105` adds `.github/workflows/windows-next-esigner-signing.yml` plus ephemeral SSL.com eSigner CKA prepare/cleanup helpers. The lane does not run on the ordinary R1-P12 branch and does not use `workflow_dispatch`, because this workflow is intentionally absent from `main`. It is activated only by a deliberately created branch named `codex/native-signing-r1p12-sandbox-<exact-40-sha>` or `codex/native-signing-r1p12-product-<exact-40-sha>`, and the suffix must equal `GITHUB_SHA` exactly.

Signing authority remains bounded to NEXT acceptance. Workflow permissions are `contents: read`; there is no GitHub Release action, R2 credential, R2 upload or stable-manifest publication. The emitted acceptance record always states `environment=next_internal_test_only`, `promotable_to_production=false` and `production_authority=false`. A GitHub Actions query at wiring head `901dda8...` returned zero runs, proving the source push did not contact eSigner or produce signed bytes.

After CKA loads the selected code-signing certificate, its thumbprint is supplied to both Tauri Windows signing and compile-time LightBI publisher verification. Tauri signs the application executable and NSIS bundle; post-build PowerShell collects `Get-AuthenticodeSignature` evidence and SHA-256 for both. Product mode requires `Valid` status, exact expected signer Subject, equal application/installer signer thumbprints, and equality with the thumbprint compiled into the LightBI runtime. Sandbox mode may prove provider connectivity/signature creation but is explicitly non-authoritative for public Windows publisher trust.

Static/source gates pass at `901dda8...`: YAML parse, `git diff --check`, release contract **11/11**, public/private boundary, generation/routing **8/8**, and full release-1.0 suite with governed regression **39/39** plus desktop production build. Provider execution remains unproven until real SSL.com credentials exist on a Windows runner. R1-P12 platform signing is therefore wired but not complete; Production authority remains a separate owner-gated phase.

## 26. 2026-09-03 R1-P12 LightBI Root/REL/ATT acceptance #8 closes NEXT anti-impersonation machine gate

Owner clarification supersedes the earlier interpretation that Authenticode itself was the R1-P12 identity signature. OS publisher evidence remains a separate future platform-hardening plane. Current Official LightBI origin is governed by the LightBI Root → REL → exact artifact → ATT chain and independent verification; the Beta/RC CI system must continue distinguishing package integrity, LightBI cryptographic identity and optional OS publisher evidence.

`Native Acceptance Artifact` run `33713352231` / #8 completed SUCCESS from exact source `148f27a8db743f75bd4fe0c06ceee0461fb14cb5`. GitHub artifact `9878574548` has archive digest `sha256:092a90d5e5ae537c1cebc580b41f6ed6ee9cdb3610fdc6375091959abe3783d5`. Independent artifact inspection confirms installer `LightBI-NEXT-0.9.2-next.r1p12.8-x64-setup.exe`, byte size `29781164`, SHA-256 `1b55ee32690f01ffb7cbaedfbd2bba22c50222c049b4902356504f858eed8151`, exactly matching generated acceptance provenance.

Those exact bytes are now represented by TEST REL `release:0.9.2-next.r1p12.8:windows:x86_64` beneath authority `next-test-20260902-031220`, with `promotableToProduction=false`. Live HTTPS verification through `/internal-trust/` proves Root/keyset/REL identity and the exact digest. ATT rehearsal accepts the REL-bound release and rejects replay/tamper/sequence/target negatives; a valid TEST ATT signature referencing an unlisted fork release is independently rejected as `attestation_release_not_allowed`.

Public source `68e5bc159fe0054fd4ab08083dbeb4fa84817847` adds the missing NEXT gateway route for `/internal-trust/`; immutable overlay `gateway-overlay-68e5bc1` is live and `/verify` plus trust JSON resolve through NEXT-030. Exact current-head release verification passes release contract 11/11, generation/routing 8/8 and governed regression 39/39. The existing OS publisher sidecar remains `NotSigned`, accurately documenting that future trust plane without preventing R1-P12 closure.

R1-P13 is authorized to begin with a dedicated fail-closed RC lane on NEXT. The current manifest contract has channels `beta | stable` and accepts SemVer prereleases, so RC should use `1.0.0-rc.N` without inventing a third channel. RC must remain non-Production and non-stable-publication authority until the later explicit gates are satisfied. Production remains untouched and Phase 2A freeze remains a separate owner decision.
