...
## 2026-09-11 — Visual Narrative semantic remediation source/browser closure

- Repaired the Multi R10 browser harness so each perspective owns an isolated page/context and Data Trust is validated against its actual trust surface rather than `collection-decision-workspace`. The previously contaminated matrix was discarded as Product evidence.
- The repaired matrix exposed one real contract gap: Accounting May/June Profitability was advertised ready but the collection builder only executed gross profit through Sales+Accounting pairs. Product now executes governed Accounting `gross_profit` as same-role independent period partitions (`no_cross_period_row_join`) while preserving existing relationship-gated Sales↔Accounting execution when Sales is present.
- Fresh browser evidence on the exact candidate is green: Single `126/126` (`0` fail/timeout/overflow/pageerror; only two expected governed-delivery count no-visual answers), synthetic `14/14` oracle-compliant (`13` executable + one truthful Inventory Value abstention; `0` oracle violation), Multi `34/34` across seven combinations (`7/7` Data Trust surfaces, `0` fail/overflow/pageerror, no duplicate follow-up labels/answers).
- Broad modified regression PASSed `11 files / 110 tests`. The first release-authoritative run correctly failed the i18n debt guard on newly introduced visible English copy; the copy was cataloged and the Top-N presentation note was moved through the runtime localization boundary rather than allowlisted. Language coverage `3/3` and registry `9/9` then PASSed.
- Final root `pnpm test:release-1.0` PASSed source-size `557`, production build `3832`, governed regression `11 files / 50 tests`, marker `release_1_0_suite=passed`. Hard-code/private-port scan and `git diff --check` were clean.
- Product commit `1ad549ac4a1b7a2b256ad7fe7df261d1a1647762` (`fix(dpr10): harden visual narrative semantics`) is pushed on `codex/dpr0-contract-freeze`. Public NEXT remains on predecessor `6ba98d9d...`; immutable deployment + exact-live representative reruns + owner Web UAT are next. Native E3/E4 and Production/51xx remain blocked/untouched.

## 2026-09-11 — Visual Narrative remediation immutable NEXT acceptance; owner Web UAT pending

- Rebuilt exact clean Product `1ad549ac4a1b7a2b256ad7fe7df261d1a1647762` (`3832` modules), staged immutable `/home/ubuntu/services/lightbi-next-web/dpr10-visual-remediation-1ad549ac`, and rotated only Gateway 5273 `2436036 -> 800669`. Core `3376963`, CP `1116289`, and Trust containers `6f8d49919877/ba78fa6bb2ce/c58c90a4604b/cb6395655a5d` did not rotate; Production/51xx was not restarted or modified.
- Direct/public `/app` return HTTP 200 and `lightbi-ui-build.json` identifies exact UI source `1ad549ac...`, NEXT-034 and CP `435e38a...`.
- Exact-live Visual Narrative/Dashboard acceptance PASSed Single Revenue, full-six two-period comparison, both packed Dashboards and Revenue-vs-Operations perspective separation. Multi R10 representative live probes PASSed Accounting pair `4/4` and full-six `7/7`, with Data Trust on its own surface, distinct analytical follow-ups, zero overflow and zero page errors.
- Representative synthetic live safety PASSed S08 truthful abstention (Inventory Value/Exposure not exposed; no Unit Price/Quantity/record-count surrogate) and S09A/S09B Actual/Target `target_combo`. The older broad public synthetic harness showed execution-control race on some cases and is diagnostic only; source acceptance continues to use the clean exact-worktree `14/14` oracle benchmark.
- Owner Web UAT on the exact immutable candidate is now the remaining Web gate. Native E3/E4 stays blocked until owner acceptance.

## 2026-09-11 — Owner Web UAT FAIL; chart-only architecture rebuild planned

- Owner review of exact live NEXT `1ad549ac...` rejected chart acceptance despite green release/browser gates: real questions still converge heavily on line/bar, 1/3/5 collapses mostly to one visual, compound/combo behavior is rare, ranked horizontal bars have an inconsistent pale palette, chart drill/BA-Step-2 affordance is missing, and no user-selectable chart palettes exist.
- Root-cause Git/source audit traced the repeated failure to wiring/authority rather than MB knowledge: `6be3422f` created official domain chart sets with no Product runtime consumer; `6ba98d9d` created a second domain-playbook path; `1ad549ac` hardened complementarity and removed official-domain complement admission. MB arrives after a bounded upstream support pool, `storyOrder` has no runtime consumer, first-eligible intent planning favors line/bar, and combo is discovered only after compatible analyses already happen to exist.
- Canonical plan §23B.17 now defines `Chart Presentation Plane Rebuild`. Understanding/Data plane is frozen: no metric/dimension rebinding, formula/SQL/aggregation/join/source/evidence/answerability changes are authorized.
- New authority direction: deterministic governance is the hard veto for illegal presentation; Official Domain Policy + MB have material voting power among legal evidence-backed choices. MB may request presentation companion analyses from existing capabilities, vote primary/support/combo/story order/1|3|5, but cannot invent or strengthen data truth.
- Required gates include semantic-freeze parity, official-domain runtime actuation, controlled MB actuation, role-aware candidate-request actuation, source-driven 1/3/5 and combo oracles, palette consistency/presets, chart click -> selected-evidence -> BA Step 2, broad six-domain E2E and exact immutable NEXT owner UAT. Native E3/E4 remains blocked; Production/51xx remains untouched.
