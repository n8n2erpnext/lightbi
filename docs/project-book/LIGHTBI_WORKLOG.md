...
- Security/regression gates PASS: Node 15/15, Rust 5/5 + Clippy, signer 6/6, attestation/P10 12/12, Distribution 193/193. Actual TEST bearer-token and TEST Root PEM bytes are absent from CLI source/build/binary; dependency review contains no web/HTTP/async/crypto authority stack.
...
## 2026-09-02 — R1-P11 NEXT private Pro delivery rehearsal passes after runtime-image hardening

- Resumed the preserved P11 work on `codex/next031-pro-delivery-rehearsal`; final source `deda7c284a6eafaa8cb69d491b96476a025ed15c`. P10 now issues one-time 60-second delivery grants bound to package ID plus X25519 delivery public key; `trust-pro-delivery` is a separate verification/delivery runtime with no signer credentials.
- Private Pro publication uses TEST PRO signing, AES-256-GCM package encryption and a random content key. Delivery consumes the grant, verifies the PRO chain/ciphertext, then wraps the content key with X25519 + HKDF-SHA256 + AES-256-GCM to the bound device. Correct-device decrypt PASS; wrong-device unwrap FAIL; consumed-grant replay FAIL.
- Live full chain PASS: real AccountAuth session + ATT/device proof + signed ENT + grant + signed PRO + encrypted package. Device/delivery private keys are ephemeral. Synthetic account and `p11:` entitlement rows return to zero; audit event `trust.next_pro_delivery_rehearsal.completed` remains with `productionAuthority=false`.
- Physical-separation probes find no private Pro marker or actual package content key in the Basic engine, CP, release or public-Trust trees. Package store is mode 0700 with mode-0600 ciphertext/content-key/record files; delivery/attestation journals contain no marker, content key or private-key PEM.
- Initial flattened-image scan found the unnecessary `publisher-cli.js` in the delivery runtime image. Although it contained no actual content key/plaintext package, it carried the rehearsal private marker literal. Hardening commit `deda7c2...` changed the image build to runtime-only `contracts-loader.js`, `delivery.js`, `server.js` and added a regression. Rebuilt image `sha256:d176985adbf35ef30275d15d53660be5d512c28ca02613c5009ca09e8c5ecfda` scans clean.
- Final exact-head gates: delivery 9/9, attestation/Pro 14/14, Distribution 197/197. Runtime remains UDS 0600, `network=none`, rootfs RO, cap-drop ALL, no-new-privileges and no signer secret. Production is untouched; P11 is NEXT rehearsal PASS only. Next: audit R1-P12 anti-impersonation closure without faking OS publisher signing.

## 2026-09-02 — R1-P12 NEXT anti-impersonation software closure verified; platform signing blocks phase exit

- Public Core `codex/next033-r1p12-identity` finalized at `079c344952749d7a01c830a1b88c1d4247a1f5cf` after fail-closed identity base `304d4b48b2f410fe623afecef86c9ab35a8352f7`. SHA-only updater semantics are now `integrity checked`, not `verified`; Production external-verifier linking is explicit-HTTPS-only and otherwise unavailable.
- Private Distribution verifier `codex/next033-r1p12-anti-impersonation` is clean/pushed at `85fa6a6961ba2bb00e2d09d92e6a9f5f815a3478`. Exact suite **204/204 PASS**; live NEXT `/verify` is TEST-only and independently verifies TEST Root→keyset→REL/ATT evidence without claiming Production identity.
...
## 2026-09-03 — R1-P12 NEXT Windows eSigner Authenticode lane wired; no signing run executed

- Public Core source advanced to `901dda8fee8d386ed0779c3f4b8ebd5a6d7d8105` with a NEXT-only SSL.com eSigner CKA signing lane. The ordinary native-acceptance workflow also now pins current reconciled CP `bb50b0d...` instead of the old NEXT-029 `6936fc4...` identity.
- The signing workflow is deliberately non-publishing and exact-source gated. It runs only on `codex/native-signing-r1p12-sandbox-<exact-40-sha>` or `codex/native-signing-r1p12-product-<exact-40-sha>` and rejects any branch suffix that does not equal `GITHUB_SHA`. Current branch push produced zero GitHub Actions runs.
- eSigner CKA preparation verifies the provider installer Authenticode, loads the certificate ephemerally with username/password/TOTP, exports the certificate thumbprint to the build, and cleans runner master-key/material afterward. No provider secret exists in source.
- The lane signs both the LightBI application executable and NSIS installer through Tauri Windows signing. This is required because the runtime `windows_publisher` verifier checks `std::env::current_exe()` and requires the observed signer thumbprint to equal the compile-time expected thumbprint. Product acceptance requires both EXE and installer `Valid`, exact signer Subject, matching signer thumbprints and exact SHA-256 evidence; sandbox cannot close public trust.
- Verification after wiring: release contract **11/11**, public/private boundary PASS, generation/routing **8/8**, full release-1.0 suite PASS, governed regression **39/39**, desktop build PASS. No real Windows signing run has occurred, so `windowsAuthenticode=false` remains correct.
- NEXT-030 live runtime remains immutable Core `7d15d69...`, gateway overlay `6bb55ab...`, CP/worker `bb50b0d...` healthy. Source head `901dda8...` is not mislabeled as the deployed generation. Production/main/role rotation were not touched; Production Phase 2A/ATT/REL/external-verifier authority remains owner-gated and R1-P13 remains closed.

## 2026-09-03 — Owner restores LightBI Root signing definition; R1-P12 NEXT closes and R1-P13 opens

- Owner clarified that the roadmap's current app-signing requirement is LightBI anti-impersonation authority, not Microsoft/Apple vendor credentials. Canonical Trust contract was corrected at docs commit `511a2d947495d8092846cffaf8e2268446ed28c2`: `official_verified` derives from Root→REL + exact artifact + valid ATT; OS publisher signing remains future hardening.
- Public source progressed through `a8e3db5` identity-state correction, `148f27a` exact installer-size provenance and `68e5bc1` NEXT `/internal-trust/` gateway routing. New immutable gateway overlay `gateway-overlay-68e5bc1` is live on NEXT-030; `/verify`, `/internal-trust/latest.json` and immutable REL JSON return successfully. Production untouched.
- GitHub Native Acceptance #8, run `33713352231`, SUCCESS. Artifact `9878574548`; exact installer `0.9.2-next.r1p12.8`, 29,781,164 bytes, SHA-256 `1b55ee32690f01ffb7cbaedfbd2bba22c50222c049b4902356504f858eed8151`. Independent extraction matches acceptance provenance. `NotSigned` is retained only as OS-publisher evidence.
- TEST REL publisher signed the exact #8 name/SHA/size as `release:0.9.2-next.r1p12.8:windows:x86_64`, authority `next-test-20260902-031220`, non-promotable. Live public verifier proves Root `next-test-root-2026-01` → REL `next-test-rel-2026-01` → exact release/artifact SHA.
- ATT E2E on #8 PASS: valid sequences 1/2 accepted; nonce replay, body tamper, sequence replay and wrong-target rejected. A certificate carrying a legitimate TEST ATT signature but forged release `release:fork-impostor-r1p12.8:windows:x86_64` is rejected HTTP 401 `attestation_release_not_allowed`.
