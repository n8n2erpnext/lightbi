...
Signer administration is **CLI-only**. The owner-approved `cli-lightbi` operator surface follows the Frappe Bench mental model: one predictable host-side entrypoint over a complex bounded system, with the existing Rust/Ratatui TUI for operator visibility. The hardened signer container remains the cryptographic appliance; no signer web admin, public signer listener, generic signing command or signer shell is permitted. Phase A remains read-only. If lifecycle/mutation phases are separately authorized later, they must stay purpose-specific, step-up protected and audited through `cli-lightbi` without moving Root/issuer private material into the CLI.
...
OS publisher signing is a separate, future platform-hardening trust plane. REL signing does not replace Windows Authenticode or macOS Developer ID/notarization, but those vendor/platform credentials are not part of the current LightBI cryptographic definition of official origin and do not block the current R1-P12 anti-impersonation closure. A later platform-release policy may require them for SmartScreen/Gatekeeper/store UX without changing LightBI Root authority.
...
| --- | --- | --- |
| `official_verified` | allowed LightBI Root → REL chain + exact artifact digest + valid ATT installation state | **Official LightBI — verified** |
...
R1-P11 has now been exercised end to end on the isolated NEXT trust domain at final private source `deda7c284a6eafaa8cb69d491b96476a025ed15c`. The P10 conjunction issues a short-lived one-time delivery grant bound to package identity and an ephemeral X25519 delivery public key. A separate `trust-pro-delivery` runtime has no signer credential or signing authority; it verifies the Root-signed PRO manifest and serves only an AES-256-GCM encrypted private package while wrapping the random content key to the attested device with X25519 + HKDF-SHA256 + AES-256-GCM. Live AccountAuth→ATT→signed ENT→grant→signed PRO delivery passes, while a different device key cannot unwrap the package and grant replay fails closed. Synthetic account/entitlement authority state is removed after rehearsal. Public Basic/runtime/release/Trust surfaces contain neither the private Pro marker nor the actual package content key. An image-hygiene probe found the publisher CLI unnecessarily present in the first delivery image; the runtime image builder was hardened to stage only `contracts-loader.js`, `delivery.js`, and `server.js`, after which a flattened-image scan found zero private-marker/content-key hits. Final delivery image is `lightbi-next-trust-pro-delivery:deda7c284a6e-trust-10de4da8`, image ID `sha256:d176985adbf35ef30275d15d53660be5d512c28ca02613c5009ca09e8c5ecfda`, with `network=none`, read-only rootfs, no published port and read-only Trust/private-package/attestation mounts. Exact-head gates pass delivery **9/9**, attestation/Pro **14/14**, and private Distribution **197/197**. This is **R1-P11 NEXT rehearsal PASS only**: its delivery/encryption/wrapping protocol remains non-production rehearsal evidence and Production P11 still depends on the frozen Production Root/signer/REL/ATT/ENT chain.


R1-P12 has now reached **NEXT software-closure verification** without crossing its Production exit gate. Public Core candidate `079c344952749d7a01c830a1b88c1d4247a1f5cf` (following fail-closed identity base `304d4b48b2f410fe623afecef86c9ab35a8352f7`) removes SHA-only updater language/metadata that could be misread as official verification, renames native staging to `prepare_integrity_checked_update`, and makes the Settings verifier link fail closed: NEXT may derive its public `/verify` route from the site origin, while a Production build exposes no independent-verifier link unless an explicit HTTPS verifier URL is configured. Private Distribution verifier source `85fa6a6961ba2bb00e2d09d92e6a9f5f815a3478` independently verifies the NEXT TEST Root → keyset → REL chain, local artifact digest/size and pasted TEST ATT evidence while stating that NEXT cannot become `Official LightBI — verified` and that OS publisher identity is a separate trust plane. Live `https://lightbi-next.thaiduy.digital/verify` remains TEST-only. Exact-head verification passes private Distribution **204/204**, public focused identity/updater **24/24**, Rust updater **5/5** with one live-only acceptance test intentionally ignored, desktop production build, and the authoritative `test:release-1.0` suite with governed regression **39/39**. The final public commit is visible on GitHub, but there is **no GitHub Actions workflow run at that exact head**, so no CI claim is made.

R1-P12 itself remains a NEXT anti-impersonation proof until a current acceptance artifact is bound to the LightBI TEST Root → REL chain, its exact digest/size is independently verified, and a TEST ATT certificate is proven release-bound with tamper/fork negatives failing closed. Windows Authenticode and macOS Developer ID/notarization are recorded future platform-hardening debt, not current LightBI-origin authority. Production remains separately owner-gated and `official_verified` remains unreachable there until a Production LightBI Root/REL/ATT/verifier chain is explicitly authorized.

Owner platform-stabilization decision on 2026-09-02: **Windows is the active Road-to-1.0 stabilization lane**. The owner has already tested the macOS Beta and observed substantial product defects, so macOS product repair, Developer ID/notarization and Linux validation are deferred. On 2026-09-03 the owner clarified that platform-vendor signing (Windows Authenticode/Microsoft ecosystem credentials and Apple Developer ID) is future hardening, not the R1-P12 LightBI anti-impersonation signature. The active R1-P12 signing authority is the LightBI Root → REL → exact artifact → ATT chain plus independent verification. Production use of that authority remains separately owner-gated.
...
`Phase 2A re-audit → explicit freeze → offline Root ceremony → purpose-separated private signer → REL → ATT → signed ENT → private PRO delivery → LightBI Root/REL/ATT anti-impersonation closure → RC/1.0`. OS-vendor publisher signing remains a separate future platform-hardening track unless a later owner-approved release policy makes it mandatory.
