...
R1-P12 itself is **not complete**. The current Windows NEXT installer has PE certificate-table offset/size `0/0` and therefore no Authenticode signature. The macOS validation workflow explicitly builds and validates an **ad-hoc signed** DMG, not Apple Developer ID signing/notarization. Production also has no configured external verification authority, and Production REL/ATT authority remains blocked behind the missing R1-P5 owner freeze. Therefore `official_verified` must remain unreachable for Production, R1-P13 (`RC → stable 1.0`) remains closed, and no branding, HTTPS, SHA-256 match, TEST REL/ATT evidence or client UI may be substituted for real supported-platform publisher trust.

Owner platform-stabilization decision on 2026-09-02: **Windows is the active Road-to-1.0 stabilization lane**. The owner has already tested the macOS Beta and observed substantial product defects, so macOS Beta defects plus Developer ID/notarization work are deferred debt; Linux validation is likewise deferred until the Windows application is genuinely stable. This is an execution-order decision, not a declaration that LightBI 1.0 is permanently Windows-only. A deferred platform must not be advertised as stable or `official_verified` until that platform later passes its own product, package and publisher-trust gates. For the active R1-P12 lane, Windows Authenticode plus Production REL/ATT/external-verifier evidence remain blocking; Apple publisher trust remains recorded debt but does not block Windows stabilization work.
