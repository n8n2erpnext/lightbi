...
`packaged Windows/native Trust + UAT -> integrated release acceptance`
...
**Signed Transport V2 NEXT server/edge runtime acceptance is complete on NEXT033.** Real HTTPS proves accepted device-signed traffic plus response sequence/body-digest correlation; replay, canonical-query mutation and raw-body mutation fail closed, and the same probes remain green after restarting CP/worker/Core/Gateway plus the verification-only attestation container. The runtime verifier/Distribution boundary still has no signer authority. The next execution gate is packaged Windows/native Trust + UAT: publish/verify the exact TEST runtime REL for the NEXT `LightBI.exe`, exercise `ensure_installation_trust` and the OS-protected device key, then prove actual packaged `native_http_request` no-downgrade behavior before owner acceptance and integrated release acceptance. Historical R1-P1/R1-P4 owner-acceptance items remain open where recorded; Production R1-P5/P6 authority remains separately owner-gated.
...
NEXT033 now accepts the V2 server/edge runtime on product `6c1f117` / CP `3afb85b`: external HTTPS positive traffic receives response digest/sequence correlation, replay/query/body tamper fail closed, and restart persistence is green. Packaged-native acceptance remains distinct. The current TEST Trust publication has no `LightBI.exe` runtime REL for `release:0.9.2-beta.7-next.33:windows:x86_64:runtime`; therefore actual Windows installation-certificate issuance, OS-protected device-key persistence, Tauri `native_http_request` and no-WebView-downgrade behavior must be proven in packaged Windows/UAT rather than inferred from the operator-only TEST harness.
