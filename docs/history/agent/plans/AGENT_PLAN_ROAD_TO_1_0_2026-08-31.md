...
`install canonical NEXT033 CI artifact -> owner packaged-native installation trust / V2 no-downgrade proof -> integrated release acceptance`
...
The active immutable NEXT generation is now `g-2026-09-05-next-033`, parent `g-2026-09-04-next-032`, product source `6c1f117`, reused server-binary provenance Core `ed4b9233`, private CP runtime `3afb85b`, schema `067_catalog_quarterly_pricing`, manifest SHA-256 `75bece214fed73acf836caf1421f128b476bfb890d8f29dd9ba9d8d861c5ced9`. Core/Gateway/CP/worker all report NEXT033, the worker is healthy, and Trust remains `blocked_pending_phase2a_freeze`. Owner role rotation has not been performed.
...
**Signed Transport V2 NEXT server/edge runtime acceptance is complete on NEXT033, and the canonical Windows runtime REL/server issuance sub-gate is now machine-closed.** GitHub Actions run `33933622073` built exact `6c1f117...` / `3afb85b...`; canonical `LightBI.exe` is SHA-256 `77952884...dae9d` at 76,444,672 bytes. The exact TEST runtime REL is externally published and cryptographically verified, and after reloading the snapshot-based installation issuer an external exact-runtime challenge/device-signature/certificate issue passes under non-promotable TEST authority. The first owner packaged restart has now failed closed as `installation_issuer_runtime_artifact_mismatch`, proving the currently installed executable is not the canonical REL-authorized byte identity. The remaining gate is therefore to install the canonical CI artifact, persist the OS-protected device identity/certificate on that exact runtime, then exercise actual packaged `native_http_request` and no-WebView-downgrade/restart behavior. Historical R1-P1/R1-P4 owner-acceptance items remain open where recorded; Production R1-P5/P6 authority remains separately owner-gated.
...
NEXT033 accepts the V2 server/edge runtime on product `6c1f117` / CP `3afb85b`: external HTTPS positive traffic receives response digest/sequence correlation, replay/query/body tamper fail closed, and restart persistence is green. The missing-runtime-REL blocker is now cleared: canonical Windows run `33933622073` produced `LightBI.exe` SHA-256 `77952884...dae9d`, the exact TEST runtime REL is live and independently verified, and the installation issuer successfully issues a TEST certificate for that exact release after its catalog snapshot is reloaded. Packaged-native owner acceptance remains distinct. The first owner restart was rejected as `installation_issuer_runtime_artifact_mismatch`, so the currently installed bytes are not canonical. The owner must install the canonical CI artifact, restart it, and issuer/attestation runtime evidence must then prove the exact runtime/certificate on its OS-protected device key plus real Tauri `native_http_request` and the no-WebView-downgrade/restart contract. The current Build Identity panel is not used as REL/ATT proof because it does not consume that evidence path. OS publisher remains unverified/NotSigned and is not upgraded by TEST REL evidence.
