...
DPR-0 through DPR-7 are CLOSED. The active implementation phase is DPR-8 Canvas-first surface refactor + shared UI system. DPR-8 must consume the semantic Question/Narrative/Visualization/Dashboard contracts already closed, apply the product-wide Information Architecture + Density Contract and shared `@lightbi/ui` primitives/tokens across Desktop/Tauri and Web Live Demo `/app`, flatten card-everywhere surfaces without reducing analytical capacity, preserve accessibility/responsiveness/governed evidence, and keep platform divergence behind explicit native capability boundaries. DPR-8 changes layout/presentation, not metric/formula/join authority. Documentation work must continue to follow `docs/project-book/LIBRARY_RULES.md`. Foundation, Understanding, Home / New Brief, Decision Workspace and Deep BA are source/live-accepted; BA Step 2 is the current and final bounded DPR-8 presentation target before phase closure.
## 24. Source bookmarks
