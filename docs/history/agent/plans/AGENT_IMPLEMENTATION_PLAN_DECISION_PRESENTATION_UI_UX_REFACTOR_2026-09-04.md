...
Status: **DPR-5 CLOSED — VISUALIZATION ONTOLOGY + CHART PATTERN LIBRARY LIVE-ACCEPTED ON NEXT; DPR-6 VISUALIZATION PLANNER + RENDERER/TYPE PRESERVATION ACTIVE**
Date: 2026-09-04
...
This document remains the refactor plan. The 2026-09-06 MB presentation-advisory foundation is source-closed separately at product commit `4be593ae57b4b1385a833675dd4ea2349900d378`; DPR-2 through DPR-5 now consume or project bounded MB presentation advice while deterministic governed authority remains upstream. DPR-5 established canonical semantic pattern eligibility but deliberately did not grant MB direct chart-selection, renderer, metric, formula, join or runtime-execution authority. Production promotion remains owner-gated.

DPR-0 through DPR-5 are CLOSED. The active implementation phase is DPR-6 Visualization Planner + renderer/type preservation. DPR-6 must resolve `analytical intent -> ChartPatternDefinition -> renderer family`, reuse the DPR-5 deterministic suitability gate so every MB/domain suggestion can be rejected, preserve pattern rules through persistence/dashboard/export/evidence drill-down, and retire the explicit `dashboard-renderer-type-collapse` debt only with proof that requested types survive persistence/rendering. Governed metric/evidence authority remains upstream and MB remains advisory. Broad visual-system migration remains owned by DPR-8 after DPR-6 and DPR-7 planner work. Documentation work must continue to follow `docs/project-book/LIBRARY_RULES.md`.
## 24. Source bookmarks
