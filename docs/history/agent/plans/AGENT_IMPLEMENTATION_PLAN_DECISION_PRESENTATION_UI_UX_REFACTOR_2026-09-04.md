...
Status: **DPR-6 CLOSED — VISUALIZATION PLANNER + RENDERER/TYPE PRESERVATION LIVE-ACCEPTED ON NEXT; DPR-7 DASHBOARD COMPOSITION PLANNER ACTIVE**
Date: 2026-09-04
...
This document remains the refactor plan. The 2026-09-06 MB presentation-advisory foundation is source-closed separately at product commit `4be593ae57b4b1385a833675dd4ea2349900d378`; DPR-2 through DPR-6 now consume or project bounded MB presentation advice while deterministic governed authority remains upstream. DPR-5 established canonical semantic pattern eligibility and DPR-6 established deterministic renderer/type resolution; neither grants MB direct renderer, metric, formula, join or runtime-execution authority. Production promotion remains owner-gated.

DPR-0 through DPR-6 are CLOSED. The active implementation phase is DPR-7 Dashboard Composition Planner. DPR-7 must compose existing governed/narrative/visualization outputs into semantic Dashboard roles and story order, enforce distinct evidence-backed management questions plus narrative/visual duplication and information-budget checks, admit MB domain/perspective/dashboard-role advice only as a rejectable prior, and remove hard-coded Bar breakdown generation as the default without changing governed metric/evidence authority. Broad canvas-first surface migration remains owned by DPR-8 after DPR-7 planner closure. Documentation work must continue to follow `docs/project-book/LIBRARY_RULES.md`.
## 24. Source bookmarks
