...
**Implementation has advanced into MB-LQ3 on NEXT, with the real R2 transfer rehearsal still OPEN.** MB-LQ0 contracts/migration design are source-closed at Control Plane `00e8abb7aed68d2fa872dd4758c0443819ba1527`. MB-LQ1 durable Postgres jobs, Redis Streams dispatch/rehydration, lease-governed async runtime and diagnostics are source-closed at `77dd7df5804da725169bd4c2260b1c7c944d7134`; NEXT async systemd unit templates are recorded at `84099b3ea7470243519682b7604a3f64da45f21e`. MB-LQ2 scheduler, `server|client` execution-target separation, bounded backpressure/fairness and Admin Queue/Scheduler controls are source-closed at `dbf7bfeb77cac8bbabff4b1525b6e7efe3665697`.

MB-LQ3 direct contribution source is now split across Product `65092cd86c7c63c861109a6c01796f1c35a62978` and Control Plane `387db91e1123cb9e6da6d565951c393fd43a65bf`, with live readiness hardening at descendant CP `c302f9935f2d7809167838defffc19f9be24af15`. Desktop polls an assigned client job, builds a privacy-safe gzip/NDJSON package locally, stages only under the LightBI app-cache outbox, and streams single or 16 MiB multipart ranges directly to short-lived R2 SigV4 URLs. The Control Plane carries only bounded signed control metadata; it does not proxy large learning payloads. Server validation binds the request to the attestation-verified installation identity, verifies size/hash/schema/privacy and preserves durable multipart/upload state in PostgreSQL. Consent withdrawal aborts the contribution, removes the local staged package and requires R2 cleanup before durable abort completion; transport failures retry rather than being mislabeled as privacy rejection.

Migration `071_micro_brain_learning_r2_transport` was applied only after status showed it as the sole pending migration. NEXT now reports `25` migrations and `pending=0`. Exact CP candidate `c302f99...` passes the full compiled suite `275/275`; Product `test:release-1.0` and governed regression `41/41` pass, native Tauri tests pass `28` with `0` failures and one live-only ignored test, and the attestation verifier suite passes `18/18`. Live NEXT API, legacy worker and all three async workers `short/default/long` run exact CP `c302f99...`. The R2 broker socket is deliberately absent because a dedicated learning-only credential has not yet been provisioned; readiness gating therefore issues no client collection jobs (`0` active and `0` new in the observed 15-minute window) rather than falling back to shared Production R2 authority.

The attestation verifier was rebuilt as `lightbi-next-trust-attestation:387db91e1123-trust-10de4da8`, preserving Trust Contracts `10de4da8e551a46f93f7b62985a0a6e611581b8e` and non-production/non-signing provenance. A live Signed Transport V2 probe for `/api/micro-brain/learning/poll` on the exact NEXT046 runtime returned the verified installation binding and exact release with `productionAuthority=false`. Issuer PID `3817149` and signer PID `12020` were not restarted. Immutable NEXT047 acceptance candidate `2ba518263d7ef5072de1b364dfc977ee1f4515ef` pins Product `65092cd...`, CP `c302f99...`, schema `071`, generation `g-2026-09-07-next-047` and parent NEXT046; its push-triggered Actions result is intentionally untracked and is not claimed PASS here.

**MB-LQ3 is not fully closed yet.** Every R2 key currently present on the VPS was fingerprint-checked and resolves to the existing shared Production-wide R2 credential/bucket, so none was reused for learning. No Cloudflare API credential is available on the VPS to mint a least-privileged key automatically. The remaining machine gate is to provision a dedicated NEXT learning-only R2 credential/bucket, activate the broker, run a real direct-upload/multipart/resume/validation/dedup/reject/cleanup rehearsal, and only then promote MB-LQ3 to complete. Production remains untouched.
