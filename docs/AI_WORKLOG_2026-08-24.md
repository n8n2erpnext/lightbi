...
- Post-release portal correction: the three SVG evidence waves now combine direct SVG path morphing with cross-browser transform animation. Their left edge reaches the Windows download CTA and the primary line has stronger contrast. Windows reduced-motion preferences retain slow movement instead of freezing the lines.
- Release navigation now carries a dedicated archive URL through the release catalog/config contract. **View all GitHub releases** always targets the repository release list, while R2 artifact URLs remain exclusive to the Windows/Linux download buttons. Distribution verification increased to 32/32 tests.
- The live VPS exposed a legacy `LIGHTBI_RELEASE_URL` containing the old Windows installer, proving the archive contract still inherited a download fallback. The backend now owns a separate `LIGHTBI_RELEASE_ARCHIVE_URL` (defaulting to the repository release list); catalog fallback and config use it independently from the installer fallback. Verification increased to 33/33 tests.
