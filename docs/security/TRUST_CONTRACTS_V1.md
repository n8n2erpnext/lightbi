...
Each path verifies root → keyset → purpose-matched issuer → payload signature, then applies its own lifecycle policy. REL, ATT and PRO payload schemas bind `product_id` exactly to `digital.thaiduy.lightbi`; a LightBI-rooted signature cannot be relabeled as another product. There is no public generic `/sign` concept and no private signer implementation in Phase 2A.
